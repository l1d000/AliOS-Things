...
#if (BLE_APP_FEB3)
#include "ais_sdk.h"              // AIS Module Definition
#include "AIS_task.h"
#endif //(BLE_APP_FEB3)

#if (BLE_APP_MS)
#include "app_ms.h"              // AIS Module Definition
#include "ms_task.h"
#endif //(BLE_APP_MS)

#if (BLE_APP_BTL)
#include "app_btl.h"              // BTL Module Definition
#include "btl_pub.h"
#include "btl_task.h"
#endif //(BLE_APP_BTL)

#if (BLE_APP_SN)
#include "app_sn.h"              // SN Module Definition
#include "sn_pub.h"
#include "sn_task.h"
#endif //(BLE_APP_SN)
                                ...
                                kernel_task_id_t const src_id)
{
    ...
    switch(param->operation)
    {
        ...
        case (GAPM_PROFILE_TASK_ADD):
        {
            ...
            if ((BLE_ROLE_MASTER != ble_get_role_mode()) && (!appm_add_svc()))
            {
                ...
                else
                {
                    if(ble_event_cb)
                    {
                        ble_event_cb(BLE_STACK_FAIL, NULL);
                    }					
                }
            }
        }
                            ...
                            kernel_task_id_t const src_id)
{
    ...
    kernel_task_id_t src_task_id = MSG_T(msgid);
    // Message policy
