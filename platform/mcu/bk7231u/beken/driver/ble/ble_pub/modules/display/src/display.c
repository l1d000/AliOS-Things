...
static void display_user_action(void)
{
    ...
    kernel_event_clear(KERNEL_EVENT_DISPLAY);
            ...
            switch (display_env.disp_action)
            {
                ...
                case DISPLAY_SELECT:
                {
                    ...
                    else if(display_env.disp_mode == DISPLAY_MODE_PERIODIC_OFF)
                    {
                        ...
                        kernel_timer_set(DISPLAY_SCREEN_TO, TASK_DISPLAY, DISPLAY_SCREEN_1ST_TIMER_DURATION);
                    }
                    else if(display_env.disp_mode == DISPLAY_MODE_PERIODIC_ON)
                    {
                        ...
                        kernel_timer_clear(DISPLAY_SCREEN_TO, TASK_DISPLAY);
                    }
                    #endif // DISPLAY_PERIODIC
                }
                break;
...
{
    // Create DISPLAY Task
    kernel_task_create(TASK_DISPLAY, &TASK_DESC_DISPLAY);

    // Initialize DISPLAY task to idle state
    kernel_state_set(TASK_DISPLAY, DISPLAY_IDLE);

    // Create DISPLAY kernel event
    kernel_event_callback_set(KERNEL_EVENT_DISPLAY, &display_user_action);
...
void display_refresh(void)
{
    ...
    kernel_event_set(KERNEL_EVENT_DISPLAY);
}
