...
static void app_ht_temp_send(void)
{
    ...
    struct htpt_temp_send_req * req = KERNEL_MSG_ALLOC(HTPT_TEMP_SEND_REQ,
...
void app_stop_timer (void)
{
    ...
    {
        kernel_timer_clear(APP_HT_MEAS_INTV_TIMER, TASK_APP);
...
void app_ht_enable_prf(uint8_t conidx)
{
    ...
    struct htpt_enable_req * req = KERNEL_MSG_ALLOC(HTPT_ENABLE_REQ,
                                                prf_get_task_from_id(TASK_ID_HTPT),
                                                TASK_APP,
                                                htpt_enable_req);
 ...
 ****************************************************************************************
 */
static int htpt_meas_intv_chg_req_ind_handler(kernel_msg_id_t const msgid,
                                          ...
                                          kernel_task_id_t const src_id)
{
    ...
    if (app_ht_env.htpt_meas_intv != 0)
    {
        ...
        {
            // Set a Timer
            kernel_timer_set(APP_HT_MEAS_INTV_TIMER, TASK_APP, app_ht_env.htpt_meas_intv*100);
    ...
    // Allocate the message
    struct htpt_meas_intv_chg_cfm * cfm = KERNEL_MSG_ALLOC(HTPT_MEAS_INTV_CHG_CFM,
                                                prf_get_task_from_id(TASK_ID_HTPT),
                                                TASK_APP,
                                                htpt_meas_intv_chg_cfm);

    // Set data
    cfm->conidx = KERNEL_IDX_GET(dest_id);
    cfm->status = 0;
...
static int htpt_temp_send_rsp_handler(kernel_msg_id_t const msgid,
...
static int htpt_cfg_indntf_ind_handler(kernel_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int app_ht_meas_intv_timer_handler(kernel_msg_id_t const msgid,
                                          void const *param,
                                          kernel_task_id_t const dest_id,
                                          kernel_task_id_t const src_id)
{
    ...
    rand_temp_step = (uint32_t)(common_rand_word()%20);
 ...
 ****************************************************************************************
 */
static int app_ht_msg_handler(kernel_msg_id_t const msgid,
                              void const *param,
                              kernel_task_id_t const dest_id,
                              kernel_task_id_t const src_id)
{
...
const struct kernel_msg_handler app_ht_msg_handler_list[] =
{
    // Note: first message is latest message checked by kernel so default is put on top.
    {KERNEL_MSG_DEFAULT_HANDLER,        (kernel_msg_func_t)app_ht_msg_handler},

//  {HTPT_ENABLE_RSP,               (kernel_msg_func_t)htpt_enable_rsp_handler},
    {HTPT_TEMP_SEND_RSP,            (kernel_msg_func_t)htpt_temp_send_rsp_handler},
    {HTPT_MEAS_INTV_CHG_REQ_IND,    (kernel_msg_func_t)htpt_meas_intv_chg_req_ind_handler},
    {HTPT_CFG_INDNTF_IND,           (kernel_msg_func_t)htpt_cfg_indntf_ind_handler},

    {APP_HT_MEAS_INTV_TIMER,        (kernel_msg_func_t)app_ht_meas_intv_timer_handler},
};

const struct kernel_state_handler app_ht_table_handler =
    {&app_ht_msg_handler_list[0], (sizeof(app_ht_msg_handler_list)/sizeof(struct kernel_msg_handler))};
