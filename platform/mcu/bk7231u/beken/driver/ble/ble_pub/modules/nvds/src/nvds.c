...
#define NVDS_MAX_STORAGE_SIZE    0x1000 // 4KB
#endif //(NVDS_READ_WRITE == 1)

// NVDS Mapping

/// Magic number offset
#define NVDS_MAGIC_NUMBER_ADDRESS         0x41000   //260k
...
uint8_t nvds_init(uint32_t *base, uint32_t len)
{
...
uint8_t nvds_get(uint8_t tag, nvds_tag_len_t * lengthPtr, uint8_t *buf)
{
    //UART_PRINTF("nvds_get\r\n");
	return NVDS_FAIL;
  
}
uint8_t nvds_put(uint8_t tag, nvds_tag_len_t length, uint8_t *buf)
{
	//UART_PRINTF("nvds_null_init\r\n");
	return (NVDS_OK);
}

uint8_t nvds_del(uint8_t tag)
{
	//UART_PRINTF("nvds_del\r\n");
  	return (NVDS_OK);
}

uint8_t nvds_lock(uint8_t tag)
{
	//UART_PRINTF("nvds_lock\r\n");
  	return (NVDS_OK);
}
