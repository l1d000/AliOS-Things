...
const struct rwip_prio rwip_priority[RWIP_PRIO_IDX_MAX]={
    ...
    #if MAX_NB_SYNC
    {RWIP_PRIO_SCOMMON_DFT,       RWIP_INCR_SCOMMON_DFT},
    #endif //MAX_NB_SYNC
...
uint8_t rwip_sleep(void)
{
	...
	if(ble_ps_enabled())
	{
	sleep_duration = MAX_SLEEP_DURATION_SHORT_WAKEUP;
	}
	else
	{
    ...
    do
    {
        ...
        rwbt_sleep_enter(rwip_slot_2_lpcycles(sleep_duration), rwip_env.ext_wakeup_enable);
        #elif (BLE_EMB_PRESENT)
        // Put BT core into deep sleep
