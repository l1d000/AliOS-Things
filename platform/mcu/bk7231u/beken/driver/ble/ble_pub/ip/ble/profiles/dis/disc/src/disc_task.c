 ...
 ****************************************************************************************
 */
static int disc_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if((state == DISC_IDLE) && (disc_env->env[conidx] == NULL))
    {
        ...
        disc_env->env[conidx] = (struct disc_cnx_env*) kernel_malloc(sizeof(struct disc_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, DISC_DISCOVERING);
        }
                                ...
                                kernel_task_id_t const src_id)
{
    ...
    else if (state == DISC_BUSY)
    {
        ...
        kernel_state_set(dest_id, DISC_IDLE);
    }
 ...
 ****************************************************************************************
 */
static int disc_rd_char_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if((state == DISC_IDLE) && (disc_env->env[conidx] != NULL))
    {
        ...
        if (search_hdl != ATT_INVALID_HDL)
        {
            ...
            kernel_state_set(dest_id, DISC_BUSY);
        }
        else
        {
                                  ...
                                  kernel_task_id_t const src_id)
{
    ...
    if(state == DISC_BUSY)
    {
        ...
        ASSERT_INFO(disc_env != NULL, dest_id, src_id);
        ASSERT_INFO(disc_env->env[KERNEL_IDX_GET(dest_id)] != NULL, dest_id, src_id);
        prf_client_att_info_rsp(&(disc_env->prf_env), KERNEL_IDX_GET(dest_id), DISC_RD_CHAR_RSP,
...
const struct kernel_msg_handler disc_default_state[] =
{
    {DISC_RD_CHAR_REQ,       (kernel_msg_func_t)disc_rd_char_req_handler},
...
const struct kernel_state_handler disc_default_handler = KERNEL_STATE_HANDLER(disc_default_state);
