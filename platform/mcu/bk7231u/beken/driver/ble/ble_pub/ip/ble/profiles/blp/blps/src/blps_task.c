 ...
 ****************************************************************************************
 */
static int blps_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
     ...
     else if(kernel_state_get(dest_id) == BLPS_IDLE)
     {
     ...
     struct blps_enable_rsp *cmp_evt = KERNEL_MSG_ALLOC(BLPS_ENABLE_RSP, src_id, dest_id, blps_enable_rsp);
...
static int gattc_read_req_ind_handler(kernel_msg_id_t const msgid,
                                      ...
                                      kernel_task_id_t const src_id)
{
    if(kernel_state_get(dest_id) == BLPS_IDLE)
    {
        // Get the address of the environment
        struct blps_env_tag *blps_env = PRF_ENV_GET(BLPS, blps);
 ...
 ****************************************************************************************
 */
static int blps_meas_send_req_handler(kernel_msg_id_t const msgid,
                                      ...
                                      kernel_task_id_t const src_id)
{
    ...
    else if(kernel_state_get(dest_id) == BLPS_IDLE)
    {
        ...
        if (status == GAP_ERR_NO_ERROR)
        {
            ...
            // Go to busy state
            kernel_state_set(dest_id, BLPS_BUSY);
        }
    }
    else
    {
                                      ...
                                      kernel_task_id_t const src_id)
{
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        struct blps_cfg_indntf_ind * ind = KERNEL_MSG_ALLOC(
                BLPS_CFG_INDNTF_IND,
                                 ...
                                 kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    ...
    switch(param->operation)
    {
        ...
        case GATTC_INDICATE:
        {
            ...
            // Go to idle state
            kernel_state_set(dest_id, BLPS_IDLE);
        }
        break;
...
const struct kernel_state_handler blps_default_handler = KERNEL_STATE_HANDLER(blps_default_state);
