 ...
 ****************************************************************************************
 */
static int rscpc_enable_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if ((state == RSCPC_IDLE) && (rscpc_env->env[conidx] == NULL))
    {
        ...
        rscpc_env->env[conidx] = (struct rscpc_cnx_env*) kernel_malloc(sizeof(struct rscpc_cnx_env), KERNEL_MEM_ATT_DB);
        ...
        if (param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, RSCPC_DISCOVERING);
        }
 ...
 ****************************************************************************************
 */
static int rscpc_read_cmd_handler(kernel_msg_id_t const msgid,
    ...
    if (state == RSCPC_IDLE)
    {
        ...
        else    // State is RSCPC_CONNECTED
        {
            ...
            if (handle != ATT_INVALID_SEARCH_HANDLE)
            {
                ...
                kernel_state_set(dest_id, RSCPC_BUSY);
 ...
 ****************************************************************************************
 */
static int rscpc_cfg_ntfind_cmd_handler(kernel_msg_id_t const msgid,
                                        ...
                                        kernel_task_id_t const src_id)
{
    ...
    if (rscpc_env != NULL)
    {
        ...
        do
        {
            if (kernel_state_get(dest_id) != RSCPC_IDLE)
            {
                // Another procedure is pending, keep the command for later
                msg_status = KERNEL_MSG_SAVED;
 ...
 ****************************************************************************************
 */
static int rscpc_ctnl_pt_cfg_req_handler(kernel_msg_id_t const msgid,
                                         ...
                                         kernel_task_id_t const src_id)
{
    ...
    if (rscpc_env != NULL)
    {
        ...
        {
            // State is Connected or Busy
            ASSERT_ERR(kernel_state_get(dest_id) > RSCPC_FREE);
 ...
 ****************************************************************************************
 */
static int rscpc_timeout_timer_ind_handler(kernel_msg_id_t const msgid,
                                           void const *param,
                                           kernel_task_id_t const dest_id,
                                           kernel_task_id_t const src_id)
{
                ...
                case GATTC_WRITE_NO_RESPONSE:
                {
                    ...
                    else if (operation == RSCPC_CTNL_PT_CFG_WR_OP_CODE)
                    {
                        if (param->status == GAP_ERR_NO_ERROR)
                        {
                            ...
                            kernel_timer_set(RSCPC_TIMEOUT_TIMER_IND, dest_id, ATT_TRANS_RTX);
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if (kernel_state_get(dest_id) == RSCPC_BUSY)
    {
        uint8_t conidx = KERNEL_IDX_GET(dest_id);
        ...
        struct rscpc_value_ind *ind = KERNEL_MSG_ALLOC(RSCPC_VALUE_IND,
    ...
    if (rscpc_env != NULL)
    {
        switch (param->type)
        {
            case (GATTC_NOTIFY):
            {
                ...
                // Instantaneous Speed
                ind->value.rsc_meas.inst_speed = common_read16p((uint16_t *)&param->value[1]);
                ...
                {
                    ind->value.rsc_meas.inst_stride_len = common_read16p(&param->value[offset]);
                    offset += 2;
                }
                ...
                {
                    ind->value.rsc_meas.total_dist = common_read32p(&param->value[offset]);
                }
            ...
            case (GATTC_INDICATE):
            {
                ...
                if (rscpc_env->env[conidx]->operation != NULL)
                {
                    if (((struct rscpc_cmd *)rscpc_env->env[conidx]->operation)->operation == RSCPC_CTNL_PT_CFG_IND_OP_CODE)
                    {
                        ...
                        kernel_timer_clear(RSCPC_TIMEOUT_TIMER_IND, dest_id);

                        // RSC Measurement value has been received
                        struct rscpc_ctnl_pt_cfg_rsp *ind = KERNEL_MSG_ALLOC(RSCPC_CTNL_PT_CFG_RSP,
                                prf_dst_task_get(&(rscpc_env->prf_env), conidx),
                                prf_src_task_get(&(rscpc_env->prf_env), conidx),
...
const struct kernel_msg_handler rscpc_default_state[] =
{
    ...
    {RSCPC_CTNL_PT_CFG_REQ,          (kernel_msg_func_t)rscpc_ctnl_pt_cfg_req_handler},
    {RSCPC_TIMEOUT_TIMER_IND,        (kernel_msg_func_t)rscpc_timeout_timer_ind_handler},
...
const struct kernel_state_handler rscpc_default_handler = KERNEL_STATE_HANDLER(rscpc_default_state);
