 ...
 ****************************************************************************************
 */
static int hogprh_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if((state == HOGPRH_IDLE) && (hogprh_env->env[conidx] == NULL))
    {
        ...
        hogprh_env->env[conidx] = (struct hogprh_cnx_env*) kernel_malloc(sizeof(struct hogprh_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, HOGPRH_BUSY);
            hogprh_env->env[conidx]->operation = kernel_param2msg(param);
 ...
 ****************************************************************************************
 */
static int hogprh_read_info_req_handler(kernel_msg_id_t const msgid, struct hogprh_read_info_req const *param,
                                      kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        struct hogprh_read_info_rsp * rsp = KERNEL_MSG_ALLOC(HOGPRH_READ_INFO_RSP,
 ...
 ****************************************************************************************
 */
static int hogprh_write_req_handler(kernel_msg_id_t const msgid, struct hogprh_write_req *param,
                                      kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    int msg_status = KERNEL_MSG_CONSUMED;
    uint8_t state = kernel_state_get(dest_id);
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        struct hogprh_write_rsp * rsp = KERNEL_MSG_ALLOC(HOGPRH_WRITE_RSP,
            ...
            && (hogprh_env->env[conidx]->operation != NULL))
    {
        switch(hogprh_env->env[conidx]->operation->id)
        {
            case HOGPRH_ENABLE_REQ:
            {
                ...
                if (param->status == ATT_ERR_NO_ERROR)
                {
                    ...
                    if(hogprh_env->env[conidx]->hids_nb > 0)
                    {
                        uint8_t report_idx;
                        uint8_t hid_idx;
                        for (hid_idx = 0 ; (hid_idx < common_min(hogprh_env->env[conidx]->hids_nb, HOGPRH_NB_HIDS_INST_MAX))
            ...
            case HOGPRH_READ_INFO_REQ:
            {
                ...
                {
                    struct hogprh_read_info_req* req = (struct hogprh_read_info_req*) kernel_msg2param(hogprh_env->env[conidx]->operation);
                    struct hogprh_read_info_rsp * rsp = KERNEL_MSG_ALLOC(HOGPRH_READ_INFO_RSP,
            ...
            {
                struct hogprh_write_req* req = (struct hogprh_write_req*) kernel_msg2param(hogprh_env->env[conidx]->operation);
        ...
        kernel_free(hogprh_env->env[conidx]->operation);
        hogprh_env->env[conidx]->operation = NULL;
        kernel_state_set(dest_id, HOGPRH_IDLE);
    }
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if(state == HOGPRH_BUSY)
    {
        ...
        if(hogprh_env->env[conidx]->operation->src_id == dest_id)
        {
            ...
            struct hogprh_report_ind* ind = KERNEL_MSG_ALLOC_DYN(HOGPRH_REPORT_IND,
        ...
        {
            struct hogprh_read_info_rsp * rsp = KERNEL_MSG_ALLOC_DYN(HOGPRH_READ_INFO_RSP,
            ...
            switch(req->info)
            {
                ...
                case HOGPRH_REPORT_MAP_EXT_REP_REF:
                {
                    ...
                    memcpy(rsp->data.report_map_ref.uuid, param->value, common_min(param->length, ATT_UUID_128_LEN));
                }break;
                /// HID Information
                case HOGPRH_HID_INFO:
                {
                    rsp->data.hid_info.bcdHID       = common_read16p(param->value);
                ...
                case HOGPRH_REPORT_NTF_CFG:
                {
                    rsp->data.report_cfg = common_read16p(param->value);
                }break;
                default:
                {
                    ASSERT_ERR(0);
            ...
            for (report_idx = 0; (report_idx < HOGPRH_NB_REPORT_INST_MAX) && !finished; report_idx++)
            {
                if (param->handle == hogprh_env->env[conidx]->hids[hid_nb].chars[HOGPRH_CHAR_REPORT + report_idx].val_hdl)
                {
                    ...
                    else
                    {
                        ...
                        struct hogprh_read_info_req* req = KERNEL_MSG_ALLOC(HOGPRH_READ_INFO_REQ,
...
const struct kernel_msg_handler hogprh_default_state[] =
{
...
const struct kernel_state_handler hogprh_default_handler = KERNEL_STATE_HANDLER(hogprh_default_state);
