...
static uint8_t bass_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct bass_db_cfg* params)
{
    ...
    if ((params->bas_nb > 0) && (params->bas_nb <= BASS_NB_BAS_INSTANCES_MAX))
    {
        ...
        bass_env = (struct bass_env_tag* ) kernel_malloc(sizeof(struct bass_env_tag), KERNEL_MEM_ATT_DB);
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        *start_hdl = shdl[0];
        ...
        kernel_state_set(env->task, BASS_IDLE);
    }
    else if(bass_env != NULL)
    {
        kernel_free(bass_env);
    }
...
static void bass_destroy(struct prf_task_env* env)
{
    ...
    {
        kernel_free(bass_env->operation);
    }
...
static void bass_notify_batt_lvl(struct bass_env_tag* bass_env, uint8_t conidx, uint8_t svc_idx)
{
    ...
    struct gattc_send_evt_cmd *batt_lvl = KERNEL_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
            KERNEL_BUILD_ID(TASK_GATTC, conidx), prf_src_task_get(&(bass_env->prf_env),0),
...
void bass_exe_operation(void)
{
    ...
    {
        struct bass_enable_req * enable = (struct bass_enable_req *) kernel_msg2param(bass_env->operation);
    ...
    {
        struct bass_batt_level_upd_req * update = (struct bass_batt_level_upd_req *) kernel_msg2param(bass_env->operation);
    ...
    if(finished)
    {
        ...
        if(bass_env->operation->id == BASS_ENABLE_REQ)
        {
            struct bass_enable_rsp * rsp = KERNEL_MSG_ALLOC(BASS_ENABLE_RSP, bass_env->operation->src_id,
        ...
        {
            struct bass_batt_level_upd_rsp * rsp = KERNEL_MSG_ALLOC(BASS_BATT_LEVEL_UPD_RSP, bass_env->operation->src_id,
        ...
        kernel_state_set(prf_src_task_get(&(bass_env->prf_env), 0), BASS_IDLE);
    }
}
