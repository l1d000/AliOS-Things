 ...
 ****************************************************************************************
 */
static int anpc_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if ((state == ANPC_IDLE) && (anpc_env->env[conidx] == NULL))
    {
        ...
        anpc_env->env[conidx] = (struct anpc_cnx_env*) kernel_malloc(sizeof(struct anpc_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if (param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, ANPC_DISCOVERING);
 ...
 ****************************************************************************************
 */
static int anpc_read_cmd_handler(kernel_msg_id_t const msgid,
                                  ...
                                  kernel_task_id_t const src_id)
{
    ...
    if (anpc_env->env[conidx] != NULL)
    {
        ...
        do
        {
            ...
            if (kernel_state_get(dest_id) == ANPC_BUSY)
            {
                ...
                (status == GAP_ERR_NO_ERROR))
            {
                ...
                kernel_state_set(dest_id, ANPC_BUSY);
            }
            else
            {
 ...
 ****************************************************************************************
 */
static int anpc_write_cmd_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if (anpc_env->env[conidx] != NULL)
    {
        ...
        {
            // Check the current state
            if (kernel_state_get(dest_id) != ANPC_IDLE)
            {
                msg_status = KERNEL_MSG_SAVED;
                break;
            }
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if (anpc_env->env[conidx] != NULL)
    {
        ASSERT_ERR(kernel_state_get(dest_id) == ANPC_BUSY);

        // Prepare the indication to send to the application
        struct anpc_value_ind *ind = KERNEL_MSG_ALLOC(ANPC_VALUE_IND,
        ...
        switch (((struct anpc_cmd *)anpc_env->env[conidx]->operation)->operation)
        {
            ...
            case (ANPC_READ_OP_CODE):
            {
                ...
                ind->value.ntf_cfg = common_read16(&param->value[0]);
            } break;
    ...
    if (anpc_env->env[conidx] != NULL)
    {
        ...
        if (param->value[0] < CAT_ID_NB)
        {
            ...
            if (param->handle == anpc_env->env[conidx]->ans.chars[ANPC_CHAR_NEW_ALERT].val_hdl)
            {
                ...
                struct anpc_value_ind *new_alert_ind = KERNEL_MSG_ALLOC_DYN(ANPC_VALUE_IND,
                        prf_dst_task_get(&anpc_env->prf_env, conidx),
            ...
            {
                // Send an indication to the application
                struct anpc_value_ind *unrd_alert_ind = KERNEL_MSG_ALLOC(ANPC_VALUE_IND,
                        prf_dst_task_get(&anpc_env->prf_env, conidx),
                        prf_src_task_get(&anpc_env->prf_env, conidx),
                        anpc_value_ind);
                ...
                // Send the message
                kernel_msg_send(unrd_alert_ind);
            }
            else
            {
                ASSERT_ERR(0);
...
const struct kernel_msg_handler anpc_default_state[] =
{
...
const struct kernel_state_handler anpc_default_handler = KERNEL_STATE_HANDLER(anpc_default_state);
