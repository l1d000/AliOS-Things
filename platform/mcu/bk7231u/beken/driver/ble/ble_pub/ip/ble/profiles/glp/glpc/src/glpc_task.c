 ...
 ****************************************************************************************
 */
static int glpc_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if((state == GLPC_IDLE) && (glpc_env->env[conidx] == NULL))
    {
        ...
        glpc_env->env[conidx] = (struct glpc_cnx_env*) kernel_malloc(sizeof(struct glpc_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, GLPC_DISCOVERING);
        }
        ...
        switch(param->operation)
        {
            ...
            case GATTC_WRITE_NO_RESPONSE:
            {
                ...
                if(glpc_env->env[conidx]->last_uuid_req == ATT_CHAR_REC_ACCESS_CTRL_PT)
                {
                    ...
                    {
                        struct glpc_racp_rsp * rsp = KERNEL_MSG_ALLOC(GLPC_RACP_RSP,
                        ...
                        // stop timer.
                        kernel_timer_clear(GLPC_RACP_REQ_TIMEOUT, dest_id);
                ...
                else
                {
                    ...
                    {
                        struct glpc_register_rsp * cfm = KERNEL_MSG_ALLOC(GLPC_REGISTER_RSP,
                                prf_dst_task_get(&(glpc_env->prf_env), conidx), dest_id,
                                glpc_register_rsp);
                        // set error status
            ...
            case GATTC_READ:
            {
                if(param->status != GAP_ERR_NO_ERROR)
                {
                    struct glpc_read_features_rsp * rsp = KERNEL_MSG_ALLOC(GLPC_READ_FEATURES_RSP,
                            prf_dst_task_get(&(glpc_env->prf_env), conidx), dest_id,
 ...
 ****************************************************************************************
 */
static int glpc_read_features_req_handler(kernel_msg_id_t const msgid,
                                          void const *param,
                                          kernel_task_id_t const dest_id,
                                          kernel_task_id_t const src_id)
{
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if(state == GLPC_IDLE)
    {
        ...
        rsp->features = common_read16p(param->value);
 ...
 ****************************************************************************************
 */
static int glpc_register_req_handler(kernel_msg_id_t const msgid,
                                     ...
                                     kernel_task_id_t const src_id)
{
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        struct glpc_register_rsp * cfm = KERNEL_MSG_ALLOC(GLPC_REGISTER_RSP, src_id, dest_id, glpc_register_rsp);
        // set error status
        cfm->status = status;
        kernel_msg_send(cfm);
    }
 ...
 ****************************************************************************************
 */
static int glpc_racp_req_handler(kernel_msg_id_t const msgid,
                                     ...
                                     kernel_task_id_t const src_id)
{
    ...
    if(state == GLPC_IDLE)
    {
        ...
        else
        {
            ...
            if(status == GAP_ERR_NO_ERROR)
            {
                ...
                kernel_timer_set(GLPC_RACP_REQ_TIMEOUT, dest_id, GLPC_RACP_TIMEOUT);
            }
        }
    }
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        struct glpc_racp_rsp * rsp = KERNEL_MSG_ALLOC(GLPC_RACP_RSP, src_id, dest_id, glpc_racp_rsp);
        // set error status
        rsp->status = status;
        kernel_msg_send(rsp);
    }
        ...
        switch(param->type)
        {
            ...
            case GATTC_NOTIFY:
            {
                ...
                if (glpc_env->env[conidx]->gls.chars[GLPC_CHAR_MEAS].val_hdl == param->handle)
                {
                    ...
                    struct glpc_meas_ind * ind = KERNEL_MSG_ALLOC(GLPC_MEAS_IND,
                            prf_dst_task_get(&(glpc_env->prf_env), conidx), dest_id,
                            glpc_meas_ind);
                ...
                else if(glpc_env->env[conidx]->gls.chars[GLPC_CHAR_MEAS_CTX].val_hdl == param->handle)
                {
                    ...
                    struct glpc_meas_ctx_ind * ind = KERNEL_MSG_ALLOC(GLPC_MEAS_CTX_IND,
                            prf_dst_task_get(&(glpc_env->prf_env), conidx), dest_id,
                            glpc_meas_ctx_ind);
 ...
 ****************************************************************************************
 */
static int glpc_racp_req_timeout_handler(kernel_msg_id_t const msgid, void const *param,
...
const struct kernel_msg_handler glpc_default_state[] =
{
    {GATTC_SDP_SVC_IND,             (kernel_msg_func_t)gattc_sdp_svc_ind_handler},
    {GLPC_READ_FEATURES_REQ,        (kernel_msg_func_t)glpc_read_features_req_handler},
    {GATTC_READ_IND,                (kernel_msg_func_t)gattc_read_ind_handler},
    {GLPC_REGISTER_REQ,             (kernel_msg_func_t)glpc_register_req_handler},
    {GLPC_RACP_REQ,                 (kernel_msg_func_t)glpc_racp_req_handler},
    {GATTC_EVENT_IND,               (kernel_msg_func_t)gattc_event_ind_handler},
    {GATTC_EVENT_REQ_IND,           (kernel_msg_func_t)gattc_event_ind_handler},
    {GLPC_RACP_REQ_TIMEOUT,         (kernel_msg_func_t)glpc_racp_req_timeout_handler},
...
const struct kernel_state_handler glpc_default_handler = KERNEL_STATE_HANDLER(glpc_default_state);
