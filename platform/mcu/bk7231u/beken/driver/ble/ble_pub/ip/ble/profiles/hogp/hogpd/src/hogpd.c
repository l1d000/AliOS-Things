...
const struct attm_desc hids_att_db[HOGPD_IDX_NB] =
{
    ...
    //[HOGPD_IDX_REPORT_MAP_VAL]                  = {ATT_CHAR_REPORT_MAP, PERM(RD, ENABLE)|PERM(RP,SEC_CON), PERM(RI, ENABLE), HOGPD_REPORT_MAP_MAX_LEN},
...
static uint8_t hogpd_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct hogpd_db_cfg* params)
{
    ...
    hogpd_env = (struct hogpd_env_tag* ) kernel_malloc(sizeof(struct hogpd_env_tag), KERNEL_MEM_ATT_DB);
    ...
    for (svc_idx = 0; ((svc_idx < params->hids_nb) && (status == GAP_ERR_NO_ERROR)); svc_idx++)
    {
        ...
        hids_db[svc_idx] = (struct attm_desc *) kernel_malloc(HOGPD_ATT_MAX * sizeof(struct attm_desc), KERNEL_MEM_NON_RETENTION);
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        kernel_state_set(env->task, HOGPD_IDLE);
    }
    else if(hogpd_env != NULL)
    {
        kernel_free(hogpd_env);
    }
...
uint16_t g_hid_handle = ATT_INVALID_HANDLE;
...
uint8_t hogpd_ntf_send(uint8_t conidx, const struct hogpd_report_info* report)
{
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        ...
        struct gattc_send_evt_cmd *report_ntf = KERNEL_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
                KERNEL_BUILD_ID(TASK_GATTC, conidx), prf_src_task_get(&(hogpd_env->prf_env), conidx),
...
uint8_t hogpd_ntf_cfg_ind_send(uint8_t conidx, uint8_t svc_idx, uint8_t att_idx, uint8_t report_idx, uint16_t ntf_cfg)
{
    ...
    {
        // Allocate the GATT notification message
        struct hogpd_ntf_cfg_ind *ntf_cfg_ind = KERNEL_MSG_ALLOC(HOGPD_NTF_CFG_IND,
