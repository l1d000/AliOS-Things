...
static uint8_t cppc_init(struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  void* params)
{
    ...
    struct cppc_env_tag* cppc_env =
            (struct cppc_env_tag* ) kernel_malloc(sizeof(struct cppc_env_tag), KERNEL_MEM_ATT_DB);
    ...
    for(idx = 0; idx < CPPC_IDX_MAX ; idx++)
    {
        ...
        kernel_state_set(KERNEL_BUILD_ID(env->task, idx), CPPC_FREE);
    }
...
static void cppc_cleanup(struct prf_task_env* env, uint8_t conidx, uint8_t reason)
{
    ...
    if(cppc_env->env[conidx] != NULL)
    {
        ...
        {
            kernel_free(kernel_param2msg(cppc_env->env[conidx]->operation));
        }
        kernel_timer_clear(CPPC_TIMEOUT_TIMER_IND, prf_src_task_get(&cppc_env->prf_env, conidx));
        kernel_free(cppc_env->env[conidx]);
        cppc_env->env[conidx] = NULL;
    }

    /* Put CPP Client in Free state */
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), CPPC_FREE);
}
...
static void cppc_create(struct prf_task_env* env, uint8_t conidx)
{
    ...
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), CPPC_IDLE);
}
...
void cppc_enable_rsp_send(struct cppc_env_tag *cppc_env, uint8_t conidx, uint8_t status)
{
    ...
    struct cppc_enable_rsp * rsp = KERNEL_MSG_ALLOC(
            CPPC_ENABLE_RSP,
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        kernel_state_set(prf_src_task_get(&(cppc_env->prf_env), conidx), CPPC_IDLE);
    }
...
{
    // Send the message
    struct cppc_cmp_evt *evt = KERNEL_MSG_ALLOC(CPPC_CMP_EVT,
                                             dest_id, src_id,
                                             cppc_cmp_evt);
...
void cppc_send_cmp_evt(struct cppc_env_tag *cppc_env, uint8_t conidx, uint8_t operation, uint8_t status)
{
    ...
    if (cppc_env->env[conidx]->operation != NULL)
    {
        kernel_msg_free(kernel_param2msg(cppc_env->env[conidx]->operation));
        cppc_env->env[conidx]->operation = NULL;
    }

    // Go back to the IDLE state if the state is busy
    if (kernel_state_get(prf_src_task_get(&(cppc_env->prf_env), conidx)) == CPPC_BUSY)
    {
        kernel_state_set(prf_src_task_get(&cppc_env->prf_env, conidx), CPPC_IDLE);
    }
...
uint8_t cppc_unpack_meas_ind (uint8_t conidx, struct gattc_event_ind const *param, struct cppc_env_tag *cppc_env)
{
    ...
    struct cppc_value_meas_ind *ind = KERNEL_MSG_ALLOC(
            CPPC_VALUE_IND,
            prf_dst_task_get(&(cppc_env->prf_env), conidx),
            prf_src_task_get(&(cppc_env->prf_env), conidx),
            cppc_value_meas_ind);
    ...
    // Flags
    ind->value.cp_meas.flags = common_read16p(&param->value[0]);
    // Instant power
    ind->value.cp_meas.inst_power = common_read16p(&param->value[2]);
    ...
    if (ind->value.cp_meas.flags & CPP_MEAS_ACCUM_TORQUE_PRESENT)
    {
        ...
        ind->value.cp_meas.accum_torque = common_read16p(&param->value[offset]);
        offset += 2;
    }
    ...
    if (ind->value.cp_meas.flags & CPP_MEAS_WHEEL_REV_DATA_PRESENT)
    {
        ...
        ind->value.cp_meas.cumul_wheel_rev = common_read32p(&param->value[offset]);
        offset += 4;
        ind->value.cp_meas.last_wheel_evt_time = common_read16p(&param->value[offset]);
        offset += 2;
    }
    ...
    if (ind->value.cp_meas.flags & CPP_MEAS_CRANK_REV_DATA_PRESENT)
    {
        ...
        ind->value.cp_meas.cumul_crank_rev = common_read16p(&param->value[offset]);
        offset += 2;
        ind->value.cp_meas.last_crank_evt_time = common_read16p(&param->value[offset]);
        offset += 2;
    }
    ...
    if (ind->value.cp_meas.flags & CPP_MEAS_EXTREME_FORCE_MAGNITUDES_PRESENT)
    {
        ...
        ind->value.cp_meas.max_force_magnitude = common_read16p(&param->value[offset]);
        offset += 2;
        ind->value.cp_meas.min_force_magnitude = common_read16p(&param->value[offset]);
        offset += 2;
    }
    ...
    {
        //Unpack Extreme Force Magnitudes (Maximum Force Magnitude & Minimum Force Magnitude)
        ind->value.cp_meas.max_torque_magnitude = common_read16p(&param->value[offset]);
        offset += 2;
        ind->value.cp_meas.min_torque_magnitude = common_read16p(&param->value[offset]);
        offset += 2;
    }
    ...
    if (ind->value.cp_meas.flags & CPP_MEAS_EXTREME_ANGLES_PRESENT)
    {
        ...
        uint32_t angle = common_read24p(&param->value[offset]);
        offset += 3;
    ...
    if (ind->value.cp_meas.flags & CPP_MEAS_TOP_DEAD_SPOT_ANGLE_PRESENT)
    {
        ...
        ind->value.cp_meas.top_dead_spot_angle = common_read16p(&param->value[offset]);
        offset += 2;
    }
    ...
    if (ind->value.cp_meas.flags & CPP_MEAS_BOTTOM_DEAD_SPOT_ANGLE_PRESENT)
    {
        ...
        ind->value.cp_meas.bot_dead_spot_angle = common_read16p(&param->value[offset]);
        offset += 2;
    }
    ...
    if (ind->value.cp_meas.flags & CPP_MEAS_ACCUM_ENERGY_PRESENT)
    {
        ...
        ind->value.cp_meas.accum_energy = common_read16p(&param->value[offset]);
        offset += 2;
    }
...
{
    // CP Measurement value has been received
    struct cppc_value_ind *ind = KERNEL_MSG_ALLOC_DYN(
            CPPC_VALUE_IND,
            prf_dst_task_get(&cppc_env->prf_env, conidx),
            prf_src_task_get(&cppc_env->prf_env, conidx),
            cppc_value_ind,
    ...
    if (ind->value.cp_vector.flags & CPP_VECTOR_CRANK_REV_DATA_PRESENT)
    {
        ...
        ind->value.cp_vector.cumul_crank_rev = common_read16p(&param->value[offset]);
        offset += 2;
        // Unpack Last Crank Evt time
        ind->value.cp_vector.last_crank_evt_time = common_read16p(&param->value[offset]);
        offset += 2;
    }
    ...
    if (ind->value.cp_vector.flags & CPP_VECTOR_FIRST_CRANK_MEAS_ANGLE_PRESENT)
    {
        ...
        ind->value.cp_vector.first_crank_meas_angle = common_read16p(&param->value[offset]);
        offset += 2;
    }
            ...
            !(ind->value.cp_vector.flags & CPP_VECTOR_INST_TORQUE_MAGNITUDE_ARRAY_PRESENT))
    {
        ...
        if (ind->value.cp_vector.nb)
        {
            for (int i = 0; i < ind->value.cp_vector.nb; i++)
            {
                ...
                ind->value.cp_vector.force_torque_magnitude[i] = common_read16p(&param->value[offset]);
                offset += 2;
            }
        }
    }
...
uint8_t cppc_pack_ctnl_pt_req (struct cppc_ctnl_pt_cfg_req *param, uint8_t *req, uint8_t *status)
{
    ...
    switch (param->ctnl_pt.op_code)
    {
        case (CPP_CTNL_PT_SET_CUMUL_VAL):
        {
            ...
            common_write32p(&req[req_len], param->ctnl_pt.value.cumul_val);
            // Update length
            req_len += 4;
        } break;
        ...
        case (CPP_CTNL_PT_SET_CRANK_LENGTH):
        {
            ...
            common_write16p(&req[req_len], param->ctnl_pt.value.crank_length);
            // Update length
            req_len += 2;
        } break;
        ...
        case (CPP_CTNL_PT_SET_CHAIN_LENGTH):
        {
            ...
            common_write16p(&req[req_len], param->ctnl_pt.value.chain_length);
            // Update length
            req_len += 2;
        } break;
        ...
        case (CPP_CTNL_PT_SET_CHAIN_WEIGHT):
        {
            ...
            common_write16p(&req[req_len], param->ctnl_pt.value.chain_weight);
            // Update length
            req_len += 2;
        } break;
        ...
        {
            // Set the span length
            common_write16p(&req[req_len], param->ctnl_pt.value.span_length);
            // Update length
            req_len += 2;
        } break;
        ...
        case (CPP_CTNL_MASK_CP_MEAS_CH_CONTENT):
        {
            ...
            common_write16p(&req[req_len], param->ctnl_pt.value.mask_content);
            // Update length
            req_len += 2;
        } break;
...
uint8_t cppc_unpack_ctln_pt_ind (uint8_t conidx, struct gattc_event_ind const *param, struct cppc_env_tag *cppc_env)
{
    ...
    struct cppc_ctnl_pt_rsp *ind = KERNEL_MSG_ALLOC(
            CPPC_CTNL_PT_RSP,
            prf_dst_task_get(&cppc_env->prf_env, conidx),
            prf_src_task_get(&cppc_env->prf_env, conidx),
            cppc_ctnl_pt_rsp);
    ...
    if ((ind->rsp.resp_value == CPP_CTNL_PT_RESP_SUCCESS) && (param->length >= 3))
    {
        switch (ind->rsp.req_op_code)
        {
            ...
            case (CPP_CTNL_PT_REQ_CRANK_LENGTH):
            {
                ind->rsp.value.crank_length = common_read16p(&param->value[offset]);
                offset += 2;
            } break;

            case (CPP_CTNL_PT_REQ_CHAIN_LENGTH):
            {
                ind->rsp.value.chain_length = common_read16p(&param->value[offset]);
                offset += 2;
            } break;

            case (CPP_CTNL_PT_REQ_CHAIN_WEIGHT):
            {
                ind->rsp.value.chain_weight = common_read16p(&param->value[offset]);
                offset += 2;
            } break;

            case (CPP_CTNL_PT_REQ_SPAN_LENGTH):
            {
                ind->rsp.value.span_length = common_read16p(&param->value[offset]);
                offset += 2;
            } break;

            case (CPP_CTNL_PT_START_OFFSET_COMP):
            {
                ind->rsp.value.offset_comp = common_read16p(&param->value[offset]);
                offset += 2;
            } break;
