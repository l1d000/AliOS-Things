...
static uint8_t pasps_init(struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl, struct pasps_db_cfg* params)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        struct pasps_env_tag* pasps_env =
                (struct pasps_env_tag* ) kernel_malloc(sizeof(struct pasps_env_tag), KERNEL_MEM_ATT_DB);
        ...
        for(uint8_t idx = 0; idx < BLE_CONNECTION_MAX ; idx++)
        {
            ...
            kernel_state_set(KERNEL_BUILD_ID(env->task, idx), PASPS_FREE);
        }
    }
...
static void pasps_destroy(struct prf_task_env* env)
{
    ...
    for(idx = 0; idx < BLE_CONNECTION_MAX ; idx++)
    {
        ...
        {
            kernel_free(pasps_env->env[idx]);
        }
    }

    // free profile environment variables
    env->env = NULL;
    kernel_free(pasps_env);
}
...
static void pasps_create(struct prf_task_env* env, uint8_t conidx)
{
    ...
    pasps_env->env[conidx] = (struct pasps_cnx_env*)
            kernel_malloc(sizeof(struct pasps_cnx_env), KERNEL_MEM_ATT_DB);
    ...
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), PASPS_IDLE);
}
...
static void pasps_cleanup(struct prf_task_env* env, uint8_t conidx, uint8_t reason)
{
    ...
    {
        kernel_free(pasps_env->env[conidx]);
        pasps_env->env[conidx] = NULL;
    }

    /* Put PASS in disabled state */
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), PASPS_FREE);
}
...
void pasps_send_cmp_evt(kernel_task_id_t src_id, kernel_task_id_t dest_id,
                        uint8_t operation, uint8_t status)
{
    // Come back to the Connected state if the state was busy.
    if (kernel_state_get(src_id) == PASPS_BUSY)
    {
        kernel_state_set(src_id, PASPS_IDLE);
    }

    // Send the message to the application
    struct pasps_cmp_evt *evt = KERNEL_MSG_ALLOC(PASPS_CMP_EVT,
                                             dest_id, src_id,
                                             pasps_cmp_evt);
