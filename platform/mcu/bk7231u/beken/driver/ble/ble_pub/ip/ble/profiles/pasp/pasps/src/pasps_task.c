 ...
 ****************************************************************************************
 */
static int pasps_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if(kernel_state_get(dest_id) == PASPS_IDLE)
    {
    ...
    struct pasps_enable_rsp *cmp_evt = KERNEL_MSG_ALLOC(PASPS_ENABLE_RSP, src_id, dest_id, pasps_enable_rsp);
 ...
 ****************************************************************************************
 */
static int pasps_update_char_val_cmd_handler(kernel_msg_id_t const msgid,
                                             ...
                                             kernel_task_id_t const src_id)
{
    ...
    if (state == PASPS_IDLE)
    {
        ...
        if (status == GAP_ERR_NO_ERROR)
        {
            ...
            if (PASPS_IS_NTF_ENABLED(conidx, pasps_env, flag))
            {
                ...
                kernel_state_set(dest_id, PASPS_BUSY);

                // Allocate the GATT notification message
                struct gattc_send_evt_cmd *ntf = KERNEL_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        ...
        struct pasps_cmp_evt *evt = KERNEL_MSG_ALLOC(PASPS_CMP_EVT, src_id, dest_id, pasps_cmp_evt);
    ...
    if(kernel_state_get(dest_id) == PASPS_IDLE)
    {
        ...
        switch(att_idx)
        {
            ...
            case PASS_IDX_ALERT_STATUS_CFG:
            {
                ...
                common_write16p(value, (pasps_env->env[conidx]->ntf_state & PASPS_FLAG_ALERT_STATUS_CFG)
            ...
            case PASS_IDX_RINGER_SETTING_CFG:
            {
                // Fill data
                value_size = sizeof(uint16_t);
                common_write16p(value, (pasps_env->env[conidx]->ntf_state & PASPS_FLAG_RINGER_SETTING_CFG)
                        ? PRF_CLI_START_NTF : PRF_CLI_STOP_NTFIND);
            } break;
                                       ...
                                       kernel_task_id_t const src_id)
{
    ...
    if (pasps_env->env[conidx] != NULL)
    {
            ...
            (param->handle == (pasps_env->shdl + PASS_IDX_RINGER_SETTING_CFG)))
        {
            ...
            {
                struct pasps_written_char_val_ind *ind = KERNEL_MSG_ALLOC(PASPS_WRITTEN_CHAR_VAL_IND,
...
const struct kernel_msg_handler pasps_default_state[] =
{
...
const struct kernel_state_handler pasps_default_handler = KERNEL_STATE_HANDLER(pasps_default_state);
