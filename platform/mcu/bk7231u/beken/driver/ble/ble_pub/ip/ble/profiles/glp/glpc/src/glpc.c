...
static uint8_t glpc_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  void* params)
{
    ...
    struct glpc_env_tag* glpc_env =
            (struct glpc_env_tag* ) kernel_malloc(sizeof(struct glpc_env_tag), KERNEL_MEM_ATT_DB);
    ...
    for(idx = 0; idx < GLPC_IDX_MAX ; idx++)
    {
        ...
        kernel_state_set(KERNEL_BUILD_ID(env->task, idx), GLPC_FREE);
    }
...
static void glpc_destroy(struct prf_task_env* env)
{
    ...
    for(idx = 0; idx < GLPC_IDX_MAX ; idx++)
    {
        ...
        {
            kernel_free(glpc_env->env[idx]);
        }
    }

    // free profile environment variables
    env->env = NULL;
    kernel_free(glpc_env);
}
...
static void glpc_create(struct prf_task_env* env, uint8_t conidx)
{
    ...
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), GLPC_IDLE);
}
...
static void glpc_cleanup(struct prf_task_env* env, uint8_t conidx, uint8_t reason)
{
    ...
    {
        kernel_free(glpc_env->env[conidx]);
        glpc_env->env[conidx] = NULL;
    }

    /* Put GLP Client in Free state */
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), GLPC_FREE);
}
...
void glpc_enable_rsp_send(struct glpc_env_tag *glpc_env, uint8_t conidx, uint8_t status)
{
    ...
    struct glpc_enable_rsp * rsp = KERNEL_MSG_ALLOC(GLPC_ENABLE_RSP,
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        // Go to IDLE state
        kernel_state_set(prf_src_task_get(&(glpc_env->prf_env), conidx), GLPC_IDLE);
    }
                               ...
                               uint16_t *seq_num)
{
    ...
    // Sequence Number
    *seq_num = common_read16p(packed_meas + cursor);
    cursor += 2;
    ...
    {
        meas_val->time_offset = common_read16p(packed_meas + cursor);
        cursor += 2;
    }
    ...
    {
        meas_val->concentration = common_read16p(packed_meas + cursor);
        cursor += 2;
    ...
    {
        meas_val->status = common_read16p(packed_meas + cursor);
        cursor += 2;
    }
                                   ...
                                   uint16_t* seq_num)
{
    ...
    // Sequence Number
    *seq_num = common_read16p(packed_meas_ctx + cursor);
    cursor += 2;
    ...
    if((meas_ctx_val->flags & GLP_CTX_CRBH_ID_AND_CRBH_PRES) != 0)
    {
        ...
        meas_ctx_val->carbo_val = common_read16p(packed_meas_ctx + cursor);
        cursor += 2;
    }
    ...
    {
        // Exercise Duration
        meas_ctx_val->exercise_dur = common_read16p(packed_meas_ctx + cursor);
        cursor += 2;
    ...
    if((meas_ctx_val->flags & GLP_CTX_MEDIC_ID_AND_MEDIC_PRES) != 0)
    {
        ...
        // Medication Present
        meas_ctx_val->med_val = common_read16p(packed_meas_ctx + cursor);
        cursor += 2;
    }
    ...
    {
        // HbA1c
        meas_ctx_val->hba1c_val = common_read16p(packed_meas_ctx + cursor);
        cursor += 2;
    }
                           ...
                           const struct glp_racp_req* racp_req)
{
            ...
            && (racp_req->filter.operator <= GLP_OP_WITHIN_RANGE_OF))
    {
        ...
        if(racp_req->filter.filter_type == GLP_FILTER_SEQ_NUMBER)
        {
            ...
            {
                // minimum value
                common_write16p(packed_val + cursor,racp_req->filter.val.seq_num.min);
                cursor +=2;
            }
            ...
            {
                // maximum value
                common_write16p(packed_val + cursor,racp_req->filter.val.seq_num.max);
                cursor +=2;
            }
        }
                             ...
                             struct glp_racp_rsp* racp_rsp)
{
    ...
    {
        racp_rsp->operand.num_of_record = common_read16p(packed_val + cursor);
        cursor += 2;
    }
    else
    {
