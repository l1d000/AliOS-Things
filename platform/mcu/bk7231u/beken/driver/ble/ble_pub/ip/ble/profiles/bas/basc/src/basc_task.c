 ...
 ****************************************************************************************
 */
static int basc_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if((state == BASC_IDLE) && (basc_env->env[conidx] == NULL))
    {
        ...
        basc_env->env[conidx] = (struct basc_cnx_env*) kernel_malloc(sizeof(struct basc_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, BASC_BUSY);
            basc_env->env[conidx]->operation = kernel_param2msg(param);
 ...
 ****************************************************************************************
 */
static int basc_read_info_req_handler(kernel_msg_id_t const msgid, struct basc_read_info_req const *param,
                                      kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        struct basc_read_info_rsp * rsp = KERNEL_MSG_ALLOC(BASC_READ_INFO_RSP,
 ...
 ****************************************************************************************
 */
static int basc_batt_level_ntf_cfg_req_handler(kernel_msg_id_t const msgid,
                                               ...
                                               kernel_task_id_t const src_id)
{
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        struct basc_batt_level_ntf_cfg_rsp * rsp = KERNEL_MSG_ALLOC(BASC_BATT_LEVEL_NTF_CFG_RSP,
                                ...
                                kernel_task_id_t const src_id)
{
            ...
            && (basc_env->env[conidx]->operation != NULL))
    {
        switch(basc_env->env[conidx]->operation->id)
        {
            case BASC_ENABLE_REQ:
            {
                ...
                if (param->status == ATT_ERR_NO_ERROR)
                {
                    ...
                    {
                        uint8_t i;
                        for (i = 0 ; (i < common_min(basc_env->env[conidx]->bas_nb, BASC_NB_BAS_INSTANCES_MAX))
            ...
            {
                struct basc_read_info_req* req = (struct basc_read_info_req*) kernel_msg2param(basc_env->env[conidx]->operation);
            ...
            {
                struct basc_batt_level_ntf_cfg_req* req = (struct basc_batt_level_ntf_cfg_req*) kernel_msg2param(basc_env->env[conidx]->operation);
        ...
        kernel_free(basc_env->env[conidx]->operation);
        basc_env->env[conidx]->operation = NULL;
        kernel_state_set(dest_id, BASC_IDLE);
    }
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if(state == BASC_BUSY)
    {
        ...
        switch(req->info)
        {
            ...
            case BASC_NTF_CFG:
            {
                rsp->data.ntf_cfg    = common_read16p(&param->value[0]);
                                         ...
                                         kernel_task_id_t const src_id)
{
    ...
    if(state != BASC_FREE)
    {
        ...
        for (bas_nb = 0; (bas_nb < basc_env->env[conidx]->bas_nb); bas_nb++)
        {
            ...
            {
                struct basc_batt_level_ind * ind = KERNEL_MSG_ALLOC(BASC_BATT_LEVEL_IND,
...
const struct kernel_msg_handler basc_default_state[] =
{
    {BASC_ENABLE_REQ,               (kernel_msg_func_t)basc_enable_req_handler},
    {BASC_READ_INFO_REQ,            (kernel_msg_func_t)basc_read_info_req_handler},
    {BASC_BATT_LEVEL_NTF_CFG_REQ,   (kernel_msg_func_t)basc_batt_level_ntf_cfg_req_handler},
...
const struct kernel_state_handler basc_default_handler = KERNEL_STATE_HANDLER(basc_default_state);
