 ...
 ****************************************************************************************
 */
static int cscpc_enable_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if ((state == CSCPC_IDLE) && (cscpc_env->env[conidx] == NULL))
    {
        ...
        cscpc_env->env[conidx] = (struct cscpc_cnx_env*) kernel_malloc(sizeof(struct cscpc_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if (param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, CSCPC_DISCOVERING);
        }
 ...
 ****************************************************************************************
 */
static int cscpc_read_cmd_handler(kernel_msg_id_t const msgid,
    ...
    if (state == CSCPC_IDLE)
    {
        ...
        else
        {
            ...
            if (handle != ATT_INVALID_SEARCH_HANDLE)
            {
                ...
                kernel_state_set(dest_id, CSCPC_BUSY);
 ...
 ****************************************************************************************
 */
static int cscpc_cfg_ntfind_cmd_handler(kernel_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int cscpc_ctnl_pt_cfg_req_handler(kernel_msg_id_t const msgid,
                                         ...
                                         kernel_task_id_t const src_id)
{
    ...
    if (cscpc_env != NULL)
    {
        ...
        do
        {
            ...
            ASSERT_ERR(kernel_state_get(dest_id) > CSCPC_FREE);
            ...
            if (kernel_state_get(dest_id) != CSCPC_IDLE)
            {
                // Another procedure is pending, keep the command for later
                msg_status = KERNEL_MSG_NO_FREE;
                // Status is GAP_ERR_NO_ERROR, no message will be sent to the application
            ...
            if (cscpc_env->env[conidx]->cscs.descs[CSCPC_DESC_SC_CTNL_PT_CL_CFG].desc_hdl != ATT_INVALID_SEARCH_HANDLE)
            {
                ...
                switch (param->sc_ctnl_pt.op_code)
                {
                    case (CSCP_CTNL_PT_OP_SET_CUMUL_VAL):
                    {
                        ...
                        common_write32p(&req[req_len], param->sc_ctnl_pt.value.cumul_val);
                        // Update length
                        req_len += 4;
                    } break;
 ...
 ****************************************************************************************
 */
static int cscpc_timeout_timer_ind_handler(kernel_msg_id_t const msgid,
                                           void const *param,
                                           kernel_task_id_t const dest_id,
                                           kernel_task_id_t const src_id)
{
                ...
                case GATTC_WRITE_NO_RESPONSE:
                {
                    ...
                    else if (operation == CSCPC_CTNL_PT_CFG_WR_OP_CODE)
                    {
                        if (param->status == GAP_ERR_NO_ERROR)
                        {
                            ...
                            kernel_timer_set(CSCPC_TIMEOUT_TIMER_IND, dest_id, ATT_TRANS_RTX);
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if (kernel_state_get(dest_id) == CSCPC_BUSY)
    {
        uint8_t conidx = KERNEL_IDX_GET(dest_id);
        ...
        struct cscpc_value_ind *ind = KERNEL_MSG_ALLOC(CSCPC_VALUE_IND,
        ...
        switch (((struct cscpc_read_cmd *)cscpc_env->env[conidx]->operation)->read_code)
        {
            ...
            case (CSCPC_RD_CSC_FEAT):
            {
                ind->value.sensor_feat = common_read16p(param->value);
...
//                ind->value.sensor_feat &= CSCP_FEAT_ALL_SUPP;
            ...
            case (CSCPC_RD_WR_SC_CTNL_PT_CFG):
            {
                common_write16p(&ind->value.ntf_cfg, param->value[0]);
            } break;
    ...
    if (cscpc_env != NULL)
    {
        switch (param->type)
        {
            case (GATTC_NOTIFY):
            {
                ...
                {
                    // Cumulative Wheel Revolutions
                    ind->value.csc_meas.cumul_wheel_rev = common_read32p(&param->value[offset]);
                    offset += 4;

                    // Last Wheel Event Time
                    ind->value.csc_meas.last_wheel_evt_time = common_read16p(&param->value[offset]);
                    offset += 2;
                }
                ...
                {
                    // Cumulative Crank Revolutions
                    ind->value.csc_meas.cumul_crank_rev = common_read16p(&param->value[offset]);
                    offset += 2;

                    // Last Crank Event Time
                    ind->value.csc_meas.last_crank_evt_time = common_read16p(&param->value[offset]);
                }
            ...
            case (GATTC_INDICATE):
            {
                ...
                if (cscpc_env->env[conidx]->operation != NULL)
                {
                    if (((struct cscpc_cmd *)cscpc_env->env[conidx]->operation)->operation == CSCPC_CTNL_PT_CFG_IND_OP_CODE)
                    {
                        ...
                        kernel_timer_clear(CSCPC_TIMEOUT_TIMER_IND, dest_id);

                        // CSC Measurement value has been received
                        struct cscpc_ctnl_pt_rsp *ind = KERNEL_MSG_ALLOC(CSCPC_CTNL_PT_RSP,
                                                                   prf_dst_task_get(&(cscpc_env->prf_env), conidx),
                                                                   prf_src_task_get(&(cscpc_env->prf_env), conidx),
                                                                   cscpc_ctnl_pt_rsp);
...
const struct kernel_msg_handler cscpc_default_state[] =
{
    ...
    {CSCPC_CTNL_PT_CFG_REQ,         (kernel_msg_func_t)cscpc_ctnl_pt_cfg_req_handler},
    {CSCPC_TIMEOUT_TIMER_IND,       (kernel_msg_func_t)cscpc_timeout_timer_ind_handler},
    {GATTC_EVENT_IND,               (kernel_msg_func_t)gattc_event_ind_handler},
    {GATTC_EVENT_REQ_IND,           (kernel_msg_func_t)gattc_event_ind_handler},
    {GATTC_CMP_EVT,                 (kernel_msg_func_t)gattc_cmp_evt_handler},
};

// Specifies the message handlers that are common to all states.
const struct kernel_state_handler cscpc_default_handler = KERNEL_STATE_HANDLER(cscpc_default_state);
