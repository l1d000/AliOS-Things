...
static uint8_t cscpc_init(struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  void* params)
{
    ...
    struct cscpc_env_tag* cscpc_env =
            (struct cscpc_env_tag* ) kernel_malloc(sizeof(struct cscpc_env_tag), KERNEL_MEM_ATT_DB);
    ...
    for(idx = 0; idx < CSCPC_IDX_MAX ; idx++)
    {
        ...
        kernel_state_set(KERNEL_BUILD_ID(env->task, idx), CSCPC_FREE);
    }
...
static void cscpc_cleanup(struct prf_task_env* env, uint8_t conidx, uint8_t reason)
{
    ...
    if(cscpc_env->env[conidx] != NULL)
    {
        ...
        {
            kernel_free(kernel_param2msg(cscpc_env->env[conidx]->operation));
        }
        kernel_timer_clear(CSCPC_TIMEOUT_TIMER_IND, prf_src_task_get(&cscpc_env->prf_env, conidx));

        kernel_free(cscpc_env->env[conidx]);
        cscpc_env->env[conidx] = NULL;
    }

    /* Put CSCP Client in Free state */
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), CSCPC_FREE);
}
...
static void cscpc_create(struct prf_task_env* env, uint8_t conidx)
{
    ...
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), CSCPC_IDLE);
}
...
void cscpc_enable_rsp_send(struct cscpc_env_tag *cscpc_env, uint8_t conidx, uint8_t status)
{
    ...
    struct cscpc_enable_rsp * rsp = KERNEL_MSG_ALLOC(
            CSCPC_ENABLE_RSP,
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        kernel_state_set(prf_src_task_get(&(cscpc_env->prf_env), conidx), CSCPC_IDLE);
    }
...
{
    // Send the message
    struct cscpc_cmp_evt *evt = KERNEL_MSG_ALLOC(CSCPC_CMP_EVT,
                                             dest_id, src_id,
                                             cscpc_cmp_evt);
...
void cscpc_send_cmp_evt(struct cscpc_env_tag *cscpc_env, uint8_t conidx, uint8_t operation, uint8_t status)
{
    ...
    if (cscpc_env->env[conidx]->operation != NULL)
    {
        kernel_msg_free(kernel_param2msg(cscpc_env->env[conidx]->operation));
        cscpc_env->env[conidx]->operation = NULL;
    }

    // Go back to the CONNECTED state if the state is busy
    if (kernel_state_get(prf_src_task_get(&(cscpc_env->prf_env), conidx)) == CSCPC_BUSY)
    {
        kernel_state_set(prf_src_task_get(&(cscpc_env->prf_env), conidx), CSCPC_IDLE);
    }
