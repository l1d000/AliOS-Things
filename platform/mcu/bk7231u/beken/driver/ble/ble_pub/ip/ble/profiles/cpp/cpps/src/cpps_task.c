...
static int cpps_enable_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if(kernel_state_get(dest_id) == CPPS_IDLE)
    {
    ...
    struct cpps_enable_rsp *cmp_evt = KERNEL_MSG_ALLOC(CPPS_ENABLE_RSP, src_id, dest_id, cpps_enable_rsp);
 ...
 ****************************************************************************************
 */
static int cpps_get_adv_data_req_handler(kernel_msg_id_t const msgid,
        struct cpps_get_adv_data_req *param,
        kernel_task_id_t const dest_id,
        kernel_task_id_t const src_id)
{
    // Message status
    uint8_t msg_status = KERNEL_MSG_CONSUMED;
    ...
    if (kernel_state_get(dest_id) == CPPS_IDLE)
    {
        ...
        struct cpps_get_adv_data_rsp *rsp = KERNEL_MSG_ALLOC_DYN(CPPS_GET_ADV_DATA_RSP,
                src_id, dest_id,
        ...
        if (CPPS_IS_FEATURE_SUPPORTED(cpps_env->prfl_cfg, CPPS_MEAS_BCST_MASK))
        {
            ...
            if (cpps_env->broadcast_enabled)
            {
                ...
                // Pack UUID of CPS
                common_write16p(&rsp->adv_data[2], ATT_SVC_CYCLING_POWER);
 ...
 ****************************************************************************************
 */
static int cpps_ntf_cp_meas_req_handler(kernel_msg_id_t const msgid,
                                          struct cpps_ntf_cp_meas_req *param,
                                          kernel_task_id_t const dest_id,
                                          kernel_task_id_t const src_id)
{
    // Message status
    uint8_t msg_status = KERNEL_MSG_NO_FREE;
    ...
    if (kernel_state_get(dest_id) == CPPS_IDLE)
    {
        ...
        cpps_env->op_data = (struct cpps_op*) kernel_malloc(sizeof(struct cpps_op), KERNEL_MEM_KERNEL_MSG);
        cpps_env->op_data->cmd         = kernel_param2msg(param);
        cpps_env->op_data->cursor      = 0;
        cpps_env->op_data->ntf_pending = NULL;

        // Go to busy state
        kernel_state_set(dest_id, CPPS_BUSY);
        ...
        if (CPPS_IS_PRESENT(param->parameters.flags, CPP_MEAS_WHEEL_REV_DATA_PRESENT))
        {
            if (CPPS_IS_FEATURE_SUPPORTED(cpps_env->features, CPP_FEAT_WHEEL_REV_DATA_SUPP))
            {
                ...
                {
                    cpps_env->cumul_wheel_rev =  (common_abs(param->parameters.cumul_wheel_rev) > cpps_env->cumul_wheel_rev) ?
 ...
 ****************************************************************************************
 */
static int cpps_ntf_cp_vector_req_handler(kernel_msg_id_t const msgid,
                                       ...
                                       kernel_task_id_t const src_id)
{
    ...
    if (cpps_env != NULL)
    {
        ...
        {
            //Send write response
            struct gattc_write_cfm *cfm = KERNEL_MSG_ALLOC(
 ...
 ****************************************************************************************
 */
static int gattc_read_req_ind_handler(kernel_msg_id_t const msgid,
    ...
    if(kernel_state_get(dest_id) == CPPS_IDLE)
    {
        ...
        switch(att_idx)
        {
            case CPS_IDX_CP_MEAS_NTF_CFG:
            {
                ...
                common_write16p(value, (cpps_env->env[conidx].prfl_ntf_ind_cfg & CPP_PRF_CFG_FLAG_CP_MEAS_NTF)
            ...
            case CPS_IDX_CP_MEAS_BCST_CFG:
            {
                ...
                if (CPPS_IS_FEATURE_SUPPORTED(cpps_env->prfl_cfg, CPPS_MEAS_BCST_MASK))
                {
                    ...
                    common_write16p(value, (cpps_env->env[conidx].prfl_ntf_ind_cfg & CPP_PRF_CFG_FLAG_SP_MEAS_NTF)
            ...
            case CPS_IDX_VECTOR_NTF_CFG:
            {
                // Fill data
                value_size = sizeof(uint16_t);
                common_write16p(value, (cpps_env->env[conidx].prfl_ntf_ind_cfg & CPP_PRF_CFG_FLAG_VECTOR_NTF)
                        ? PRF_CLI_START_NTF : PRF_CLI_STOP_NTFIND);
            } break;
            ...
            case CPS_IDX_CTNL_PT_IND_CFG:
            {
                // Fill data
                value_size = sizeof(uint16_t);
                common_write16p(value, (cpps_env->env[conidx].prfl_ntf_ind_cfg & CPP_PRF_CFG_FLAG_CTNL_PT_IND)
            ...
            case CPS_IDX_CP_FEAT_VAL:
            {
                ...
                common_write32p(value, cpps_env->features);
            } break;
 ...
 ****************************************************************************************
 */
static int cpps_vector_cfg_cfm_handler(kernel_msg_id_t const msgid,
                                          ...
                                          kernel_task_id_t const src_id)
{
    ...
    if (kernel_state_get(dest_id) == CPPS_IDLE)
    {
        ...
        //Send write response
        struct gattc_write_cfm *cfm = KERNEL_MSG_ALLOC(
                GATTC_WRITE_CFM, KERNEL_BUILD_ID(TASK_GATTC, param->conidx), dest_id, gattc_write_cfm);
 ...
 ****************************************************************************************
 */
static int cpps_ctnl_pt_cfm_handler(kernel_msg_id_t const msgid,
                                          ...
                                          kernel_task_id_t const src_id)
{
    ...
    if (kernel_state_get(dest_id) == CPPS_BUSY)
    {
        do
        {
            ...
            struct gattc_send_evt_cmd *ctl_pt_rsp = KERNEL_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
                    KERNEL_BUILD_ID(TASK_GATTC, param->conidx), dest_id,
...
const struct kernel_msg_handler cpps_default_state[] =
{
    ...
    {CPPS_NTF_CP_MEAS_REQ,          (kernel_msg_func_t) cpps_ntf_cp_meas_req_handler},
    {CPPS_NTF_CP_VECTOR_REQ,        (kernel_msg_func_t) cpps_ntf_cp_vector_req_handler},
    {CPPS_CTNL_PT_CFM,              (kernel_msg_func_t) cpps_ctnl_pt_cfm_handler},
    {CPPS_VECTOR_CFG_CFM,           (kernel_msg_func_t) cpps_vector_cfg_cfm_handler},
    {GATTC_CMP_EVT,                 (kernel_msg_func_t) gattc_cmp_evt_handler},
};

/// Specifies the message handlers that are common to all states.
const struct kernel_state_handler cpps_default_handler = KERNEL_STATE_HANDLER(cpps_default_state);
