...
static uint8_t  lanc_init(struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  void* params)
{
    ...
    struct lanc_env_tag* lanc_env =
            (struct lanc_env_tag* ) kernel_malloc(sizeof(struct lanc_env_tag), KERNEL_MEM_ATT_DB);
    ...
    for(idx = 0; idx < LANC_IDX_MAX ; idx++)
    {
        ...
        kernel_state_set(KERNEL_BUILD_ID(env->task, idx), LANC_FREE);
    }
...
static void lanc_cleanup(struct prf_task_env* env, uint8_t conidx, uint8_t reason)
{
    ...
    if(lanc_env->env[conidx] != NULL)
    {
        ...
        {
            kernel_free(kernel_param2msg(lanc_env->env[conidx]->operation));
        }
        kernel_timer_clear(LANC_TIMEOUT_TIMER_IND, prf_src_task_get(&lanc_env->prf_env, conidx));

        kernel_free(lanc_env->env[conidx]);
        lanc_env->env[conidx] = NULL;
    }

    /* Put LAN Client in Free state */
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), LANC_FREE);
}
...
static void lanc_create(struct prf_task_env* env, uint8_t conidx)
{
    ...
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), LANC_IDLE);
}
...
void lanc_enable_rsp_send(struct lanc_env_tag *lanc_env, uint8_t conidx, uint8_t status)
{
    ...
    struct lanc_enable_rsp * rsp = KERNEL_MSG_ALLOC(
            LANC_ENABLE_RSP,
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        kernel_state_set(prf_src_task_get(&(lanc_env->prf_env), conidx), LANC_IDLE);
    }
...
{
    // Send the message
    struct lanc_cmp_evt *evt = KERNEL_MSG_ALLOC(LANC_CMP_EVT,
                                             dest_id, src_id,
                                             lanc_cmp_evt);
...
void lanc_send_cmp_evt(struct lanc_env_tag *lanc_env, uint8_t conidx, uint8_t operation, uint8_t status)
{
    ...
    if (lanc_env->env[conidx]->operation != NULL)
    {
        kernel_msg_free(kernel_param2msg(lanc_env->env[conidx]->operation));
        lanc_env->env[conidx]->operation = NULL;
    }

    // Go back to the CONNECTED state if the state is busy
    if (kernel_state_get(prf_src_task_get(&(lanc_env->prf_env), conidx)) == LANC_BUSY)
    {
        kernel_state_set(prf_src_task_get(&(lanc_env->prf_env), conidx), LANC_IDLE);
    }
...
uint8_t lanc_unpack_loc_speed_ind (uint8_t conidx, struct gattc_event_ind const *param, struct lanc_env_tag *lanc_env)
{
    ...
    struct lanc_value_ind *ind = KERNEL_MSG_ALLOC(LANC_VALUE_IND,
                                                prf_dst_task_get(&(lanc_env->prf_env), conidx),
                                                prf_src_task_get(&(lanc_env->prf_env), conidx),
                                                lanc_value_ind);
    ...
    {
        // Flags
        ind->value.loc_speed.flags = common_read16p(&param->value[0]);

        if (ind->value.loc_speed.flags & LANP_LSPEED_INST_SPEED_PRESENT)
        {
            //Unpack instantaneous speed
            ind->value.loc_speed.inst_speed = common_read16p(&param->value[offset]);
            offset += 2;
        }
        ...
        if (ind->value.loc_speed.flags & LANP_LSPEED_TOTAL_DISTANCE_PRESENT)
        {
            ...
            ind->value.loc_speed.total_dist = common_read24p(&param->value[offset]);
            offset += 3;
        }
        ...
        {
            //Unpack Location
            ind->value.loc_speed.latitude = common_read32p(&param->value[offset]);
            offset += 4;
            ind->value.loc_speed.longitude = common_read32p(&param->value[offset]);
            offset += 4;
        }
        ...
        if (ind->value.loc_speed.flags & LANP_LSPEED_ELEVATION_PRESENT)
        {
            ...
            ind->value.loc_speed.elevation = common_read24p(&param->value[offset]);
            offset += 3;
        }
        ...
        {
            // Unpack heading
            ind->value.loc_speed.heading = common_read16p(&param->value[offset]);
            offset += 2;
        }
...
uint8_t lanc_unpack_navigation_ind (uint8_t conidx, struct gattc_event_ind const *param, struct lanc_env_tag *lanc_env)
{
    ...
    {
        // Flags
        ind->value.navigation.flags = common_read16p(&param->value[0]);
        // Bearing
        ind->value.navigation.bearing = common_read16p(&param->value[2]);
        // Heading
        ind->value.navigation.heading = common_read16p(&param->value[4]);

        if (ind->value.navigation.flags & LANP_NAVI_REMAINING_DIS_PRESENT)
        {
            //Unpack remaining distance (24 bits)
            ind->value.navigation.remaining_distance = common_read24p(&param->value[offset]);
            offset += 3;
        }
        ...
        if (ind->value.navigation.flags & LANP_NAVI_REMAINING_VER_DIS_PRESENT)
        {
            ...
            ind->value.navigation.remaining_ver_distance = common_read24p(&param->value[offset]);
            offset += 3;
        }
...
uint8_t lanc_unpack_pos_q_ind (struct gattc_read_ind const *param, struct lanc_value_ind *ind)
{
    ...
    // Flags
    ind->value.pos_q.flags = common_read16p(&param->value[0]);
    ...
    if (ind->value.pos_q.flags & LANP_POSQ_TIME_TO_FIRST_FIX_PRESENT)
    {
        ...
        ind->value.pos_q.time_first_fix = common_read16p(&param->value[offset]);
        offset += 2;
    }
    ...
    {
        //Unpack ehpe
        ind->value.pos_q.ehpe = common_read32p(&param->value[offset]);
        offset += 4;
    }
    ...
    {
        //Unpack evpe
        ind->value.pos_q.evpe = common_read32p(&param->value[offset]);
        offset += 4;
    }
...
uint8_t lanc_pack_ln_ctnl_pt_req (struct lanc_ln_ctnl_pt_cfg_req *param, uint8_t *req, uint8_t *status)
{
    ...
    switch (param->ln_ctnl_pt.op_code)
    {
        case (LANP_LN_CTNL_PT_SET_CUMUL_VALUE):
        {
            ...
            common_write24p(&req[req_len], param->ln_ctnl_pt.value.cumul_val);
            // Update length
            req_len += 3;
        } break;
        ...
        {
            // Set mask content
            common_write16p(&req[req_len], param->ln_ctnl_pt.value.mask_content);
            // Update length
            req_len += 2;
        } break;
        ...
        {
            // Set route number
            common_write16p(&req[req_len], param->ln_ctnl_pt.value.route_number);
            // Update length
            req_len += 2;
        } break;
        ...
        case (LANP_LN_CTNL_PT_SET_ELEVATION):
        {
            ...
            common_write24p(&req[req_len], param->ln_ctnl_pt.value.elevation);
            // Update length
            req_len += 3;
        } break;
...
uint8_t lanc_unpack_ln_ctln_pt_ind (struct gattc_event_ind const *param, kernel_task_id_t src, kernel_task_id_t dest)
{
    // Offset
    uint8_t offset = LANP_LAN_LN_CNTL_PT_RSP_MIN_LEN;

    // Control Point value has been received
    struct lanc_ln_ctnl_pt_rsp *ind = KERNEL_MSG_ALLOC_DYN(LANC_LN_CTNL_PT_RSP,
                                            dest,
                                            src,
                                            lanc_ln_ctnl_pt_rsp,
                                            param->length);
    ...
    if ((ind->rsp.resp_value == LANP_LN_CTNL_PT_RESP_SUCCESS) && (param->length >= 3))
    {
        switch (ind->rsp.req_op_code)
        {
            case (LANP_LN_CTNL_PT_REQ_NUMBER_OF_ROUTES):
            {
                ind->rsp.value.number_of_routes = common_read16p(&param->value[offset]);
                offset += 2;
