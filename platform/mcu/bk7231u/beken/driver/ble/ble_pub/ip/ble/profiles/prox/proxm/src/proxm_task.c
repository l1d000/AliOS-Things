 ...
 ****************************************************************************************
 */
static int proxm_enable_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if((state == PROXM_IDLE) && (proxm_env->env[conidx] == NULL))
    {
        ...
        proxm_env->env[conidx] = (struct proxm_cnx_env*) kernel_malloc(sizeof(struct proxm_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, PROXM_DISCOVERING);
        }
        else
        {
 ...
 ****************************************************************************************
 */
static int proxm_rd_req_handler(kernel_msg_id_t const msgid,
    ...
    if(state == PROXM_IDLE)
    {
        ...
        else
        {
            ...
            if(status == GAP_ERR_NO_ERROR)
            {
                ...
                kernel_state_set(dest_id, PROXM_BUSY);
            }
        }
        ...
        if(status != GAP_ERR_NO_ERROR)
        {
            struct proxm_rd_rsp * rsp = KERNEL_MSG_ALLOC(PROXM_RD_RSP,
 ...
 ****************************************************************************************
 */
static int proxm_wr_alert_lvl_req_handler(kernel_msg_id_t const msgid,
                                          ...
                                          kernel_task_id_t const src_id)
{
    ...
    if(state == PROXM_IDLE)
    {
        ...
        if(status != GAP_ERR_NO_ERROR)
        {
            struct proxm_wr_alert_lvl_rsp * rsp = KERNEL_MSG_ALLOC(PROXM_WR_ALERT_LVL_RSP,
    ...
    else if (state == PROXM_BUSY)
    {
        switch(param->operation)
        {
            ...
            case GATTC_WRITE_NO_RESPONSE:
            {
                ...
                kernel_state_set(dest_id, PROXM_IDLE);
            }
            break;
...
static int gattc_read_ind_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    if(kernel_state_get(dest_id) == PROXM_BUSY)
    {
        struct proxm_env_tag *proxm_env = PRF_ENV_GET(PROXM, proxm);
        uint8_t conidx = KERNEL_IDX_GET(dest_id);
...
const struct kernel_msg_handler proxm_default_state[] =
{
    {GATTC_SDP_SVC_IND,           (kernel_msg_func_t)gattc_sdp_svc_ind_handler},
    {PROXM_ENABLE_REQ,            (kernel_msg_func_t)proxm_enable_req_handler},
    {PROXM_WR_ALERT_LVL_REQ,      (kernel_msg_func_t)proxm_wr_alert_lvl_req_handler},
    {PROXM_RD_REQ,                (kernel_msg_func_t)proxm_rd_req_handler},
    {GATTC_READ_IND,              (kernel_msg_func_t)gattc_read_ind_handler},
    {GATTC_CMP_EVT,               (kernel_msg_func_t)gattc_cmp_evt_handler},
};

// Specifies the message handlers that are common to all states.
const struct kernel_state_handler proxm_default_handler = KERNEL_STATE_HANDLER(proxm_default_state);
