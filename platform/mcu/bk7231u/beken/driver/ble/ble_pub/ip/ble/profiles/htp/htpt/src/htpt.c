                            ...
                            uint8_t sec_lvl, struct htpt_db_cfg* params)
{
    ...
    if( status == ATT_ERR_NO_ERROR )
    {
        ...
        struct htpt_env_tag* htpt_env =
                (struct htpt_env_tag* ) kernel_malloc(sizeof(struct htpt_env_tag), KERNEL_MEM_ATT_DB);
        ...
        kernel_state_set(env->task, HTPT_IDLE);
    }
    return (status);
}
...
static void htpt_destroy(struct prf_task_env* env)
{
    ...
    {
        kernel_free(htpt_env->operation);
    }


    env->env = NULL;
    kernel_free(htpt_env);
}
...
uint8_t htpt_pack_temp_value(uint8_t *packed_temp, struct htp_temp_meas temp_meas)
{
    ...
    *(packed_temp + cursor) = temp_meas.flags;
    ...
    common_write32p(packed_temp + cursor, temp_meas.temp);
    cursor += 4;
...
void htpt_exe_operation(void)
{
    ...
    while(htpt_env->operation->cursor < BLE_CONNECTION_MAX)
    {
        ...
        {
            // trigger the event
            struct gattc_send_evt_cmd * evt = KERNEL_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
                    KERNEL_BUILD_ID(TASK_GATTC , htpt_env->operation->cursor), prf_src_task_get(&htpt_env->prf_env, 0),
    ...
    if(finished)
    {
        ...
        if(htpt_env->operation->dest_id != prf_src_task_get(&htpt_env->prf_env, 0))
        {
            ...
            struct htpt_meas_intv_upd_rsp * rsp =
                    KERNEL_MSG_ALLOC(((htpt_env->operation->op == HTPT_CFG_MEAS_INTV_IND) ? HTPT_MEAS_INTV_UPD_RSP : HTPT_TEMP_SEND_RSP),
        ...
        kernel_state_set(prf_src_task_get(&(htpt_env->prf_env), 0), HTPT_IDLE);
    }
}
...
uint8_t htpt_update_ntf_ind_cfg(uint8_t conidx, uint8_t cfg, uint16_t valid_val, uint16_t value)
{
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        ...
        struct htpt_cfg_indntf_ind * ind = KERNEL_MSG_ALLOC(HTPT_CFG_INDNTF_IND,
