 ...
 ****************************************************************************************
 */
static int paspc_enable_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if ((state == PASPC_IDLE) && (paspc_env->env[conidx] == NULL))
    {
        ...
        paspc_env->env[conidx] = (struct paspc_cnx_env*) kernel_malloc(sizeof(struct paspc_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if (param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, PASPC_DISCOVERING);
        ...
        else
        {
            ...
            // Go to BUSY state
            kernel_state_set(dest_id, PASPC_BUSY);
        }
    }
 ...
 ****************************************************************************************
 */
static int paspc_read_cmd_handler(kernel_msg_id_t const msgid,
                                  ...
                                  kernel_task_id_t const src_id)
{
    ...
    if (paspc_idx_env != NULL)
    {
        ...
        do
        {
            ...
            if (kernel_state_get(dest_id) == PASPC_BUSY)
            {
 ...
 ****************************************************************************************
 */
static int paspc_write_cmd_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if (paspc_idx_env != NULL)
    {
        ...
        {
            // Check the current state
            if (kernel_state_get(dest_id) != PASPC_IDLE)
            {
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if (paspc_env != NULL)
    {
        ASSERT_ERR(kernel_state_get(dest_id) == PASPC_BUSY);

        // Prepare the indication message for the HL
        struct paspc_value_ind *ind = KERNEL_MSG_ALLOC(PASPC_VALUE_IND,
        ...
        switch (paspc_env->env[conidx]->last_char_code)
        {
            ...
            case (PASPC_RD_WR_ALERT_STATUS_CFG):
            {
                ind->value.alert_status_ntf_cfg = common_read16p(&param->value[0]);
            } break;

            // Read Ringer Setting Characteristic Client Char. Cfg. Descriptor Value
            case (PASPC_RD_WR_RINGER_SETTING_CFG):
            {
                ind->value.ringer_setting_ntf_cfg = common_read16p(&param->value[0]);
            } break;
...
const struct kernel_msg_handler paspc_default_state[] =
{
...
const struct kernel_state_handler paspc_default_handler = KERNEL_STATE_HANDLER(paspc_default_state);
