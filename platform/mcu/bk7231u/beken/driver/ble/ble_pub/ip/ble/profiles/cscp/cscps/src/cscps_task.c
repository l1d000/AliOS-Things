 ...
 ****************************************************************************************
 */
static int cscps_enable_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if(kernel_state_get(dest_id) == CSCPS_IDLE)
    {
    ...
    struct cscps_enable_rsp *cmp_evt = KERNEL_MSG_ALLOC(CSCPS_ENABLE_RSP, src_id, dest_id, cscps_enable_rsp);
                                      ...
                                      kernel_task_id_t const src_id)
{
    if(kernel_state_get(dest_id) == CSCPS_IDLE)
    {
        ...
        switch(att_idx)
        {
            ...
            {
                // Fill data
                cfm = KERNEL_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(uint16_t));
                cfm->length = sizeof(uint16_t);
                common_write16p(cfm->value, (cscps_env->prfl_ntf_ind_cfg[conidx] & CSCP_PRF_CFG_FLAG_CSC_MEAS_NTF)
            ...
            case CSCS_IDX_CSC_FEAT_VAL:
            {
                ...
                cfm = KERNEL_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(uint16_t));
                cfm->length = sizeof(uint16_t);
                common_write16p(cfm->value, cscps_env->features);
            } break;

            case CSCS_IDX_SENSOR_LOC_VAL:
            {
                // Fill data
                cfm = KERNEL_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(uint8_t));
            ...
            case CSCS_IDX_SC_CTNL_PT_NTF_CFG:
            {
                ...
                cfm = KERNEL_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(uint16_t));
                cfm->length = sizeof(uint16_t);
                common_write16p(cfm->value, (cscps_env->prfl_ntf_ind_cfg[conidx] & CSCP_PRF_CFG_FLAG_SC_CTNL_PT_IND)
                        ? PRF_CLI_START_IND : PRF_CLI_STOP_NTFIND);
            } break;

            default:
            {
                cfm = KERNEL_MSG_ALLOC(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm);
                cfm->length = 0;
 ...
 ****************************************************************************************
 */
static int cscps_ntf_csc_meas_req_handler(kernel_msg_id_t const msgid,
                                          ...
                                          kernel_task_id_t const src_id)
{
    ...
    if (kernel_state_get(dest_id) == CSCPS_IDLE)
    {
        ...
        cscps_env->ntf = (struct cscps_ntf*) kernel_malloc(sizeof(struct cscps_ntf), KERNEL_MEM_KERNEL_MSG);
        ...
        if (CSCPS_IS_PRESENT(param->flags, CSCP_MEAS_WHEEL_REV_DATA_PRESENT))
        {
            ...
            if (param->wheel_rev < 0)
            {
                ...
                if (common_abs(param->wheel_rev) > cscps_env->tot_wheel_rev)
                {
            ...
            // Cumulative Wheel Resolutions
            common_write32p(&cscps_env->ntf->value[cscps_env->ntf->length], cscps_env->tot_wheel_rev);
            cscps_env->ntf->length += 4;

            // Last Wheel Event Time
            common_write16p(&cscps_env->ntf->value[cscps_env->ntf->length], param->last_wheel_evt_time);
        ...
        {
            // Cumulative Crank Revolutions
            common_write32p(&cscps_env->ntf->value[cscps_env->ntf->length], param->cumul_crank_rev);
            cscps_env->ntf->length += 2;

            // Last Crank Event Time
            common_write16p(&cscps_env->ntf->value[cscps_env->ntf->length], param->last_crank_evt_time);
            cscps_env->ntf->length += 2;
        }
        ...
        // Go to busy state
        kernel_state_set(dest_id, CSCPS_BUSY);
 ...
 ****************************************************************************************
 */
static int cscps_sc_ctnl_pt_cfm_handler(kernel_msg_id_t const msgid,
                                          ...
                                          kernel_task_id_t const src_id)
{
    ...
    if (kernel_state_get(dest_id) == CSCPS_BUSY)
    {
        do
                                       ...
                                       kernel_task_id_t const src_id)
{
    ...
    if (cscps_env != NULL)
    {
        ...
        if (param->handle == (CSCPS_HANDLE(CSCS_IDX_CSC_MEAS_NTF_CFG)))
        {
            ...
            // Get the value
            common_write16p(&ntf_cfg, param->value[0]);
            ...
            if (ntf_cfg <= PRF_CLI_START_NTF)
            {
                ...
                struct cscps_cfg_ntfind_ind *ind = KERNEL_MSG_ALLOC(CSCPS_CFG_NTFIND_IND,
                        prf_dst_task_get(&cscps_env->prf_env, conidx),
                        prf_src_task_get(&cscps_env->prf_env, conidx),
            ...
            if (CSCPS_IS_FEATURE_SUPPORTED(cscps_env->prfl_cfg, CSCPS_SC_CTNL_PT_MASK))
            {
                ...
                else if (param->handle == (CSCPS_HANDLE(CSCS_IDX_SC_CTNL_PT_VAL)))
                {
                    ...
                    do
                    {
                        ...
                        struct cscps_sc_ctnl_pt_req_ind *req_ind = KERNEL_MSG_ALLOC(CSCPS_SC_CTNL_PT_REQ_IND,
                                prf_dst_task_get(&cscps_env->prf_env, conidx),
                                prf_src_task_get(&cscps_env->prf_env, conidx),
                        ...
                        switch(param->value[0])
                        {
                            ...
                            case (CSCP_CTNL_PT_OP_SET_CUMUL_VAL):
                            {
                                ...
                                if (CSCPS_IS_FEATURE_SUPPORTED(cscps_env->features, CSCP_FEAT_WHEEL_REV_DATA_SUPP))
                                {
                                    ...
                                    // Cumulative value
                                    req_ind->value.cumul_value = common_read32p(&param->value[1]);
                                }
                            } break;
...
const struct kernel_msg_handler cscps_default_state[] =
{
    ...
    {CSCPS_NTF_CSC_MEAS_REQ,        (kernel_msg_func_t) cscps_ntf_csc_meas_req_handler},
    {CSCPS_SC_CTNL_PT_CFM,          (kernel_msg_func_t) cscps_sc_ctnl_pt_cfm_handler},
...
const struct kernel_state_handler cscps_default_handler = KERNEL_STATE_HANDLER(cscps_default_state);
