...
static uint8_t cscps_init(struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl, struct cscps_db_cfg* param)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        struct cscps_env_tag* cscps_env =
                (struct cscps_env_tag* ) kernel_malloc(sizeof(struct cscps_env_tag), KERNEL_MEM_ATT_DB);
        ...
        kernel_state_set(env->task, CSCPS_IDLE);
    }
...
static void cscps_destroy(struct prf_task_env* env)
{
    ...
    {
        kernel_free(cscps_env->ntf);
    }

    // free profile environment variables
    env->env = NULL;
    kernel_free(cscps_env);
}
...
void cscps_send_cmp_evt(uint8_t conidx, uint8_t src_id, uint8_t dest_id, uint8_t operation, uint8_t status)
{
    ...
    if (kernel_state_get(src_id) == CSCPS_BUSY)
    {
        kernel_state_set(src_id, CSCPS_IDLE);
    }
    ...
    // Send the message
    struct cscps_cmp_evt *evt = KERNEL_MSG_ALLOC(CSCPS_CMP_EVT,
                                             dest_id, src_id,
                                             cscps_cmp_evt);
...
void cscps_exe_operation(void)
{
    ...
    while(cscps_env->ntf->cursor < BLE_CONNECTION_MAX)
    {
        ...
        if(CSCPS_IS_NTFIND_ENABLED(cscps_env->ntf->cursor, CSCP_PRF_CFG_FLAG_CSC_MEAS_NTF))
        {
            ...
            struct gattc_send_evt_cmd *meas_val = KERNEL_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
                    KERNEL_BUILD_ID(TASK_GATTC, cscps_env->ntf->cursor),
    ...
    if(finished)
    {
        ...
        struct cscps_ntf_csc_meas_rsp *rsp = KERNEL_MSG_ALLOC(CSCPS_NTF_CSC_MEAS_RSP,
        ...
        kernel_state_set(prf_src_task_get(&(cscps_env->prf_env), 0), CSCPS_IDLE);
    }
}
