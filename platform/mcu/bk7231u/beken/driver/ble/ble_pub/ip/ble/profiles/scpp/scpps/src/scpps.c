...
static uint8_t scpps_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct scpps_db_cfg* params)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        scpps_env = (struct scpps_env_tag* ) kernel_malloc(sizeof(struct scpps_env_tag), KERNEL_MEM_ATT_DB);
        ...
        kernel_state_set(env->task, SCPPS_IDLE);
    }
    else if(scpps_env != NULL)
    {
        kernel_free(scpps_env);
    }
...
{
    struct scpps_scan_refresh_send_rsp * rsp = KERNEL_MSG_ALLOC(SCPPS_SCAN_REFRESH_SEND_RSP,
