 ...
 ****************************************************************************************
 */
static int bass_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if(state == BASS_IDLE)
    {
            ...
            || (gapc_get_conhdl(param->conidx) == GAP_INVALID_CONHDL))
        {
            ...
            struct bass_enable_rsp* rsp = KERNEL_MSG_ALLOC(BASS_ENABLE_RSP, src_id,
        ...
        else
        {
            ...
            kernel_state_set(dest_id, BASS_BUSY);
            bass_env->ntf_cfg[param->conidx] = param->ntf_cfg;
            bass_env->operation = kernel_param2msg(param);
            bass_env->cursor = 0;
 ...
 ****************************************************************************************
 */
static int bass_batt_level_upd_req_handler(kernel_msg_id_t const msgid,
                                            ...
                                            kernel_task_id_t const src_id)
{
    ...
    if(state == BASS_IDLE)
    {
        ...
        {
            // an error occurs, trigg it.
            struct bass_batt_level_upd_rsp * rsp = KERNEL_MSG_ALLOC(BASS_BATT_LEVEL_UPD_RSP, src_id,
                                      ...
                                      kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
                ...
                && ((ntf_cfg == PRF_CLI_STOP_NTFIND) || (ntf_cfg == PRF_CLI_START_NTF)))
        {
            ...
            struct bass_batt_level_ntf_cfg_ind * ind = KERNEL_MSG_ALLOC(BASS_BATT_LEVEL_NTF_CFG_IND,
...
const struct kernel_msg_handler bass_default_state[] =
{
    {BASS_ENABLE_REQ,               (kernel_msg_func_t) bass_enable_req_handler},
    {BASS_BATT_LEVEL_UPD_REQ,       (kernel_msg_func_t) bass_batt_level_upd_req_handler},
...
const struct kernel_state_handler bass_default_handler = KERNEL_STATE_HANDLER(bass_default_state);
