...
#if (BLE_FEB3_SERVER)
extern const struct prf_task_cbs* feb3s_prf_itf_get(void);
...
static const struct prf_task_cbs * prf_itf_get(uint16_t task_id)
{
    ...
    switch(id)
    {
		...
		#if (BLE_FEB3_SERVER)
		case TASK_ID_FEB3S:
		prf_cbs = feb3s_prf_itf_get();
		break;
		#endif // (BLE_FEB3_SERVER)

		#if (BLE_MS_SERVER)
		case TASK_ID_MS:
		prf_cbs = ms_prf_itf_get();
		break;
		#endif // (BLE_MS_SERVER)

        #if (BLE_BTL_SERVER)
		case TASK_ID_BTL:
		prf_cbs = btl_prf_itf_get();
		break;
		#endif // (BLE_BTL_SERVER)

        #if (BLE_SN_SERVER)
		case TASK_ID_SN:
		prf_cbs = sn_prf_itf_get();
		break;
		#endif // (BLE_SN_SERVER)

        #if (BLE_AYLA_SERVER)
		case TASK_ID_AYLA:
		prf_cbs = ayla_prf_itf_get();
		break;
...
uint8_t prf_add_profile(struct gapm_profile_task_add_cmd * params, kernel_task_id_t* prf_task)
{
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        ...
        if(i == BLE_NB_PROFILES)
        {
			...
			//UART_PRINTF("GAP_ERR_INSUFF_RESOURCES 1\r\n");
        }
    }
