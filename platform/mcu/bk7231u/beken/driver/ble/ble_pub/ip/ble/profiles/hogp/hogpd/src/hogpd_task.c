 ...
 ****************************************************************************************
 */
static int hogpd_enable_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    // Send back response
    struct hogpd_enable_rsp* rsp = KERNEL_MSG_ALLOC(HOGPD_ENABLE_RSP, src_id, dest_id, hogpd_enable_rsp);
 ...
 ****************************************************************************************
 */
static int hogpd_report_upd_req_handler(kernel_msg_id_t const msgid,
                                        ...
                                        kernel_task_id_t const src_id)
{
    ...
    if((state & HOGPD_REQ_BUSY) == HOGPD_IDLE)
    {
        ...
        if (status != GAP_ERR_NO_ERROR)
        {
            ...
            struct hogpd_report_upd_rsp *rsp = KERNEL_MSG_ALLOC(HOGPD_REPORT_UPD_RSP,
        ...
        {
            kernel_state_set(dest_id, state | HOGPD_REQ_BUSY);
        }
    }
 ...
 ****************************************************************************************
 */
static int hogpd_report_cfm_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if((state & HOGPD_OP_BUSY) == HOGPD_OP_BUSY)
    {
        ...
        if(hogpd_env->op.operation == HOGPD_OP_REPORT_READ)
        {
            ...
            //Send read response
            struct gattc_read_cfm * rd_cfm = KERNEL_MSG_ALLOC_DYN(GATTC_READ_CFM,
                    KERNEL_BUILD_ID(TASK_GATTC,  hogpd_env->op.conidx), dest_id, gattc_read_cfm, length);
            ...
            kernel_msg_send(rd_cfm);
        }
        // write operation
        else
        {
            //Send write response
            struct gattc_write_cfm * wr_cfm = KERNEL_MSG_ALLOC(GATTC_WRITE_CFM,
                    KERNEL_BUILD_ID(TASK_GATTC,  hogpd_env->op.conidx), dest_id, gattc_write_cfm);
            wr_cfm->handle = hogpd_env->op.handle;
            wr_cfm->status = status;
            kernel_msg_send(wr_cfm);
        }
        ...
        kernel_state_set(dest_id, state & ~HOGPD_OP_BUSY);
    }
 ...
 ****************************************************************************************
 */
static int hogpd_proto_mode_cfm_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if((state & HOGPD_OP_BUSY) == HOGPD_OP_BUSY)
    {
        ...
        {
            //Send read response
            struct gattc_read_cfm * rd_cfm = KERNEL_MSG_ALLOC(GATTC_READ_CFM,
                    KERNEL_BUILD_ID(TASK_GATTC,  hogpd_env->op.conidx), dest_id, gattc_read_cfm);
            rd_cfm->handle = hogpd_env->op.handle;
            rd_cfm->status = status;
            kernel_msg_send(rd_cfm);
        }
        ...
        if(status == GAP_ERR_NO_ERROR)
        {
            ...
            switch(att_idx)
            {
                ...
                case HOGPD_IDX_HID_CTNL_PT_VAL:
                {
                    ...
                    struct hogpd_ctnl_pt_ind *cp_ind = KERNEL_MSG_ALLOC(HOGPD_CTNL_PT_IND,
                ...
                {
                    // send control point indication
                    struct hogpd_proto_mode_req_ind *prot_ind = KERNEL_MSG_ALLOC(HOGPD_PROTO_MODE_REQ_IND,
                                                                prf_dst_task_get(&(hogpd_env->prf_env), conidx), dest_id,
                ...
                case HOGPD_IDX_REPORT_VAL:
                {
                    ...
                    struct hogpd_report_req_ind *report_ind = KERNEL_MSG_ALLOC_DYN(HOGPD_REPORT_REQ_IND,
                            prf_dst_task_get(&(hogpd_env->prf_env), conidx), dest_id,
                ...
                case HOGPD_IDX_REPORT_NTF_CFG:
                {
                    uint16_t ntf_cfg = common_read16p(param->value);
                                      ...
                                      kernel_task_id_t const src_id)
{
    ...
    if((state & HOGPD_OP_BUSY) == HOGPD_IDLE)
    {
        ...
        {
            //Send write response
            struct gattc_read_cfm * cfm = KERNEL_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, length);
            cfm->handle = param->handle;
            cfm->status = status;
            cfm->length = length;
        ...
        {
            // send report indication
            struct hogpd_report_req_ind *report_ind = KERNEL_MSG_ALLOC(HOGPD_REPORT_REQ_IND,
                    prf_dst_task_get(&(hogpd_env->prf_env), conidx), dest_id,
                                 ...
                                 kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    if(param->operation == GATTC_NOTIFY)
    {
		...
		if(g_hid_handle == hogpd_get_att_handle(hogpd_env, 0x0, 0x14, 0x3))  //filter audio handle
		{
			// send report update response
	        struct hogpd_report_upd_rsp *rsp = KERNEL_MSG_ALLOC(HOGPD_REPORT_UPD_RSP,
        ...
        kernel_state_set(dest_id, kernel_state_get(dest_id) & ~HOGPD_REQ_BUSY);
...
const struct kernel_msg_handler hogpd_default_state[] =
{
    ...
    { HOGPD_REPORT_CFM,              (kernel_msg_func_t) hogpd_report_cfm_handler },
...
const struct kernel_state_handler hogpd_default_handler = KERNEL_STATE_HANDLER(hogpd_default_state);
