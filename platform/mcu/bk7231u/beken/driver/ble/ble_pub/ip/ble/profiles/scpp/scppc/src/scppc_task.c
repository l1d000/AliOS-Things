 ...
 ****************************************************************************************
 */
static int scppc_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if((state == SCPPC_IDLE) && (scppc_env->env[conidx] == NULL))
    {
        ...
        scppc_env->env[conidx] = (struct scppc_cnx_env*) kernel_malloc(sizeof(struct scppc_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, SCPPC_BUSY);
 ...
 ****************************************************************************************
 */
static int scppc_scan_intv_wd_wr_req_handler(kernel_msg_id_t const msgid,
                                             ...
                                             kernel_task_id_t const src_id)
{
    ...
    if(state == SCPPC_IDLE)
    {
        ...
        else
        {
            ...
            if(handle != ATT_INVALID_HANDLE)
            {
                ...
                common_write16p(&(data[0]) , param->scan_intv_wd.le_scan_intv);
                common_write16p(&(data[sizeof(uint16_t)]) , param->scan_intv_wd.le_scan_window);
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        struct scppc_scan_intv_wd_wr_rsp * rsp = KERNEL_MSG_ALLOC(SCPPC_SCAN_INTV_WD_WR_RSP,
 ...
 ****************************************************************************************
 */
static int scppc_scan_refresh_ntf_cfg_rd_req_handler(kernel_msg_id_t const msgid,
                                                     void const *param,
                                                     kernel_task_id_t const dest_id,
                                                     kernel_task_id_t const src_id)
{
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        struct scppc_scan_refresh_ntf_cfg_rd_rsp * rsp = KERNEL_MSG_ALLOC(SCPPC_SCAN_REFRESH_NTF_CFG_RD_RSP,
 ...
 ****************************************************************************************
 */
static int scppc_scan_refresh_ntf_cfg_req_handler(kernel_msg_id_t const msgid,
                                                  ...
                                                  kernel_task_id_t const src_id)
{
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        struct scppc_scan_refresh_ntf_cfg_rsp * rsp = KERNEL_MSG_ALLOC(SCPPC_SCAN_REFRESH_NTF_CFG_RSP,
                                ...
                                kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
            ...
            && (scppc_env->env[conidx]->operation != SCPPC_OP_NONE))
    {
        ...
        scppc_env->env[conidx]->operation = SCPPC_OP_NONE;
        kernel_state_set(dest_id, SCPPC_IDLE);
    }
 ...
 ****************************************************************************************
 */
static int gattc_read_ind_handler(kernel_msg_id_t const msgid, struct gattc_read_ind const *param,
                                  kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    uint8_t state = kernel_state_get(dest_id);
    ...
    if(state == SCPPC_BUSY)
    {
        ...
        rsp->ntf_cfg = common_read16p(&(param->value[0]));
                                        ...
                                        kernel_task_id_t const src_id)
{
    ...
    if(state != SCPPC_FREE)
    {
        ...
        kernel_msg_send_basic(SCPPC_SCAN_REFRESH_IND, prf_dst_task_get(&(scppc_env->prf_env), KERNEL_IDX_GET(src_id)), dest_id);
    }
...
const struct kernel_msg_handler scppc_default_state[] =
{
    {SCPPC_ENABLE_REQ,                      (kernel_msg_func_t)scppc_enable_req_handler},
    {GATTC_SDP_SVC_IND,                     (kernel_msg_func_t)gattc_sdp_svc_ind_handler},

    {SCPPC_SCAN_INTV_WD_WR_REQ,             (kernel_msg_func_t)scppc_scan_intv_wd_wr_req_handler},
    {SCPPC_SCAN_REFRESH_NTF_CFG_RD_REQ,     (kernel_msg_func_t)scppc_scan_refresh_ntf_cfg_rd_req_handler},
    {SCPPC_SCAN_REFRESH_NTF_CFG_REQ,        (kernel_msg_func_t)scppc_scan_refresh_ntf_cfg_req_handler},
...
const struct kernel_state_handler scppc_default_handler = KERNEL_STATE_HANDLER(scppc_default_state);
