 ...
 ****************************************************************************************
 */
static int htpc_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if((state == HTPC_IDLE) && (htpc_env->env[conidx] == NULL))
    {
        ...
        htpc_env->env[conidx] = (struct htpc_cnx_env*) kernel_malloc(sizeof(struct htpc_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            htpc_env->env[conidx]->operation = kernel_param2msg(param);
            msg_status = KERNEL_MSG_NO_FREE;
            kernel_state_set(dest_id, HTPC_BUSY);
        }
 ...
 ****************************************************************************************
 */
static int htpc_wr_meas_intv_req_handler(kernel_msg_id_t const msgid,
                                        ...
                                        kernel_task_id_t const src_id)
{
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        struct htpc_wr_meas_intv_rsp *rsp = KERNEL_MSG_ALLOC(HTPC_WR_MEAS_INTV_RSP, src_id, dest_id, htpc_wr_meas_intv_rsp);
        rsp->status = status;
        kernel_msg_send(rsp);
    }
 ...
 ****************************************************************************************
 */
static int htpc_health_temp_ntf_cfg_req_handler(kernel_msg_id_t const msgid,
                                       ...
                                       kernel_task_id_t const src_id)
{
    ...
    {
        struct htpc_health_temp_ntf_cfg_rsp* rsp = KERNEL_MSG_ALLOC(HTPC_HEALTH_TEMP_NTF_CFG_RSP,
                                                                src_id, dest_id,
 ...
 ****************************************************************************************
 */
static int htpc_rd_char_req_handler(kernel_msg_id_t const msgid,
                                ...
                                kernel_task_id_t const src_id)
{
    ...
    if(state == HTPC_BUSY)
    {
        switch(htpc_env->env[conidx]->operation->id)
        {
            ...
            {
                struct htpc_wr_meas_intv_rsp *rsp = KERNEL_MSG_ALLOC(HTPC_WR_MEAS_INTV_RSP,
        ...
        {
            kernel_free(htpc_env->env[conidx]->operation);
            htpc_env->env[conidx]->operation = NULL;
        }
        // set state to IDLE
        kernel_state_set(dest_id, HTPC_IDLE);
    }
                                        ...
                                        kernel_task_id_t const src_id)
{
    ...
    else if (param->handle == htpc_env->env[conidx]->hts.chars[HTPC_CHAR_HTS_MEAS_INTV].val_hdl)
    {
        ...
        struct htpc_meas_intv_ind * ind = KERNEL_MSG_ALLOC(HTPC_MEAS_INTV_IND,
...
const struct kernel_msg_handler htpc_default_state[] =
{
...
const struct kernel_state_handler htpc_default_handler = KERNEL_STATE_HANDLER(htpc_default_state);
