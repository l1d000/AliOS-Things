 ...
 ****************************************************************************************
 */
static int hrpc_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if((state == HRPC_IDLE) && (hrpc_env->env[conidx] == NULL))
    {
        ...
        hrpc_env->env[conidx] = (struct hrpc_cnx_env*) kernel_malloc(sizeof(struct hrpc_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, HRPC_DISCOVERING);
        }
        ...
        else if(state == HRPC_BUSY)
        {
            switch(param->operation)
            {
                ...
                case GATTC_WRITE_NO_RESPONSE:
                {
                    ...
                    struct hrpc_cfg_indntf_rsp *rsp = KERNEL_MSG_ALLOC(rsp_msg_id,
            ...
            kernel_state_set(prf_src_task_get(&hrpc_env->prf_env, conidx), HRPC_IDLE);
        }
    }
    return (KERNEL_MSG_CONSUMED);
}
 ...
 ****************************************************************************************
 */
static int hrpc_rd_char_req_handler(kernel_msg_id_t const msgid,
    ...
    if(state == HRPC_IDLE)
    {
        ...
        else
        {
            ...
            if (search_hdl != ATT_INVALID_SEARCH_HANDLE)
            {
                ...
                kernel_state_set(dest_id, HRPC_BUSY);
 ...
 ****************************************************************************************
 */
static int hrpc_cfg_indntf_req_handler(kernel_msg_id_t const msgid,
                                       ...
                                       kernel_task_id_t const src_id)
{
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        struct hrpc_cfg_indntf_rsp *rsp = KERNEL_MSG_ALLOC(HRPC_CFG_INDNTF_RSP, src_id, dest_id, hrpc_cfg_indntf_rsp);
        rsp->status    = status;
        // Send the message
        kernel_msg_send(rsp);
    }
...
****************************************************************************************
*/
static int hrpc_wr_cntl_point_req_handler(kernel_msg_id_t const msgid,
                                       ...
                                       kernel_task_id_t const src_id)
{
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        struct hrpc_wr_cntl_point_rsp *rsp = KERNEL_MSG_ALLOC(HRPC_WR_CNTL_POINT_RSP, src_id, dest_id, hrpc_wr_cntl_point_rsp);
        rsp->status    = status;
        // Send the message
        kernel_msg_send(rsp);
    }
                                        ...
                                        kernel_task_id_t const src_id)
{
    ...
    if(hrpc_env != NULL)
    {
                ...
                (param->type == GATTC_NOTIFY))
        {
            ...
            struct hrpc_hr_meas_ind * ind = KERNEL_MSG_ALLOC(
                    HRPC_HR_MEAS_IND,
...
const struct kernel_msg_handler hrpc_default_state[] =
{
    ...
    {HRPC_CFG_INDNTF_REQ,           (kernel_msg_func_t)hrpc_cfg_indntf_req_handler},
    {GATTC_EVENT_IND,               (kernel_msg_func_t)gattc_event_ind_handler},
    {HRPC_RD_CHAR_REQ,              (kernel_msg_func_t)hrpc_rd_char_req_handler},
    {GATTC_READ_IND,                (kernel_msg_func_t)gattc_read_ind_handler},
    {HRPC_WR_CNTL_POINT_REQ,        (kernel_msg_func_t)hrpc_wr_cntl_point_req_handler},
};

// Specifies the message handlers that are common to all states.
const struct kernel_state_handler hrpc_default_handler = KERNEL_STATE_HANDLER(hrpc_default_state);
