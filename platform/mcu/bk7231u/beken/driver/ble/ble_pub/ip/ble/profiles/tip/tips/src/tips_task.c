 ...
 ****************************************************************************************
 */
static int tips_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
     ...
     else if(kernel_state_get(dest_id) == TIPS_IDLE)
     {
     ...
     struct tips_enable_rsp * rsp = KERNEL_MSG_ALLOC(
             TIPS_ENABLE_RSP,
             src_id,
             dest_id,
             tips_enable_rsp);
 ...
 ****************************************************************************************
 */
static int tips_upd_curr_time_req_handler(kernel_msg_id_t const msgid,
                                          ...
                                          kernel_task_id_t const src_id)
{
    ...
    if ((tips_env->env[conidx] != NULL) && (kernel_state_get(dest_id) == TIPS_IDLE))
    {
        ...
        if ((tips_env->env[conidx]->ntf_state & TIPS_CTS_CURRENT_TIME_CFG) == TIPS_CTS_CURRENT_TIME_CFG)
        {
            ...
            if (status == GAP_ERR_NO_ERROR)
            {
                ...
                // Go to idle state
                kernel_state_set(dest_id, TIPS_BUSY);
                // Send the event
    ...
    {
        // Inform APP
        struct tips_upd_curr_time_rsp *ind = KERNEL_MSG_ALLOC(
                                      ...
                                      kernel_task_id_t const src_id)
{
    ...
    if((kernel_state_get(dest_id) == TIPS_IDLE) && (tips_env->env[conidx] != NULL))
    {
        ...
        if (param->handle == tips_env->cts_shdl + CTS_IDX_CURRENT_TIME_CFG)
        {
            ...
            struct gattc_read_cfm* cfm = KERNEL_MSG_ALLOC_DYN(
                    GATTC_READ_CFM,
                    src_id, dest_id,
                    gattc_read_cfm,
                    sizeof(uint16_t));
        ...
        else
        {
            ...
            struct tips_rd_req_ind* ind = KERNEL_MSG_ALLOC(
                    TIPS_RD_REQ_IND,
                    prf_dst_task_get(&(tips_env->prf_env), conidx),
                    prf_src_task_get(&(tips_env->prf_env), conidx),
                    tips_rd_req_ind);
            ...
            {
                // Free message
                kernel_msg_free(kernel_param2msg(ind));
                // Send data to peer device
                struct gattc_read_cfm* cfm = KERNEL_MSG_ALLOC(
                        GATTC_READ_CFM,
                        src_id, dest_id,
                        gattc_read_cfm);
 ...
 ****************************************************************************************
 */
static int tips_rd_cfm_handler(kernel_msg_id_t const msgid,
                                ...
                                kernel_task_id_t const src_id)
{
    ...
    if ((kernel_state_get(dest_id) == TIPS_BUSY) && (tips_env->env[conidx] != NULL))
    {
        if (tips_env->env[conidx]->handle != ATT_INVALID_HANDLE)
        {
            ...
            kernel_msg_send(cfm);
            // Go to idle state
            kernel_state_set(dest_id, TIPS_IDLE);
...
//    uint8_t time_upd_ctnl_pt     = 0x00;
    ...
    if (param->handle == tips_env->cts_shdl + CTS_IDX_CURRENT_TIME_CFG)
    {
        ...
        if ((cfg_value == PRF_CLI_STOP_NTFIND) || (cfg_value == PRF_CLI_START_NTF))
        {
            ...
            struct tips_current_time_ccc_ind * msg = KERNEL_MSG_ALLOC(
    ...
    else if (param->handle == tips_env->rtus_shdl + RTUS_IDX_TIME_UPD_CTNL_PT_VAL)
    {
            ...
            (param->value[0] == TIPS_TIME_UPD_CTNL_PT_CANCEL))
        {
            ...
            struct tips_time_upd_ctnl_pt_ind * msg = KERNEL_MSG_ALLOC(
...
const struct kernel_msg_handler tips_default_state[] =
{
    ...
    {TIPS_UPD_CURR_TIME_REQ,            (kernel_msg_func_t)tips_upd_curr_time_req_handler},
...
const struct kernel_state_handler tips_default_handler = KERNEL_STATE_HANDLER(tips_default_state);
