 ...
 ****************************************************************************************
 */
static int glps_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    cmp_evt = KERNEL_MSG_ALLOC(GLPS_ENABLE_RSP, src_id, dest_id, glps_enable_rsp);
 ...
 ****************************************************************************************
 */
static int glps_meas_send_req_handler(kernel_msg_id_t const msgid,
                                      ...
                                      kernel_task_id_t const src_id)
{
    ...
    if(state == GLPS_IDLE)
    {
        ...
        if(!GLPS_IS(conidx, SENDING_MEAS))
        {
            ...
            else
            {
                ...
                {
                    // Allocate the GATT notification message
                    struct gattc_send_evt_cmd *meas_ctx_val = KERNEL_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
                            KERNEL_BUILD_ID(TASK_GATTC, conidx), dest_id,
    ...
    {
        // send completed information to APP task that contains error status
        struct glps_cmp_evt * cmp_evt = KERNEL_MSG_ALLOC(GLPS_CMP_EVT, src_id,
 ...
 ****************************************************************************************
 */
static int glps_send_racp_rsp_cmd_handler(kernel_msg_id_t const msgid,
                                     ...
                                     kernel_task_id_t const src_id)
{
    ...
    if(state == GLPS_IDLE)
    {
        ...
        else
        {
            ...
            struct gattc_send_evt_cmd *racp_rsp_val = KERNEL_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
                    KERNEL_BUILD_ID(TASK_GATTC, conidx), dest_id,
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        ...
        struct glps_cmp_evt * cmp_evt = KERNEL_MSG_ALLOC(GLPS_CMP_EVT, src_id, dest_id, glps_cmp_evt);
                                      ...
                                      kernel_task_id_t const src_id)
{
    ...
    if(state == GLPS_IDLE)
    {
                ...
                || (att_idx == GLS_IDX_REC_ACCESS_CTRL_IND_CFG))
        {
            ...
            cli_cfg = common_read16(&(param->value));
            ...
            if (status == GAP_ERR_NO_ERROR)
            {
                ...
                struct glps_cfg_indntf_ind * ind = KERNEL_MSG_ALLOC(GLPS_CFG_INDNTF_IND,
        ...
        else if (att_idx == GLS_IDX_REC_ACCESS_CTRL_VAL)
        {
            ...
            else if(GLPS_IS(conidx, RACP_ON_GOING))
            {
                ...
                if((param->offset == 0) && (param->value[0] == GLP_REQ_ABORT_OP))
                {
                    ...
                    struct glps_racp_req_rcv_ind * req = KERNEL_MSG_ALLOC(GLPS_RACP_REQ_RCV_IND,
                                      ...
                                      kernel_task_id_t const src_id)
{
    ...
    if(state == GLPS_IDLE)
    {
        ...
        struct gattc_read_cfm* cfm = KERNEL_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(uint16_t));
        cfm->handle = param->handle;
        ...
        switch(att_idx)
        {
            ...
            {
                common_write16p(cfm->value, (glps_env->env[conidx]->evt_cfg & GLPS_MEAS_NTF_CFG)
                        ? PRF_CLI_START_NTF : PRF_CLI_STOP_NTFIND);
            }
            break;
            case GLS_IDX_MEAS_CTX_NTF_CFG:
            {
                common_write16p(cfm->value, (glps_env->env[conidx]->evt_cfg & GLPS_MEAS_CTX_NTF_CFG)
                        ? PRF_CLI_START_NTF : PRF_CLI_STOP_NTFIND);
            }
            break;
            case GLS_IDX_REC_ACCESS_CTRL_IND_CFG:
            {
                common_write16p(cfm->value, (glps_env->env[conidx]->evt_cfg & GLPS_RACP_IND_CFG)
                        ? PRF_CLI_START_IND : PRF_CLI_STOP_NTFIND);
            }
            break;
            case GLS_IDX_FEATURE_VAL:
            {
                common_write16p(cfm->value, glps_env->features);
            }
            break;
            default:
            {
        ...
        switch(param->operation)
        {
            case GATTC_NOTIFY:
            {
                        ...
                        || (GLPS_IS(conidx, MEAS_CTX_SENT) && (GLPS_IS(conidx, MEAS_SENT))))
                {
                    ...
                    struct glps_cmp_evt * cmp_evt = KERNEL_MSG_ALLOC(GLPS_CMP_EVT, prf_dst_task_get(&(glps_env->prf_env), conidx),
                            dest_id, glps_cmp_evt);
...
const struct kernel_msg_handler glps_default_state[] =
{
    ...
    {GLPS_SEND_MEAS_WITH_CTX_CMD,         (kernel_msg_func_t) glps_meas_send_req_handler},
    {GLPS_SEND_MEAS_WITHOUT_CTX_CMD,      (kernel_msg_func_t) glps_meas_send_req_handler},
...
const struct kernel_state_handler glps_default_handler = KERNEL_STATE_HANDLER(glps_default_state);
