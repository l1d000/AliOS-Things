 ...
 ****************************************************************************************
 */
static int blpc_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if((state == BLPC_IDLE) && (blpc_env->env[conidx] == NULL))
    {
        ...
        blpc_env->env[conidx] = (struct blpc_cnx_env*) kernel_malloc(sizeof(struct blpc_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, BLPC_DISCOVERING);
        }
    ...
    if (blpc_env != NULL)
    {
        ...
        else if(BLPC_BUSY)
        {
            switch(param->operation)
            {
                ...
                {
                    struct blpc_cfg_indntf_rsp *wr_cfm = KERNEL_MSG_ALLOC(
                            BLPC_CFG_INDNTF_RSP,
            ...
            kernel_state_set(prf_src_task_get(&blpc_env->prf_env, conidx), BLPC_IDLE);
        }
    }
    return (KERNEL_MSG_CONSUMED);
}
 ...
 ****************************************************************************************
 */
static int blpc_rd_char_req_handler(kernel_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int blpc_cfg_indntf_req_handler(kernel_msg_id_t const msgid,
                                       ...
                                       kernel_task_id_t const src_id)
{
    ...
    if((blpc_env->env[conidx] != NULL) && (kernel_state_get(dest_id) == BLPC_IDLE))
    {
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        struct blpc_cfg_indntf_rsp *rsp = KERNEL_MSG_ALLOC(BLPC_CFG_INDNTF_RSP, src_id, dest_id, blpc_cfg_indntf_rsp);
        ...
        switch(param->type)
        {
            ...
            case GATTC_NOTIFY:
            {
                    ...
                    || ((param->handle == blpc_env->env[conidx]->bps.chars[BLPC_CHAR_BP_MEAS].val_hdl)))
                {
                    ...
                    struct blpc_bp_meas_ind * ind = KERNEL_MSG_ALLOC(
                            BLPC_BP_MEAS_IND,
                            prf_dst_task_get(&(blpc_env->prf_env), conidx),
                            prf_src_task_get(&(blpc_env->prf_env), conidx),
                            blpc_bp_meas_ind);
...
const struct kernel_msg_handler blpc_default_state[] =
{
    ...
    {BLPC_RD_CHAR_REQ,              (kernel_msg_func_t)blpc_rd_char_req_handler},
    ...
    {BLPC_CFG_INDNTF_REQ,           (kernel_msg_func_t)blpc_cfg_indntf_req_handler},
};

// Specifies the message handlers that are common to all states.
const struct kernel_state_handler blpc_default_handler = KERNEL_STATE_HANDLER(blpc_default_state);
