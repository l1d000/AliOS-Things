...
static uint8_t glps_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl, struct glps_db_cfg* params)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        struct glps_env_tag* glps_env =
                (struct glps_env_tag* ) kernel_malloc(sizeof(struct glps_env_tag), KERNEL_MEM_ATT_DB);
        ...
        for(idx = 0; idx < GLPS_IDX_MAX ; idx++)
        {
            ...
            kernel_state_set(KERNEL_BUILD_ID(env->task, idx), GLPS_FREE);
        }
    }
...
static void glps_destroy(struct prf_task_env* env)
{
    ...
    for(idx = 0; idx < GLPS_IDX_MAX ; idx++)
    {
        ...
        {
            kernel_free(glps_env->env[idx]);
        }
    }

    // free profile environment variables
    env->env = NULL;
    kernel_free(glps_env);
}
...
static void glps_create(struct prf_task_env* env, uint8_t conidx)
{
    ...
    glps_env->env[conidx] = (struct glps_cnx_env*)
            kernel_malloc(sizeof(struct glps_cnx_env), KERNEL_MEM_ATT_DB);

    memset(glps_env->env[conidx], 0, sizeof(struct glps_cnx_env));

    /* Put GLS in Idle state */
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), GLPS_IDLE);
}
...
static void glps_cleanup(struct prf_task_env* env, uint8_t conidx, uint8_t reason)
{
    ...
    {
        kernel_free(glps_env->env[conidx]);
        glps_env->env[conidx] = NULL;
    }

    /* Put GLS in Free state */
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), GLPS_FREE);
}
                             ...
                             uint16_t seq_num)
{
    ...
    // Sequence Number
    common_write16p(packed_meas + cursor, seq_num);
    cursor += 2;
    ...
    {
        common_write16p(packed_meas + cursor, meas_val->time_offset);
        cursor += 2;
    }
    ...
    {
        common_write16p(packed_meas + cursor, meas_val->concentration);
        cursor += 2;
    ...
    {
        common_write16p(packed_meas + cursor, meas_val->status);
        cursor += 2;
    }
                                 ...
                                 uint16_t seq_num)
{
    ...
    // Sequence Number
    common_write16p(packed_meas_ctx + cursor, seq_num);
    cursor += 2;
    ...
    if((meas_ctx_val->flags & GLP_CTX_CRBH_ID_AND_CRBH_PRES) != 0)
    {
        ...
        common_write16p(packed_meas_ctx + cursor, meas_ctx_val->carbo_val);
        cursor += 2;
    }
    ...
    {
        // Exercise Duration
        common_write16p(packed_meas_ctx + cursor, meas_ctx_val->exercise_dur);
        cursor += 2;
    ...
    if((meas_ctx_val->flags & GLP_CTX_MEDIC_ID_AND_MEDIC_PRES) != 0)
    {
        ...
        // Medication Present
        common_write16p(packed_meas_ctx + cursor, meas_ctx_val->med_val);
        cursor += 2;
    }
    ...
    {
        // HbA1c
        common_write16p(packed_meas_ctx + cursor, meas_ctx_val->hba1c_val);
        cursor += 2;
    }
            ...
            && (racp_req->filter.operator <= GLP_OP_WITHIN_RANGE_OF))
    {
        ...
        if(racp_req->filter.filter_type == GLP_FILTER_SEQ_NUMBER)
        {
                    ...
                    || (racp_req->filter.operator == GLP_OP_WITHIN_RANGE_OF))
            {
                ...
                // retrieve minimum value
                racp_req->filter.val.seq_num.min = common_read16p(packed_val + cursor);
                cursor +=2;
            }
                    ...
                    || (racp_req->filter.operator == GLP_OP_WITHIN_RANGE_OF))
            {
                ...
                // retrieve maximum value
                racp_req->filter.val.seq_num.max = common_read16p(packed_val + cursor);
                cursor +=2;
            }
                                      ...
                                      struct glp_racp_rsp* racp_rsp)
{
    ...
    {
        common_write16(packed_val+cursor, racp_rsp->operand.num_of_record);
        cursor += 2;
    }
    else
    {
