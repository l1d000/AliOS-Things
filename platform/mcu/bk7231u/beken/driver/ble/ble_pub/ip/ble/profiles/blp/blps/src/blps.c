...
static uint8_t blps_init(struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl, struct blps_db_cfg* params)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        struct blps_env_tag* blps_env =
                (struct blps_env_tag* ) kernel_malloc(sizeof(struct blps_env_tag), KERNEL_MEM_ATT_DB);
        ...
        kernel_state_set(env->task, BLPS_IDLE);
    }
...
static void blps_create(struct prf_task_env* env, uint8_t conidx)
{
    ...
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), BLPS_IDLE);
}
...
uint8_t blps_pack_meas_value(uint8_t *packed_bp, const struct bps_bp_meas* pmeas_val)
{
    ...
    *(packed_bp) = pmeas_val->flags;
    ...
    common_write16p(packed_bp + 1, pmeas_val->systolic);

    // Blood Pressure Measurement Compound Value - Diastolic (mmHg)
    common_write16p(packed_bp + 3, pmeas_val->diastolic);

    //  Blood Pressure Measurement Compound Value - Mean Arterial Pressure (mmHg)
    common_write16p(packed_bp + 5, pmeas_val->mean_arterial_pressure);
    ...
    {
        common_write16p(packed_bp + cursor, pmeas_val->pulse_rate);
        cursor += 2;
    }
    ...
    {
        common_write16p(packed_bp + cursor, pmeas_val->meas_status);
        cursor += 2;
    }
...
void blps_meas_send_rsp_send(uint8_t conidx, uint8_t status)
{
    ...
    struct blps_meas_send_rsp * rsp = KERNEL_MSG_ALLOC(BLPS_MEAS_SEND_RSP,
