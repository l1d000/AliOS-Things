 ...
 ****************************************************************************************
 */
static int hogpbh_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if((state == HOGPBH_IDLE) && (hogpbh_env->env[conidx] == NULL))
    {
        ...
        hogpbh_env->env[conidx] = (struct hogpbh_cnx_env*) kernel_malloc(sizeof(struct hogpbh_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, HOGPBH_BUSY);
            hogpbh_env->env[conidx]->operation = kernel_param2msg(param);
 ...
 ****************************************************************************************
 */
static int hogpbh_read_info_req_handler(kernel_msg_id_t const msgid, struct hogpbh_read_info_req const *param,
                                      kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        struct hogpbh_read_info_rsp * rsp = KERNEL_MSG_ALLOC(HOGPBH_READ_INFO_RSP,
 ...
 ****************************************************************************************
 */
static int hogpbh_write_req_handler(kernel_msg_id_t const msgid, struct hogpbh_write_req const *param,
                                      kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    int msg_status = KERNEL_MSG_CONSUMED;
    uint8_t state = kernel_state_get(dest_id);
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        struct hogpbh_write_rsp * rsp = KERNEL_MSG_ALLOC(HOGPBH_WRITE_RSP,
                                ...
                                kernel_task_id_t const src_id)
{
            ...
            && (hogpbh_env->env[conidx]->operation != NULL))
    {
        switch(hogpbh_env->env[conidx]->operation->id)
        {
            case HOGPBH_ENABLE_REQ:
            {
                ...
                if (param->status == ATT_ERR_NO_ERROR)
                {
                    ...
                    {
                        uint8_t i;
                        for (i = 0 ; (i < common_min(hogpbh_env->env[conidx]->hids_nb, HOGPBH_NB_HIDS_INST_MAX))
            ...
            {
                struct hogpbh_read_info_req* req = (struct hogpbh_read_info_req*) kernel_msg2param(hogpbh_env->env[conidx]->operation);
            ...
            {
                struct hogpbh_write_req* req = (struct hogpbh_write_req*) kernel_msg2param(hogpbh_env->env[conidx]->operation);
        ...
        kernel_free(hogpbh_env->env[conidx]->operation);
        hogpbh_env->env[conidx]->operation = NULL;
        kernel_state_set(dest_id, HOGPBH_IDLE);
    }
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if(state == HOGPBH_BUSY)
    {
        ...
        struct hogpbh_read_info_rsp * rsp = KERNEL_MSG_ALLOC_DYN(HOGPBH_READ_INFO_RSP,
        ...
        switch(req->info)
        {
            ...
            case HOGPBH_BOOT_MOUSE_IN_NTF_CFG:
            {
               rsp->data.ntf_cfg = common_read16p(param->value);
            }break;
            default:
            {
                ASSERT_ERR(0);
                                         ...
                                         kernel_task_id_t const src_id)
{
    ...
    if(state != HOGPBH_FREE)
    {
        ...
        if(att_info != HOGPBH_INFO_MAX)
        {
            ...
            struct hogpbh_boot_report_ind* ind = KERNEL_MSG_ALLOC_DYN(HOGPBH_BOOT_REPORT_IND,
...
const struct kernel_msg_handler hogpbh_default_state[] =
{
...
const struct kernel_state_handler hogpbh_default_handler = KERNEL_STATE_HANDLER(hogpbh_default_state);
