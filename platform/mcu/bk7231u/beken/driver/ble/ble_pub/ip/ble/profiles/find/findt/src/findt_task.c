                                      ...
                                      kernel_task_id_t const src_id)
{
    ...
    if((att_idx == FINDT_IAS_IDX_ALERT_LVL_VAL) && (param->value[0] <= FINDT_ALERT_HIGH))
    {
        ...
        struct findt_alert_ind *ind = KERNEL_MSG_ALLOC(FINDT_ALERT_IND,
...
const struct kernel_msg_handler findt_default_state[] =
{
    {GATTC_WRITE_REQ_IND,                 (kernel_msg_func_t) gattc_write_req_ind_handler},
};

// Specifies the message handlers that are common to all states.
const struct kernel_state_handler findt_default_handler = KERNEL_STATE_HANDLER(findt_default_state);
