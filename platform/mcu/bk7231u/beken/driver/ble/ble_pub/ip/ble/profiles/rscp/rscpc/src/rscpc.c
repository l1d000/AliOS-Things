...
static uint8_t rscpc_init(struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  void* params)
{
    ...
    struct rscpc_env_tag* rscpc_env =
            (struct rscpc_env_tag* ) kernel_malloc(sizeof(struct rscpc_env_tag), KERNEL_MEM_ATT_DB);
    ...
    for(idx = 0; idx < RSCPC_IDX_MAX ; idx++)
    {
        ...
        kernel_state_set(KERNEL_BUILD_ID(env->task, idx), RSCPC_FREE);
    }
...
static void rscpc_cleanup(struct prf_task_env* env, uint8_t conidx, uint8_t reason)
{
    ...
    if(rscpc_env->env[conidx] != NULL)
    {
        ...
        {
            kernel_free(kernel_param2msg(rscpc_env->env[conidx]->operation));
        }
        kernel_timer_clear(RSCPC_TIMEOUT_TIMER_IND, prf_src_task_get(&rscpc_env->prf_env, conidx));
        kernel_free(rscpc_env->env[conidx]);
        rscpc_env->env[conidx] = NULL;
    }

    /* Put RSCP Client in Free state */
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), RSCPC_FREE);
}
...
static void rscpc_create(struct prf_task_env* env, uint8_t conidx)
{
    ...
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), RSCPC_IDLE);
}
...
void rscpc_enable_rsp_send(struct rscpc_env_tag *rscpc_env, uint8_t conidx, uint8_t status)
{
    ...
    struct rscpc_enable_rsp * rsp = KERNEL_MSG_ALLOC(
            RSCPC_ENABLE_RSP,
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        kernel_state_set(prf_src_task_get(&(rscpc_env->prf_env), conidx), RSCPC_IDLE);
    }
...
{
    // Send the message
    struct rscpc_cmp_evt *evt = KERNEL_MSG_ALLOC(RSCPC_CMP_EVT,
                                             dest_id, src_id,
                                             rscpc_cmp_evt);
...
void rscpc_send_cmp_evt(struct rscpc_env_tag *rscpc_env, uint8_t conidx, uint8_t operation, uint8_t status)
{
    ...
    if (rscpc_env->env[conidx]->operation != NULL)
    {
        kernel_msg_free(kernel_param2msg(rscpc_env->env[conidx]->operation));
        rscpc_env->env[conidx]->operation = NULL;
    }

    // Go back to the CONNECTED state if the state is busy
    if (kernel_state_get(prf_src_task_get(&(rscpc_env->prf_env), conidx)) == RSCPC_BUSY)
    {
        kernel_state_set(prf_src_task_get(&(rscpc_env->prf_env), conidx), RSCPC_IDLE);
    }
