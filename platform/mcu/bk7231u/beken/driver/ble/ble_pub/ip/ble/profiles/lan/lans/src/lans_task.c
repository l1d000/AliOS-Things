...
static int lans_enable_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if(kernel_state_get(dest_id) == LANS_IDLE)
    {
    ...
    struct lans_enable_rsp *cmp_evt = KERNEL_MSG_ALLOC(LANS_ENABLE_RSP, src_id, dest_id, lans_enable_rsp);
                                      ...
                                      kernel_task_id_t const src_id)
{
    if(kernel_state_get(dest_id) == LANS_IDLE)
    {
        ...
        switch(att_idx)
        {
            ...
            {
                // Allocate structure
                cfm = KERNEL_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(uint32_t));
                cfm->length = sizeof(uint32_t);
                // Fill data
                common_write32p(cfm->value, lans_env->features);
            } break;
            ...
            case LNS_IDX_POS_Q_VAL:
            {
                ...
                {
                    // Allocate structure
                    cfm = KERNEL_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(struct lanp_posq));
                    // Fill data
            ...
            case LNS_IDX_LOC_SPEED_NTF_CFG:
            {
                ...
                // Fill data
                common_write16p(cfm->value, (lans_env->env[conidx].prfl_ntf_ind_cfg & LANS_PRF_CFG_FLAG_LOC_SPEED_NTF)
            ...
            case LNS_IDX_NAVIGATION_NTF_CFG:
            {
                ...
                if (LANS_IS_FEATURE_SUPPORTED(lans_env->prfl_cfg, LANS_NAVI_MASK))
                {
                    ...
                    cfm = KERNEL_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(uint16_t));
                    cfm->length = sizeof(uint16_t);
                    common_write16p(cfm->value, (lans_env->env[conidx].prfl_ntf_ind_cfg & LANS_PRF_CFG_FLAG_NAVIGATION_NTF)
                            ? PRF_CLI_START_NTF : PRF_CLI_STOP_NTFIND);
                }
                else
                {
            ...
            case LNS_IDX_LN_CTNL_PT_IND_CFG:
            {
                ...
                if (LANS_IS_FEATURE_SUPPORTED(lans_env->prfl_cfg, LANS_LN_CTNL_PT_MASK))
                {
                    ...
                    cfm->length = sizeof(uint16_t);
                    // Fill data
                    common_write16p(cfm->value, (lans_env->env[conidx].prfl_ntf_ind_cfg & LANS_PRF_CFG_FLAG_LN_CTNL_PT_IND)
 ...
 ****************************************************************************************
 */
static int lans_ntf_loc_speed_req_handler(kernel_msg_id_t const msgid,
                                          ...
                                          kernel_task_id_t const src_id)
{
    ...
    if (kernel_state_get(dest_id) == LANS_IDLE)
    {
        ...
        lans_env->op_data = (struct lans_op*) kernel_malloc(sizeof(struct lans_op), KERNEL_MEM_KERNEL_MSG);
        lans_env->op_data->cmd         = kernel_param2msg(param);
        lans_env->op_data->cursor      = 0;
        lans_env->op_data->ntf_pending = NULL;

        // Go to busy state
        kernel_state_set(dest_id, LANS_BUSY);
 ...
 ****************************************************************************************
 */
static int lans_upd_pos_q_req_handler(kernel_msg_id_t const msgid,
                                          ...
                                          kernel_task_id_t const src_id)
{
    ...
    struct lans_upd_pos_q_rsp *rsp = KERNEL_MSG_ALLOC(LANS_UPD_POS_Q_RSP,
 ...
 ****************************************************************************************
 */
static int lans_ntf_navigation_req_handler(kernel_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int lans_ln_ctnl_pt_cfm_handler(kernel_msg_id_t const msgid,
                                          ...
                                          kernel_task_id_t const src_id)
{
    ...
    if (kernel_state_get(dest_id) == LANS_BUSY)
    {
        do
        {
...
const struct kernel_msg_handler lans_default_state[] =
{
    ...
    {LANS_NTF_LOC_SPEED_REQ,        (kernel_msg_func_t)lans_ntf_loc_speed_req_handler},
    {LANS_UPD_POS_Q_REQ,            (kernel_msg_func_t)lans_upd_pos_q_req_handler},
    {LANS_NTF_NAVIGATION_REQ,       (kernel_msg_func_t)lans_ntf_navigation_req_handler},
    {LANS_LN_CTNL_PT_CFM,           (kernel_msg_func_t)lans_ln_ctnl_pt_cfm_handler},
...
const struct kernel_state_handler lans_default_handler = KERNEL_STATE_HANDLER(lans_default_state);
