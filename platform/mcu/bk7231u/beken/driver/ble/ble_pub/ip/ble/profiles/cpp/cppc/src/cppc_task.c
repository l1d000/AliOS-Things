 ...
 ****************************************************************************************
 */
static int cppc_enable_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if((state == CPPC_IDLE) && (cppc_env->env[conidx] == NULL))
    {
        ...
        cppc_env->env[conidx] = (struct cppc_cnx_env*) kernel_malloc(sizeof(struct cppc_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, CPPC_DISCOVERING);
        }
 ...
 ****************************************************************************************
 */
static int cppc_read_req_handler(kernel_msg_id_t const msgid,
    ...
    if(state == CPPC_IDLE)
    {
        ...
        else
        {
            ...
            if (hdl != ATT_INVALID_SEARCH_HANDLE)
            {
                ...
                kernel_state_set(dest_id, CPPC_BUSY);
 ...
 ****************************************************************************************
 */
static int cppc_cfg_ntfind_cmd_handler(kernel_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int cppc_ctnl_pt_cfg_req_handler(kernel_msg_id_t const msgid,
                                         ...
                                         kernel_task_id_t const src_id)
{
    ...
    if (cppc_env != NULL)
    {
        ...
        do
        {
            ...
            ASSERT_ERR(kernel_state_get(dest_id) > CPPC_FREE);
            ...
            if (kernel_state_get(dest_id) != CPPC_IDLE)
            {
                // Another procedure is pending, keep the command for later
                msg_status = KERNEL_MSG_SAVED;
 ...
 ****************************************************************************************
 */
static int cppc_timeout_timer_ind_handler(kernel_msg_id_t const msgid,
                                           void const *param,
                                           kernel_task_id_t const dest_id,
                                           kernel_task_id_t const src_id)
{
                ...
                case GATTC_WRITE_NO_RESPONSE:
                {
                    ...
                    else if (operation == CPPC_CTNL_PT_CFG_WR_OP_CODE)
                    {
                        if (param->status == GAP_ERR_NO_ERROR)
                        {
                            ...
                            kernel_timer_set(CPPC_TIMEOUT_TIMER_IND, dest_id, ATT_TRANS_RTX);
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if (state == CPPC_BUSY)
    {
        ...
        struct cppc_value_ind *ind = KERNEL_MSG_ALLOC(
                CPPC_VALUE_IND,
        ...
        switch (((struct cpps_read_cmd *)cppc_env->env[conidx]->operation)->read_code)
        {
            ...
            {
                ind->value.sensor_feat = common_read32p(&param->value[0]);
            ...
            case (GATTC_INDICATE):
            {
                ...
                if (cppc_env->env[conidx]->operation != NULL)
                {
                    if (((struct cppc_cmd *)cppc_env->env[conidx]->operation)->operation == CPPC_CTNL_PT_CFG_IND_OP_CODE)
                    {
                        ...
                        kernel_timer_clear(CPPC_TIMEOUT_TIMER_IND, dest_id);
...
const struct kernel_state_handler cppc_default_handler = KERNEL_STATE_HANDLER(cppc_default_state);
