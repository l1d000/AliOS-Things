 ...
 ****************************************************************************************
 */
static int rscps_enable_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if(kernel_state_get(dest_id) == RSCPS_IDLE)
    {
    ...
    struct rscps_enable_rsp *cmp_evt = KERNEL_MSG_ALLOC(RSCPS_ENABLE_RSP, src_id, dest_id, rscps_enable_rsp);
    ...
    if(kernel_state_get(dest_id) == RSCPS_IDLE)
    {
        ...
        switch(att_idx)
        {
            case RSCS_IDX_RSC_MEAS_NTF_CFG:
            {
                ...
                common_write16p(cfm->value, (rscps_env->prfl_ntf_ind_cfg[conidx] & RSCP_PRF_CFG_FLAG_RSC_MEAS_NTF)
            ...
            case RSCS_IDX_RSC_FEAT_VAL:
            {
                ...
                cfm = KERNEL_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(uint16_t));
                cfm->length = sizeof(uint16_t);
                common_write16p(cfm->value, rscps_env->features);
            } break;
            ...
            case RSCS_IDX_SC_CTNL_PT_NTF_CFG:
            {
                ...
                cfm = KERNEL_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(uint16_t));
                cfm->length = sizeof(uint16_t);
                common_write16p(cfm->value, (rscps_env->prfl_ntf_ind_cfg[conidx] & RSCP_PRF_CFG_FLAG_SC_CTNL_PT_IND)
 ...
 ****************************************************************************************
 */
static int rscps_ntf_rsc_meas_req_handler(kernel_msg_id_t const msgid,
                                          ...
                                          kernel_task_id_t const src_id)
{
    ...
    if (kernel_state_get(dest_id) == RSCPS_IDLE)
    {
        ...
        rscps_env->ntf = (struct rscps_ntf*) kernel_malloc(sizeof(struct rscps_ntf), KERNEL_MEM_KERNEL_MSG);
        ...
        // Instantaneous Speed
        common_write16p(&rscps_env->ntf->value[1], param->inst_speed);
        ...
        {
            common_write16p(&rscps_env->ntf->value[rscps_env->ntf->length], param->inst_stride_len);
        ...
        {
            common_write32p(&rscps_env->ntf->value[rscps_env->ntf->length], param->total_dist);
        ...
        // Go to busy state
        kernel_state_set(dest_id, RSCPS_BUSY);
 ...
 ****************************************************************************************
 */
static int rscps_sc_ctnl_pt_cfm_handler(kernel_msg_id_t const msgid,
                                          ...
                                          kernel_task_id_t const src_id)
{
    ...
    if (kernel_state_get(dest_id) == RSCPS_BUSY)
    {
        do
        {
    ...
    if (rscps_env != NULL)
    {
        ...
        if (param->handle == (RSCPS_HANDLE(RSCS_IDX_RSC_MEAS_NTF_CFG)))
        {
            ...
            if (ntf_cfg <= PRF_CLI_START_NTF)
            {
                ...
                struct rscps_cfg_ntfind_ind *ind = KERNEL_MSG_ALLOC(RSCPS_CFG_NTFIND_IND,
                        prf_dst_task_get(&rscps_env->prf_env, conidx),
                        prf_src_task_get(&rscps_env->prf_env, conidx),
            ...
            if (RSCPS_IS_FEATURE_SUPPORTED(rscps_env->prf_cfg, RSCPS_SC_CTNL_PT_MASK))
            {
                ...
                else if (param->handle == (RSCPS_HANDLE(RSCS_IDX_SC_CTNL_PT_VAL)))
                {
                    ...
                    do
                    {
                        ...
                        struct rscps_sc_ctnl_pt_req_ind *req_ind = KERNEL_MSG_ALLOC(RSCPS_SC_CTNL_PT_REQ_IND,
                                prf_dst_task_get(&rscps_env->prf_env, conidx),
                                prf_src_task_get(&rscps_env->prf_env, conidx),
...
const struct kernel_msg_handler rscps_default_state[] =
{
    ...
    {RSCPS_NTF_RSC_MEAS_REQ,        (kernel_msg_func_t) rscps_ntf_rsc_meas_req_handler},
    {RSCPS_SC_CTNL_PT_CFM,          (kernel_msg_func_t) rscps_sc_ctnl_pt_cfm_handler},
...
const struct kernel_state_handler rscps_default_handler = KERNEL_STATE_HANDLER(rscps_default_state);
