 ...
 ****************************************************************************************
 */
static int anps_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if(kernel_state_get(dest_id) == ANPS_IDLE)
    {
     ...
     struct anps_enable_rsp *cmp_evt = KERNEL_MSG_ALLOC(ANPS_ENABLE_RSP, src_id, dest_id, anps_enable_rsp);
 ...
 ****************************************************************************************
 */
static int anps_ntf_alert_cmd_handler(kernel_msg_id_t const msgid,
                                      ...
                                      kernel_task_id_t const src_id)
{
    ...
    if(state == ANPS_IDLE)
    {
        ...
        do
        {
            ...
            // Go to Busy state
            kernel_state_set(dest_id, ANPS_BUSY);
        } while(0);
    }
    ...
    if(kernel_state_get(dest_id) == ANPS_IDLE)
    {
        ...
        switch(att_idx)
        {
            case ANS_IDX_SUPP_NEW_ALERT_CAT_VAL:
            {
                ...
                common_write16p(value, anps_env->supp_new_alert_cat);
            } break;
            ...
            case ANS_IDX_NEW_ALERT_CFG:
            {
                // Fill data
                value_size = sizeof(uint16_t);
                common_write16p(value, ANPS_IS_ALERT_ENABLED(conidx, anps_env, ANP_NEW_ALERT));
            } break;
            ...
            case ANS_IDX_SUPP_UNREAD_ALERT_CAT_VAL:
            {
                // Fill data
                value_size = sizeof(uint16_t);
                common_write16p(value, anps_env->supp_unread_alert_cat);
            } break;
            ...
            case ANS_IDX_UNREAD_ALERT_STATUS_CFG:
            {
                // Fill data
                value_size = sizeof(uint16_t);
                common_write16p(value, ANPS_IS_ALERT_ENABLED(conidx, anps_env, ANP_UNREAD_ALERT));
            } break;
...
const struct kernel_msg_handler anps_default_state[] =
{
...
const struct kernel_state_handler anps_default_handler = KERNEL_STATE_HANDLER(anps_default_state);
