 ...
 ****************************************************************************************
 */
static int scpps_enable_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if(state == SCPPS_IDLE)
    {
        ...
        struct scpps_enable_rsp * rsp = KERNEL_MSG_ALLOC(SCPPS_ENABLE_RSP, src_id,
 ...
 ****************************************************************************************
 */
static int scpps_scan_refresh_send_req_handler(kernel_msg_id_t const msgid,
                                               ...
                                               kernel_task_id_t const src_id)
{
    ...
    if(state == SCPPS_IDLE)
    {
                ...
                && ((scpps_env->ntf_cfg[param->conidx] & PRF_CLI_START_NTF) != 0))
        {
            ...
            struct gattc_send_evt_cmd *refresh_ntf = KERNEL_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
            ...
            kernel_msg_send(refresh_ntf);
            // wait for event to be taken in account in Lower layers.
            kernel_state_set(dest_id, SCPPS_BUSY);
        }
                                      ...
                                      kernel_task_id_t const src_id)
{
    ...
    if (param->handle == scpps_env->start_hdl + SCPS_IDX_SCAN_INTV_WD_VAL)
    {
        ...
        // Extract value
        le_scan_intv = common_read16p(&param->value[0]);
        le_scan_window = common_read16p(&param->value[sizeof(uint16_t)]);
...

        {
            ...
            struct scpps_scan_intv_wd_ind * ind = KERNEL_MSG_ALLOC(SCPPS_SCAN_INTV_WD_IND,
    ...
    else if (param->handle == scpps_env->start_hdl + SCPS_IDX_SCAN_REFRESH_NTF_CFG)
    {
        ...
        if ((scpps_env->features & SCPPS_SCAN_REFRESH_SUP) != 0)
        {
            ...
            if ((ntf_cfg == PRF_CLI_STOP_NTFIND) || (ntf_cfg == PRF_CLI_START_NTF))
            {
                ...
                struct scpps_scan_refresh_ntf_cfg_ind * ind = KERNEL_MSG_ALLOC(SCPPS_SCAN_REFRESH_NTF_CFG_IND,
                                                                        prf_dst_task_get(&(scpps_env->prf_env), conidx),
                                      ...
                                      kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
        ...
        && ((scpps_env->features & SCPPS_SCAN_REFRESH_SUP) != 0))
    {
        ...
        common_write16p(&cfm->value[0], ntf_cfg);
                                 ...
                                 kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    ...
    if((state == SCPPS_BUSY) && (param->operation == GATTC_NOTIFY))
    {
        ...
        scpps_scan_refresh_rsp_send(PRF_ENV_GET(SCPPS, scpps), KERNEL_IDX_GET(src_id), param->status);
        kernel_state_set(dest_id, SCPPS_IDLE);
    }
...
const struct kernel_msg_handler scpps_default_state[] =
{
    {SCPPS_ENABLE_REQ,              (kernel_msg_func_t) scpps_enable_req_handler},
    {SCPPS_SCAN_REFRESH_SEND_REQ,   (kernel_msg_func_t) scpps_scan_refresh_send_req_handler},
...
const struct kernel_state_handler scpps_default_handler = KERNEL_STATE_HANDLER(scpps_default_state);
