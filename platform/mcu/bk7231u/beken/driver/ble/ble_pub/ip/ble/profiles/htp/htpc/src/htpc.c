                            ...
                            uint8_t sec_lvl,  void* params)
{
    ...
    struct htpc_env_tag* htpc_env =
            (struct htpc_env_tag* ) kernel_malloc(sizeof(struct htpc_env_tag), KERNEL_MEM_ATT_DB);
    ...
    for(idx = 0; idx < HTPC_IDX_MAX ; idx++)
    {
        ...
        kernel_state_set(KERNEL_BUILD_ID(env->task, idx), HTPC_FREE);
    }
...
static void htpc_destroy(struct prf_task_env* env)
{
    ...
    for(idx = 0; idx < HTPC_IDX_MAX ; idx++)
    {
        if(htpc_env->env[idx] != NULL)
        {
            ...
            {
                kernel_msg_free(htpc_env->env[idx]->operation);
            }

            kernel_free(htpc_env->env[idx]);
        }
    }

    // free profile environment variables
    env->env = NULL;
    kernel_free(htpc_env);
}
...
static void htpc_create(struct prf_task_env* env, uint8_t conidx)
{
    ...
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), HTPC_IDLE);
}
...
static void htpc_cleanup(struct prf_task_env* env, uint8_t conidx, uint8_t reason)
{
    ...
    if(htpc_env->env[conidx] != NULL)
    {
        ...
        {
            kernel_msg_free(htpc_env->env[conidx]->operation);
        }

        kernel_free(htpc_env->env[conidx]);
        htpc_env->env[conidx] = NULL;
    }

    /* Put HTP Client in Free state */
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), HTPC_FREE);
}
                      ...
                      uint8_t length, uint8_t flag_stable_meas, uint8_t conidx)
{
    ...
    if (length >= HTPC_PACKED_TEMP_MIN_LEN)
    {
        ...
        struct htpc_temp_ind *ind = KERNEL_MSG_ALLOC(HTPC_TEMP_IND,
        ...
        ind->temp_meas.temp = common_read32p(packed_temp + cursor);
        cursor += 4;
...
void htpc_enable_rsp_send(struct htpc_env_tag *htpc_env, uint8_t conidx, uint8_t status)
{
    ...
    struct htpc_enable_rsp *cfm = KERNEL_MSG_ALLOC(HTPC_ENABLE_RSP,
                                                prf_dst_task_get(&(htpc_env->prf_env), conidx),
                                                prf_src_task_get(&(htpc_env->prf_env), conidx),
                                                htpc_enable_rsp);
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        // Go to IDLE state
        kernel_state_set(prf_src_task_get(&(htpc_env->prf_env), conidx), HTPC_IDLE);
