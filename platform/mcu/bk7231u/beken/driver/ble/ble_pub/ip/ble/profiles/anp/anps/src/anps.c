...
static uint8_t anps_init(struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl, struct anps_db_cfg* params)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        struct anps_env_tag* anps_env =
                (struct anps_env_tag* ) kernel_malloc(sizeof(struct anps_env_tag), KERNEL_MEM_ATT_DB);
        ...
        for(uint8_t idx = 0; idx < BLE_CONNECTION_MAX ; idx++)
        {
            ...
            kernel_state_set(KERNEL_BUILD_ID(env->task, idx), ANPS_FREE);
        }
    }
...
static void anps_destroy(struct prf_task_env* env)
{
    ...
    for(idx = 0; idx < BLE_CONNECTION_MAX ; idx++)
    {
        ...
        {
            kernel_free(anps_env->env[idx]);
        }
    }

    // free profile environment variables
    env->env = NULL;
    kernel_free(anps_env);
}
...
static void anps_create(struct prf_task_env* env, uint8_t conidx)
{
    ...
    anps_env->env[conidx] = (struct anps_cnx_env*)
            kernel_malloc(sizeof(struct anps_cnx_env), KERNEL_MEM_ATT_DB);

    memset(anps_env->env[conidx], 0, sizeof(struct anps_cnx_env));
    /* Put ANS in idle state */
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), ANPS_IDLE);
...
static void anps_cleanup(struct prf_task_env* env, uint8_t conidx, uint8_t reason)
{
    ...
    {
        kernel_free(anps_env->env[conidx]);
        anps_env->env[conidx] = NULL;
    }

    /* Put ANS in disabled state */
    kernel_state_set(KERNEL_BUILD_ID(env->task, conidx), ANPS_FREE);
}
...
void anps_send_cmp_evt(kernel_task_id_t src_id, kernel_task_id_t dest_id,
                       uint8_t operation, uint8_t status)
{
    // Come back to the Connected state if the state was busy.
    if (kernel_state_get(src_id) == ANPS_BUSY)
    {
        kernel_state_set(src_id, ANPS_IDLE);
    }

    // Send the message to the application
    struct anps_cmp_evt *evt = KERNEL_MSG_ALLOC(ANPS_CMP_EVT,
                                            dest_id, src_id,
                                            anps_cmp_evt);
...
{
    // Send the message to the application
    struct anps_ntf_status_update_ind *ind = KERNEL_MSG_ALLOC(ANPS_NTF_STATUS_UPDATE_IND,
                                     ...
                                     uint8_t category_id)
{
    ...
    // Send the message to the application
    struct anps_ntf_immediate_req_ind *ind = KERNEL_MSG_ALLOC(ANPS_NTF_IMMEDIATE_REQ_IND,
            prf_dst_task_get(&idx_env->prf_env, conidx),
            prf_src_task_get(&idx_env->prf_env, conidx),
