...
static uint8_t rscps_init(struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl, struct rscps_db_cfg* param)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        struct rscps_env_tag* rscps_env =
                (struct rscps_env_tag* ) kernel_malloc(sizeof(struct rscps_env_tag), KERNEL_MEM_ATT_DB);
        ...
        kernel_state_set(env->task, RSCPS_IDLE);
    }
...
static void rscps_destroy(struct prf_task_env* env)
{
    ...
    {
        kernel_free(rscps_env->ntf);
    }

    // free profile environment variables
    env->env = NULL;
    kernel_free(rscps_env);
}
...
void rscps_send_cmp_evt(uint8_t conidx, uint8_t src_id, uint8_t dest_id, uint8_t operation, uint8_t status)
{
    ...
    if (kernel_state_get(src_id) == RSCPS_BUSY)
    {
        kernel_state_set(src_id, RSCPS_IDLE);
    }
    ...
    // Send the message
    struct rscps_cmp_evt *evt = KERNEL_MSG_ALLOC(RSCPS_CMP_EVT,
                                             dest_id, src_id,
                                             rscps_cmp_evt);
...
void rscps_exe_operation(void)
{
    ...
    while(rscps_env->ntf->cursor < BLE_CONNECTION_MAX)
    {
        ...
        if(RSCPS_IS_NTFIND_ENABLED(rscps_env->ntf->cursor, RSCP_PRF_CFG_FLAG_RSC_MEAS_NTF))
        {
            ...
            struct gattc_send_evt_cmd *meas_val = KERNEL_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
                    KERNEL_BUILD_ID(TASK_GATTC, rscps_env->ntf->cursor),
    ...
    if(finished)
    {
        ...
        struct rscps_ntf_rsc_meas_rsp *rsp = KERNEL_MSG_ALLOC(RSCPS_NTF_RSC_MEAS_RSP,
        ...
        kernel_state_set(prf_src_task_get(&(rscps_env->prf_env), 0), RSCPS_IDLE);
    }
}
