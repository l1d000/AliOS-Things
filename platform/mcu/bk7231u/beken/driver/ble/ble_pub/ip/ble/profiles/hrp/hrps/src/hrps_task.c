 ...
 ****************************************************************************************
 */
static int hrps_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
     ...
     if(kernel_state_get(dest_id) == HRPS_IDLE)
     {
     ...
     struct hrps_enable_rsp *cmp_evt = KERNEL_MSG_ALLOC(HRPS_ENABLE_RSP, src_id, dest_id, hrps_enable_rsp);
 ...
 ****************************************************************************************
 */
static int hrps_meas_send_req_handler(kernel_msg_id_t const msgid,
                                      ...
                                      kernel_task_id_t const src_id)
{
    ...
    if (kernel_state_get(dest_id) == HRPS_IDLE)
    {
        if(param->meas_val.nb_rr_interval <= HRS_MAX_RR_INTERVAL)
        {
            ...
            hrps_env->operation = (struct hrps_op*) kernel_malloc(sizeof(struct hrps_op), KERNEL_MEM_KERNEL_MSG);
            ...
            // Go to busy state
            kernel_state_set(dest_id, HRPS_BUSY);
        }
        else
        {
            // Send confirmation
    ...
    if (hrps_env != NULL)
    {
        ...
        if (param->handle == (hrps_env->shdl + HRS_IDX_HR_MEAS_NTF_CFG))
        {
            ...
            if (status == GAP_ERR_NO_ERROR)
            {
                ...
                struct hrps_cfg_indntf_ind * ind = KERNEL_MSG_ALLOC(HRPS_CFG_INDNTF_IND,
        ...
        else
        {
            if (HRPS_IS_SUPPORTED(hrps_env->features, HRPS_ENGY_EXP_FEAT_SUP))
            {
                ...
                {
                    //inform APP of configuration change
                    struct hrps_energy_exp_reset_ind * ind = KERNEL_MSG_ALLOC(HRPS_ENERGY_EXP_RESET_IND,
                            prf_dst_task_get(&hrps_env->prf_env, conidx),
                            prf_src_task_get(&hrps_env->prf_env, conidx),
...
{
    if((kernel_state_get(dest_id) == HRPS_BUSY) && (param->operation == GATTC_NOTIFY))
    {
    ...
    if(kernel_state_get(dest_id) == HRPS_IDLE)
    {
        ...
        switch(att_idx)
        {
            case HRS_IDX_HR_MEAS_NTF_CFG:
            {
                ...
                common_write16p(value, hrps_env->hr_meas_ntf[conidx] & ~HRP_PRF_CFG_PERFORMED_OK);
            } break;
...
const struct kernel_msg_handler hrps_default_state[] =
{
    ...
    {HRPS_MEAS_SEND_REQ,     (kernel_msg_func_t) hrps_meas_send_req_handler},
...
const struct kernel_state_handler hrps_default_handler = KERNEL_STATE_HANDLER(hrps_default_state);
