 ...
 ****************************************************************************************
 */
static int tipc_enable_req_handler(kernel_msg_id_t const msgid,
                                   ...
                                   kernel_task_id_t const src_id)
{
    ...
    if ((state == TIPC_IDLE) && (tipc_env->env[conidx] == NULL))
    {
        ...
        tipc_env->env[conidx] = (struct tipc_cnx_env*) kernel_malloc(sizeof(struct tipc_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, TIPC_DISCOVERING);
        }
 ...
 ****************************************************************************************
 */
static int tipc_rd_char_req_handler(kernel_msg_id_t const msgid,
                                        ...
                                        kernel_task_id_t const src_id)
{
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        ...
        struct tipc_rd_char_rsp *ind = KERNEL_MSG_ALLOC(
                TIPC_RD_CHAR_RSP,
 ...
 ****************************************************************************************
 */
static int tipc_ct_ntf_cfg_req_handler(kernel_msg_id_t const msgid,
                                       ...
                                       kernel_task_id_t const src_id)
{
    ...
    if((tipc_env->env[conidx] != NULL) && (kernel_state_get(dest_id) == TIPC_IDLE))
    {
        //Only NTF
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        struct tipc_ct_ntf_cfg_rsp *ind = KERNEL_MSG_ALLOC(
                TIPC_CT_NTF_CFG_RSP,
                prf_dst_task_get(&(tipc_env->prf_env), conidx),
                prf_src_task_get(&(tipc_env->prf_env), conidx),
                tipc_ct_ntf_cfg_rsp);
 ...
 ****************************************************************************************
 */
static int tipc_wr_time_upd_ctnl_pt_req_handler(kernel_msg_id_t const msgid,
                                                ...
                                                kernel_task_id_t const src_id)
{
    ...
    if((kernel_state_get(dest_id) == TIPC_IDLE) && (tipc_env->env[conidx] != NULL))
    {
    ...
    {
        //send app error indication
        struct tipc_wr_time_upd_ctnl_pt_rsp *ind = KERNEL_MSG_ALLOC(
                                ...
                                kernel_task_id_t const src_id)
{
    ...
    else if(state == TIPC_IDLE)
    {
        switch(param->operation)
        {
            case GATTC_WRITE:
            {
                struct tipc_ct_ntf_cfg_rsp *wr_cfm = KERNEL_MSG_ALLOC(
                        TIPC_CT_NTF_CFG_RSP,
                        prf_dst_task_get(&(tipc_env->prf_env), conidx),
                        prf_src_task_get(&(tipc_env->prf_env), conidx),
                        tipc_ct_ntf_cfg_rsp);
            ...
            {
                struct tipc_wr_time_upd_ctnl_pt_rsp *wr_cfm = KERNEL_MSG_ALLOC(
                        TIPC_WR_TIME_UPD_CTNL_PT_RSP,
                        prf_dst_task_get(&(tipc_env->prf_env), conidx),
                        prf_src_task_get(&(tipc_env->prf_env), conidx),
                        tipc_wr_time_upd_ctnl_pt_rsp);
            ...
            case GATTC_READ:
            {
                ...
                {
                    struct tipc_rd_char_rsp *rd_cfm = KERNEL_MSG_ALLOC(
                            TIPC_RD_CHAR_RSP,
                            prf_dst_task_get(&(tipc_env->prf_env), conidx),
                            prf_src_task_get(&(tipc_env->prf_env), conidx),
                            tipc_rd_char_rsp);
                                        ...
                                        kernel_task_id_t const src_id)
{
    ...
    if(tipc_env->env[conidx] != NULL)
    {
        if(param->handle == tipc_env->env[conidx]->cts.chars[TIPC_CHAR_CTS_CURR_TIME].val_hdl)
        {
            ...
            struct tipc_ct_ind * ind = KERNEL_MSG_ALLOC(
                    TIPC_CT_IND,
                    prf_dst_task_get(&(tipc_env->prf_env), conidx),
                    prf_src_task_get(&(tipc_env->prf_env), conidx),
                    tipc_ct_ind);
...
const struct kernel_msg_handler tipc_default_state[] =
{
    ...
    {TIPC_CT_NTF_CFG_REQ,           (kernel_msg_func_t)tipc_ct_ntf_cfg_req_handler},
    {GATTC_EVENT_IND,               (kernel_msg_func_t)gattc_event_ind_handler},
    {TIPC_RD_CHAR_REQ,              (kernel_msg_func_t)tipc_rd_char_req_handler},
    {GATTC_READ_IND,                (kernel_msg_func_t)gattc_read_ind_handler},
    {TIPC_WR_TIME_UPD_CTNL_PT_REQ,  (kernel_msg_func_t)tipc_wr_time_upd_ctnl_pt_req_handler},
};

// Specifies the message handlers that are common to all states.
const struct kernel_state_handler tipc_default_handler = KERNEL_STATE_HANDLER(tipc_default_state);
