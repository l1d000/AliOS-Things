 ...
 ****************************************************************************************
 */
static int lanc_enable_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if((state == LANC_IDLE) && (lanc_env->env[conidx] == NULL))
    {
        ...
        lanc_env->env[conidx] = (struct lanc_cnx_env*) kernel_malloc(sizeof(struct lanc_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, LANC_DISCOVERING);
        }
 ...
 ****************************************************************************************
 */
static int lanc_read_cmd_handler(kernel_msg_id_t const msgid,
    ...
    if (state == LANC_IDLE)
    {
        ...
        else
        {
            ...
            if (hdl != ATT_INVALID_SEARCH_HANDLE)
            {
                ...
                kernel_state_set(dest_id, LANC_BUSY);
 ...
 ****************************************************************************************
 */
static int lanc_cfg_ntfind_cmd_handler(kernel_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int lanc_ln_ctnl_pt_cfg_req_handler(kernel_msg_id_t const msgid,
                                         ...
                                         kernel_task_id_t const src_id)
{
    ...
    if (lanc_env != NULL)
    {
        ...
        do
        {
            ...
            ASSERT_ERR(kernel_state_get(dest_id) > LANC_FREE);
            ...
            if (kernel_state_get(dest_id) != LANC_IDLE)
            {
                // Another procedure is pending, keep the command for later
                msg_status = KERNEL_MSG_SAVED;
 ...
 ****************************************************************************************
 */
static int lanc_timeout_timer_ind_handler(kernel_msg_id_t const msgid,
                                           void const *param,
                                           kernel_task_id_t const dest_id,
                                           kernel_task_id_t const src_id)
{
                ...
                case GATTC_WRITE_NO_RESPONSE:
                {
                    ...
                    else if (operation == LANC_LN_CTNL_PT_CFG_WR_OP_CODE)
                    {
                        if (param->status == GAP_ERR_NO_ERROR)
                        {
                            ...
                            kernel_timer_set(LANC_TIMEOUT_TIMER_IND, dest_id, ATT_TRANS_RTX);
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if (state == LANC_BUSY)
    {
        ...
        switch (((struct lanc_read_cmd *)lanc_env->env[conidx]->operation)->read_code)
        {
            ...
            {
                ind->value.LN_feat = common_read32p(&param->value[0]);
...
//                ind->value.LN_feat &= LANP_FEAT_ALL_SUPP;
            ...
            {
                ind->value.ntf_cfg = common_read16p(&param->value[0]);
            } break;
            ...
            case (GATTC_INDICATE):
            {
                ...
                if (lanc_env->env[conidx]->operation != NULL)
                {
                    if (((struct lanc_cmd *)lanc_env->env[conidx]->operation)->operation == LANC_LN_CTNL_PT_CFG_IND_OP_CODE)
                    {
                        ...
                        kernel_timer_clear(LANC_TIMEOUT_TIMER_IND, dest_id);
...
const struct kernel_msg_handler lanc_default_state[] =
{
    ...
    {LANC_LN_CTNL_PT_CFG_REQ,       (kernel_msg_func_t)lanc_ln_ctnl_pt_cfg_req_handler},
    {LANC_TIMEOUT_TIMER_IND,        (kernel_msg_func_t)lanc_timeout_timer_ind_handler},
...
const struct kernel_state_handler lanc_default_handler = KERNEL_STATE_HANDLER(lanc_default_state);
