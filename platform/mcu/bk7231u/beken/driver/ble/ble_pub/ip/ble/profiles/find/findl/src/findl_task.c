 ...
 ****************************************************************************************
 */
static int findl_enable_req_handler(kernel_msg_id_t const msgid,
                                    ...
                                    kernel_task_id_t const src_id)
{
    ...
    if((state == FINDL_IDLE) && (findl_env->env[conidx] == NULL))
    {
        ...
        findl_env->env[conidx] = (struct findl_cnx_env*) kernel_malloc(sizeof(struct findl_cnx_env),KERNEL_MEM_ATT_DB);
        ...
        if(param->con_type == PRF_CON_DISCOVERY)
        {
            ...
            kernel_state_set(dest_id, FINDL_DISCOVERING);
        }
 ...
 ****************************************************************************************
 */
static int findl_set_alert_req_handler(kernel_msg_id_t const msgid,
    ...
    if(state == FINDL_IDLE)
    {
                ...
                (param->alert_lvl == FINDL_ALERT_HIGH))
        {
            if(findl_env->env[conidx]->ias.alert_lvl_char.char_hdl != 0x0000)
            {
                ...
                kernel_state_set(dest_id, FINDL_BUSY);
            }
            else
            {
        ...
        struct findl_set_alert_rsp *rsp = KERNEL_MSG_ALLOC(FINDL_SET_ALERT_RSP,
                                ...
                                kernel_task_id_t const src_id)
{
    ...
    {
        kernel_state_set(dest_id, FINDL_IDLE);
    }
...
const struct kernel_msg_handler findl_default_state[] =
{
    ...
    {FINDL_SET_ALERT_REQ,          (kernel_msg_func_t)findl_set_alert_req_handler},
};

/// Specifies the message handlers that are common to all states.
const struct kernel_state_handler findl_default_handler = KERNEL_STATE_HANDLER(findl_default_state);
