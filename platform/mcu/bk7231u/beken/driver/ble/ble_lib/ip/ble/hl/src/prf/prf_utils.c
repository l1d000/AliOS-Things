...
void prf_pack_char_pres_fmt(uint8_t *packed_val, const struct prf_char_pres_fmt* char_pres_fmt)
{
    ...
    *(packed_val + 1) =         char_pres_fmt->exponent;
    common_write16p(packed_val + 2, char_pres_fmt->unit);
    *(packed_val + 4) =         char_pres_fmt->name_space;
    common_write16p(packed_val + 5, char_pres_fmt->description);
}
...
void prf_unpack_char_pres_fmt(const uint8_t *packed_val, struct prf_char_pres_fmt* char_pres_fmt)
{
    ...
    char_pres_fmt->unit         = common_read16p(packed_val + 2);
    char_pres_fmt->name_space    = *(packed_val + 4);
    char_pres_fmt->description  = common_read16p(packed_val + 5);
}
...
{
    struct gattc_read_cmd * req  = KERNEL_MSG_ALLOC(GATTC_READ_CMD, KERNEL_BUILD_ID(TASK_GATTC, conidx),
...
void prf_register_atthdl2gatt(prf_env_t *prf_env, uint8_t conidx, struct prf_svc *svc)
{
    if(svc->shdl != ATT_INVALID_HANDLE)
    {
        ...
        struct gattc_reg_to_peer_evt_cmd * reg = KERNEL_MSG_ALLOC(GATTC_REG_TO_PEER_EVT_CMD,
                KERNEL_BUILD_ID(TASK_GATTC, conidx), prf_src_task_get(prf_env, conidx),
...
void prf_disc_svc_send(prf_env_t *prf_env, uint8_t conidx, uint16_t uuid)
{
    ...
    common_write16p(&(svc_req->uuid[0]), uuid);
                    ...
                    uint16_t handle, uint8_t* value, uint16_t length, uint8_t operation,uint8_t seq_num)
{
    ...
    {
        struct gattc_write_cmd *wr_char = KERNEL_MSG_ALLOC_DYN(GATTC_WRITE_CMD,
                KERNEL_BUILD_ID(TASK_GATTC, conidx), prf_src_task_get(prf_env, conidx),
...
void prf_gatt_write_ntf_ind(prf_env_t *prf_env, uint8_t conidx, uint16_t handle, uint16_t ntf_ind_cfg,uint8_t seq_num)
{
    ...
    common_write16p((&value[0]), ntf_ind_cfg);
                             ...
                             uint8_t status, struct gattc_read_ind const* read_ind)
{
    ...
    // prepare response
    struct prf_att_info *rsp = KERNEL_MSG_ALLOC_DYN(msg_id,
...
{
    common_write16p(packed_date, date_time->year);
...
{
    date_time->year = common_read16p(&(packed_date[0]));
