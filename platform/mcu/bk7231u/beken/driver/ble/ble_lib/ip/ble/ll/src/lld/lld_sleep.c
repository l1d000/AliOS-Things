...
/* 将 lld_sleep_env  变量放到ROM 区，把lld_sleep.c 函数放到flash中便于维护  
将	static struct lld_sleep_env_tag lld_sleep_env;
修改为：struct lld_sleep_env_tag lld_sleep_env;

看哪些lld_sleep 中的函数被ROM中调用了的，则需要通过指针方式传递。
1、lld_sleep_init() 会被lld.c中 用到，用
lld_call_flash_list.lld_lld_sleep_init();替代
