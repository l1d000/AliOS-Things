...
uint32_t vif_get_beacon_loss_count(void)
{
	uint32_t count = 0;
    struct vif_info_tag *vif_entry =
        (struct vif_info_tag *)co_list_pick(&vif_mgmt_env.used_list);

	if(vif_entry && (VIF_STA == vif_entry->type))
	{
		count = vif_entry->u.sta.beacon_loss_cnt;
	}

	//os_printf("bloss_cnt:%d\r\n", count);
	return count;
}

void vif_clear_beacon_loss_count(void)
{
    struct vif_info_tag *vif_entry =
        (struct vif_info_tag *)co_list_pick(&vif_mgmt_env.used_list);
