...
#define CFG_ABS_TIMEOUT_BEACON_LOSS_PATCH    1
...
static UINT32 sta_intermittent_beacon_loss_cnt = 0;
                            ...
                            struct sta_info_tag *p_sta_entry, uint32_t tim)
{
...
#if CFG_USE_STA_PS
    {
        power_save_set_dtim_period(co_read8p(tim + MAC_TIM_PERIOD_OFT));
        power_save_set_dtim_count(co_read8p(tim + MAC_TIM_CNT_OFT));
    }
#endif
...
#endif //(RW_UMESH_EN)
    {
        ...
        #ifdef CFG_ABS_TIMEOUT_BEACON_LOSS_PATCH
        if (next_tbtt != vif_entry->tbtt_timer.time && hal_machw_time() != 0xdead5555)
        {           
            //ASSERT(hal_machw_time() != 0xdead5555);
        
            if(((int32_t)(next_tbtt -hal_machw_time() - (uint32_t)8000)) > 0
                && (((int32_t)(next_tbtt -hal_machw_time() - (uint32_t)5000000)) < 0
#if CFG_USE_STA_PS
                || (power_save_if_sleep_at_first())
#endif
                    ))
		    {
            ...
            else if(((int32_t)(next_tbtt -hal_machw_time() - (uint32_t)5000000)) > 0)
            {
                //os_printf("tbtt %d  %x %x\r\n",__LINE__,next_tbtt,vif_entry->tbtt_timer.time);
            }
            else
            {
                //os_printf("tbtt %d %x\r\n",__LINE__,vif_entry->tbtt_timer.time);
            }
        }
        #else		
        ...
        #endif // CFG_ABS_TIMEOUT_BEACON_LOSS_PATCH
    }
}
#endif
...
static void mm_ap_probe_cfm(void *env, uint32_t status)
{
    ...
    if (status & FRAME_SUCCESSFUL_TX_BIT)
    {
        ...
        //os_printf("reset the beacon loss counter\r\n");
    }
    else
    {
        // Otherwise, we consider that the connection is lost
        mm_send_connection_loss_ind(vif_entry);
        os_printf("mm_ap_probe_cfm: connection lost\r\n");
    }
}


static bool mm_check_tx_status(struct vif_info_tag *p_vif_entry, uint32_t status,
                               cfm_func_ptr cfm)
{
...
static void mm_ap_probe_null_cfm(void *env, uint32_t status)
{
    struct vif_info_tag *vif_entry = (struct vif_info_tag *) env;

    // Check if an error has occurred during the NULL frame transmission
    if (!mm_check_tx_status((struct vif_info_tag *)env, status, mm_ap_probe_null_cfm))
    {
        return;
    }
...
static uint32_t mm_compute_beacon_ie(struct bcn_frame *bcn, uint16_t len)
{
    uint32_t crc;
    uint32_t bcn_addr = CPU2HW(bcn);
    uint32_t addr = bcn_addr + MAC_BEACON_VARIABLE_PART_OFT;

    crc = 0; //capabilities:short slot time may be change.
    ...
    while (len >= MAC_INFOELT_INFO_OFT)
    {
        ...
        {
        case MAC_ELTID_TIM:
        case MAC_ELTID_BSS_LOAD:
        case MAC_ELTID_IBSS:
            // Skip this element
            break;
        default:           
            // Compute the CRC on this element 
            if(ie_id == MAC_ELTID_RATES 
                || ie_id == MAC_ELTID_DS
                || ie_id == MAC_ELTID_RSN_IEEE
                || ie_id == MAC_ELTID_HT_CAPA
                )
                {
                    if(ie_id == MAC_ELTID_HT_CAPA)
                    {
                        crc = co_crc32(addr, 2, crc);
                    }
                    else
                    {
                        crc = co_crc32(addr, ie_len, crc);
                     ...
                     struct sta_info_tag *p_sta_entry, uint32_t *tim)
{
    ...
    uint32_t ie_prev = vif_entry->u.sta.mon_ie_crc;

    if(vif_entry->u.sta.beacon_loss_cnt > 1)
        {
        sta_intermittent_beacon_loss_cnt += vif_entry->u.sta.beacon_loss_cnt;
    }
    ...
    if (txl_cntrl_tx_check(vif_entry) &&
        (ke_time_past(vif_entry->u.sta.mon_last_tx + MM_KEEP_ALIVE_PERIOD)) &&
        (txl_frame_send_null_frame(vif_entry->u.sta.ap_id, NULL, NULL) == CO_OK))
    {
        sta_intermittent_beacon_loss_cnt = 0;

        // Update the keep-alive time
        if(ke_time() != 0xdead5555)
        {
            vif_entry->u.sta.mon_last_tx = ke_time();
        }
        else
        {
            vif_entry->u.sta.mon_last_tx += MM_KEEP_ALIVE_PERIOD;
        }
    }
    ...
    vif_entry->u.sta.mon_ie_crc = mm_compute_beacon_ie(bcn, len);
...
#if NX_POWERSAVE
#if CFG_USE_STA_PS
    if(power_save_ps_mode_get() == PS_DTIM_PS_MODE)
    {
        if(ie_prev && (ie_prev != vif_entry->u.sta.mon_ie_crc)
            && mhdr_get_station_status() == MSG_GOT_IP)
        {
            os_printf("bcn crc change %x %x\r\n",ie_prev,vif_entry->u.sta.mon_ie_crc);
            mm_send_connection_loss_ind(vif_entry);
        }
    }
#endif
...
void mm_reset(void)
{
    ...
    case MM_GOING_TO_IDLE:
        // MM is going to idle, the timer would be cleared if it was not triggered
        if (!(ke_evt_get() & KE_EVT_HW_IDLE_BIT))
        {
            // hal_machw_idle_irq_handler() isn't invoked and won't be invoked, need set to IDLE
            os_printf("mm_reset MM_GOING_TO_IDLE->MM_IDLE\n");
...
void mm_sta_tbtt(void *env)
{
    ...
    do
    {
		...
		if(vif_entry->u.sta.ap_id < STA_MAX)
		{
            ...
            if(hal_machw_time() != 0xdead5555)
            {
                if(((int32_t)(vif_entry->tbtt_timer.time - hal_machw_time() - (uint32_t)5000) < 0) 
                    && ((int32_t)(tbtt_time -hal_machw_time() - (uint32_t)8000)) > 0
                    && ((int32_t)(tbtt_time -hal_machw_time() - (uint32_t)5000000)) < 0)
    		    {
                    
    			   // Program next TBTT based on previous TBTT
    			    mm_timer_set(&vif_entry->tbtt_timer, tbtt_time);
    		    }
                else if(((int32_t)(tbtt_time -hal_machw_time() -(uint32_t)5000000)) > 0)
                {
                    //os_printf("tbtt %d %x %x\r\n",__LINE__,tbtt_time,vif_entry->tbtt_timer.time);
                }
                else
                {
                    //os_printf("tbtt %d %x\r\n",__LINE__,vif_entry->tbtt_timer.time);
            ...
            #else
			if(((int32_t)(tbtt_time -hal_machw_time() -(uint32_t)5000000)) > 0)
            {
            	//os_printf("tbtt_timeL:%x:%x\r\n", tbtt_time, hal_machw_time());
            }
        ...
        if (vif_entry->u.sta.beacon_loss_cnt > MM_BEACON_LOSS_THD 
            || sta_intermittent_beacon_loss_cnt > 3 * MM_BEACON_LOSS_THD)
        {
            //os_printf("checking_beacon_loss %d %d\r\n",vif_entry->u.sta.beacon_loss_cnt,sta_intermittent_beacon_loss_cnt);
        ...
        else if ((MM_BEACON_LOSS_THD > 15) 
			&& (vif_entry->u.sta.beacon_loss_cnt > (MM_BEACON_LOSS_THD - 15))
			&& (vif_entry->u.sta.beacon_loss_cnt < (MM_BEACON_LOSS_THD - 13)))
        {
            //os_printf("try_to_save_your_life\r\n");
            hal_assert_rec();
        }
#if (NX_CHNL_CTXT)
        else if (vif_entry->u.sta.beacon_loss_cnt > (MM_BEACON_LOSS_THD - 1)
            || sta_intermittent_beacon_loss_cnt > 3 * MM_BEACON_LOSS_THD - 1)
        {
...
void mm_hw_idle_evt(int dummy)
{
    ...
    mm_timer_clear(&mm_env.machw_timer);
