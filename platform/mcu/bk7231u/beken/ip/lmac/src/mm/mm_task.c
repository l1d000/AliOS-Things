                     ...
                     ke_task_id_t const src_id)
{
    ...
    ASSERT_EQ(ke_state_get(dest_id), MM_IDLE);
...
static int
mm_get_channel_req_handler(ke_msg_id_t const msgid,
                           ke_task_id_t const dest_id,
                           ke_task_id_t const src_id)
{
    struct mm_get_channel_cfm *cfm;
    struct phy_channel_info info;
	
    // Allocate confirmation message
    cfm = KE_MSG_ALLOC(MM_GET_CHANNEL_CFM, src_id, dest_id, mm_get_channel_cfm);
	
    phy_get_channel(&info, 0);
    cfm->center1_freq = info.info2;
                              ...
                              ke_task_id_t const src_id)
{
    ...
    if (vif_entry->type == VIF_STA)
    {
        ...
        #else //(NX_MULTI_ROLE)
		os_printf("------%s:%d %d\r\n",__FUNCTION,__LINE__,param->beacon_int);
                         ...
                         ke_task_id_t const src_id)
{
    ...
    os_printf("mm_set_power_req_handler :%x\r\n",param->power);
                         ...
                         ke_task_id_t const src_id)
{
    ...
    if (co_list_is_empty(&vif_mgmt_env.used_list))
    {
        ...
        // but, need to check if need to entry.
                       ...
                       ke_task_id_t const src_id)
{
    ...
    uint8_t ctrl_port_state = sta_entry->ctrl_port_state;
    struct mac_addr sta_mac;

    memcpy((void *)&sta_mac, (const void *)&sta_entry->mac_addr, sizeof(sta_entry->mac_addr));
    ...
    if (TASK_ME == src_id)
    {
        if ((ctrl_port_state == PORT_CLOSING) 
                || ((ctrl_port_state == PORT_OPEN) && (!vif_entry->default_key)))
        {
            struct apm_deassoc_ind *sta_deassoc = KE_MSG_ALLOC(APM_DEASSOC_IND, TASK_API,
                                             TASK_MM, apm_deassoc_ind);
            memcpy(sta_deassoc->mac, (const void *)&sta_mac, sizeof(sta_mac));
            ke_msg_send(sta_deassoc);
        }
        else
        {
            struct apm_assoc_failed_ind *sta_assoc_failed = KE_MSG_ALLOC(APM_ASSOC_FAILED_IND, TASK_API,
                                             TASK_MM, apm_assoc_failed_ind);
            memcpy(sta_assoc_failed->mac, (const void *)&sta_mac, sizeof(sta_mac));
            ke_msg_send(sta_assoc_failed);
        }
    }
    else if (TASK_SM == src_id)
    {
        if (ctrl_port_state == PORT_OPEN)
        {
            struct sm_deassoc_ind *sta_deassoc = KE_MSG_ALLOC(SM_DEASSOC_IND, TASK_API,
                                             TASK_MM, sm_deassoc_ind);
            memcpy(sta_deassoc->mac, (const void *)&sta_mac, sizeof(sta_mac));
            ke_msg_send(sta_deassoc);
        }
        else
        {
            struct sm_assoc_failed_ind *sta_assoc_failed = KE_MSG_ALLOC(SM_ASSOC_FAILED_IND, TASK_API,
                                             TASK_MM, sm_assoc_failed_ind);
            memcpy(sta_assoc_failed->mac, (const void *)&sta_mac, sizeof(sta_mac));
            ke_msg_send(sta_assoc_failed);
        }
    }
...
void mm_hw_machw_callback(void *env)
{
    ke_state_t mm_state;
    uint8_t hw_state;
    GLOBAL_INT_DECLARATION();
    ...
    GLOBAL_INT_DISABLE();
    mm_state = ke_state_get(TASK_MM);
    hw_state = nxmac_current_state_getf();
    if ((HW_IDLE == hw_state) && (MM_GOING_TO_IDLE == mm_state))
    {
        if (!(ke_evt_get() & KE_EVT_HW_IDLE_BIT))
        {
            os_printf("%s:%d MM_IDLE\n", __FUNCTION__, __LINE__);
            ke_state_set(TASK_MM, MM_IDLE);
        }
    }
    GLOBAL_INT_RESTORE();
                     ...
                     ke_task_id_t const src_id)
{
    ...
    switch (ke_state_get(dest_id))
    {
        case MM_IDLE:
            ...
            {
                os_printf("%s hw_state=%d ignore\n", __FUNCTION__, nxmac_current_state_getf());
            }
            ...
            switch (msgid)
            {
                case MM_SET_CHANNEL_REQ:
                    ...
                    break;
				case MM_GET_CHANNEL_REQ:
					status = mm_get_channel_req_handler(msgid, dest_id, src_id);
					break;
        ...
        case MM_GOING_TO_IDLE:
            /**
              * mm is going to idle, and the message would move to queue_saved list
              * HAL_IDLE_TIMER should be trigger in IDLE_REQ_TIMEOUT(us)
              * otherwise 
              */
            mm_env.machw_timer.cb = mm_hw_machw_callback;
            mm_env.machw_timer.env = NULL;
            mm_timer_set(&mm_env.machw_timer, 50000);
            break;
...
const struct ke_msg_handler mm_default_state[] =
{
    ...
    {MM_GET_CHANNEL_REQ, (ke_msg_func_t)mm_hw_config_handler},
};
