...
void mm_timer_save_before_soft_reset(void)
{
	...
	for(;;)
	{
		...
		else if((ke_time() > MM_TIMER_COUNT_HALF_MAX) 
					&& (timepoint < MM_TIMER_DELAY_MAX))
		{
			timer->duration = timepoint + (MM_TIMER_COUNT_MAX - ke_time());
		}
		else
		{
			timer->duration = 100;
		...
		timer = (struct mm_timer_tag *)timer->list_hdr.next;
	}
}
