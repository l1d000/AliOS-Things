...
void txl_cntrl_dec_pck_cnt(void)
{
    GLOBAL_INT_DECLARATION();
    #if NX_POWERSAVE
    GLOBAL_INT_DISABLE();
    // dec the number of packets in the TX path
    txl_cntrl_env.pck_cnt--;
    GLOBAL_INT_RESTORE();
    #endif
}

UINT32 txl_cntrl_pck_get(void )
{
    uint32_t value = 0;
    GLOBAL_INT_DECLARATION();
    GLOBAL_INT_DISABLE();
    value =  txl_cntrl_env.pck_cnt;
    GLOBAL_INT_RESTORE();
    return value;
}
...
void txl_reset(void)
{
    ...
    for (i = 0; i < NX_TXQ_CNT; i++)
    {
        ...
        while (1)
        {
            ...
            agg_desc = (struct tx_agg_desc *)co_list_pop_front(&txlist->aggregates);
            if (agg_desc == NULL)
                break;
