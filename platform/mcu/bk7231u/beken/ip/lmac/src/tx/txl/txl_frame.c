...
uint32_t txl_frame_is_disallowed_hw(struct txl_frame_desc_tag *frame)
{
    uint32_t ac;
    uint32_t disallowed = 0;

    for(ac = AC_BK; ac < AC_MAX; ac ++)
    {
        if(txl_check_hd_is_current(ac, &frame->txdesc))
        {
            disallowed = 1;
        }
    }
...
struct txl_frame_desc_tag *txl_frame_find_desc(void)
{
    uint32_t i;
    ...
    for(i = 0; i < NX_TXFRAME_CNT; i ++)
    {
        ...
        if(frame)
        {
            if(txl_frame_is_disallowed_hw(frame))
            {
                co_list_push_back(&txl_frame_env.desc_free, &frame->txdesc.list_hdr);
                frame = NULL;
            }
...
struct txl_frame_desc_tag *txl_frame_get(int type, int len)
{
    ...
    if (frame != NULL)
    {
        ...
        //#if NX_MFP
        //Delete this marco, in AP+STA, STA with wep shared key error, RW patch
...
uint8_t txl_frame_send_null_frame(uint8_t sta_idx, cfm_func_ptr cfm, void *env)
{
    ...
    do
    {
        if(true == rwnxl_get_reseting_flag())
            {
            //os_printf("can not send null when RECOVER\r\n");
            break;
        }
