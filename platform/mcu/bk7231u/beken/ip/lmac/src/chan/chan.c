...
static void chan_tx_cfm(void *dummy, uint32_t status)
{
    ...
    if (chan_env.cfm_cnt == 0)
    {
        ...
        /**
         * this function is invoked as asynced callback, and the app may call sa_station_init()
         * at the same time, a conflict flow may like this:
         * a. sa_station_init() is invoked and set MM_STATE TO MM_IDLE
         * b. this callback is invoked clear all data and set MM_STATE TO MM_HOST_BYPASSED
         * c. MM_START_REQ is received and then check MM_STATE but wrong
         */
        ke_state_t old_state = ke_state_get(TASK_MM);
        mm_force_idle_req();
        if (old_state == MM_IDLE)
        {
            ke_state_set(TASK_MM, MM_IDLE);
            bk_printf("chan_tx_cfm: TASK_MM_STATE=MM_IDLE, ignore\n");
        }
...
{    
    ...
    struct chan_ctxt_tag *p_chan_entry = vif->chan_ctxt;

    if(p_chan_entry && p_chan_entry->channel.prim20_freq)
    {
        freq = p_chan_entry->channel.prim20_freq;
    }
