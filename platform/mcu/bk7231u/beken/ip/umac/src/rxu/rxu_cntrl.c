...
int wpa_parse_wpa_ie(const u8 *, size_t ,struct wpa_ie_data *);
...
#ifndef CFG_DISABLE_MONITOR_MODE
...
uint32_t rxu_mtr_is_bcast(struct mac_hdr *machdr)
{
	uint8_t *addr = 0;
	uint32_t ds_status;
	uint32_t brd_flag = 0;
	...
	if(addr)
	{
		brd_flag = MAC_ADDR_IS_BSCT(addr);
	}
...
{	
	...
	uint32_t group_cipher_type = CTYPERAM_NULL_KEY;
	...
	if(CIPHER_OPEN_SYSTEM != cipher_type)
	{
		group_cipher_type = CTYPERAM_TKIP;
	}
	...
	uint32_t len)
{
	...
	if(CTYPERAM_NULL_KEY != cipherType)
	{
                                           ...
                                           struct rxu_cntrl_defrag *p_defrag)
{
    ...
    pbuf = pbuf_alloc(PBUF_RAW, mpdu_len, PBUF_RAM);
    
    hostbuf = (uint32_t)pbuf->payload;
	
	if ( p_defrag->dma_addr == 0 )
	{
		p_defrag->dma_addr = CPU2HW(pbuf);
        
        g_rx_info.sta_idx = rx_status->sta_idx;
        g_rx_info.vif_idx = rx_status->vif_idx;
        g_rx_info.dst_idx = rx_status->dst_idx;
        g_rx_info.rssi = (dma_hdrdesc->hd.recvec1c >> 24) & 0xff;
        phy_get_channel(&dma_hdrdesc->phy_info, PHY_PRIM);
        g_rx_info.center_freq = (dma_hdrdesc->phy_info.info1 >> 16) & 0xffff;
        
        dma_desc->src = pd->pbd.datastartptr + rx_status->payl_offset;
	}
	else
	{
		pb = (struct pbuf *)HW2CPU(p_defrag->dma_addr);
 ...
 *
 */
static void rxu_cntrl_defrag_len_update(struct rxu_cntrl_defrag *p_defrag)
{
    struct pbuf *pbuf = NULL, *pb0 = NULL, *pb1 = NULL;
	uint8_t *pd = NULL;

	if(g_rwnx_connector.rx_alloc_func)
	{
		(*g_rwnx_connector.rx_alloc_func)(&pbuf, p_defrag->frame_len);
	}
	else
	{
	    return;
    ...
    pd = (uint8_t *)pbuf->payload;

	pb0 = (struct pbuf *)HW2CPU(p_defrag->dma_addr);

    while ( pb0 )
    {
        pb1 = pbuf_dechain(pb0);
        os_memcpy(pd, (uint8_t *)pb0->payload, pb0->len);
        pd += pb0->len;
        pbuf_free(pb0);
        pb0 = pb1;
    }
    ...
    { 
        g_rx_info.length = p_defrag->frame_len;
        g_rx_info.data = pbuf; 

        (*g_rwnx_connector.data_outbound_func)(&g_rx_info);
    }
    else
    {
        pbuf_free(pbuf);
                                      ...
                                      uint16_t pdu_len)
{
    ...
    do
    {
        ...
        if (!p_defrag)
        {
            ...
            rxu_cntrl_mac2eth_update(swdesc);
            p_defrag->cpy_len -= rx_status->payl_offset;
            p_defrag->frame_len -= rx_status->payl_offset;
        ...
        else
        {
            ...
            else
            {
                ...
                rxu_cntrl_defrag_len_update(p_defrag);
                              ...
                              uint16_t machdr_length)
{
	...
	if (fn) 
	{
		...
		if(mgnt_buf)
		{
			os_memcpy(mgnt_buf, (uint8_t *)payload, length);
			fn(mgnt_buf, length, rssi);
			os_free(mgnt_buf);
		}
		else
		{
			RXU_PRT("-------malloc mgnt buffer fail.\r\n");
		}
	}
#endif
    ...
    {
        // if is beacon or proble rsp, forward this packet 
        if((copy_mac_hdr==true) && (dest_id == TASK_SCANU))
            mode = VIF_STA;
        else if((framectrl & MAC_FCTRL_TYPESUBTYPE_MASK) == MAC_FCTRL_PROBEREQ)
        {
            // if is probereq and ap's vif is exist, need handle by vif_ap
            vif_entry = vif_mgmt_first_used();
            while(vif_entry) 
            {
    ...
    {    
        // if that useful? for sta_id ==  INVALID_STA_IDX
		if(mode == VIF_STA)
		{
        	upload = false;
		}
	...
	if(bk_wlan_is_monitor_mode())
	{
		return rxu_mgt_monitor(framectrl, payload, length, rssi);
	}
    ...
    if (dest_id != TASK_NONE)
    {
		if(mode == VIF_AP)
		{
			ke_mgmt_packet_tx((unsigned char *)payload, length, *vif_idx);
		}
    ...
    do
    {
		...
		{			
        	...
        	{    
        		...
        		{  
        		#ifdef MONITOR_LOG
        		    uint8_t format = 0, chbw = 0;
                    struct mac_hdr *machdr = (struct mac_hdr *)frame;
                    struct mac_addr macaddr3 = {0x22d0,0x8dbe,0x4f4e};
                    
        		    if(MAC_ADDR_CMP(&machdr->addr3, &macaddr3)) {
                	    os_printf("1a:%08x\r\n", rhd->recvec1a);
                        os_printf("1b:%08x\r\n", rhd->recvec1b);
                        os_printf("1c:%08x\r\n", rhd->recvec1c);
                        os_printf("1d:%08x\r\n", rhd->recvec1d);
                        format = (rhd->recvec1b >> 16) & 0x07;
                        chbw = (rhd->recvec1b >> 19) & 0x02;
                        os_printf("f:%03x, b:%02x\r\n", format, chbw);
        		    }
                #endif
