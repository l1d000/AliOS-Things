 ...
 */
static int
sm_get_bss_info_handler(ke_msg_id_t const msgid,
                       void const *param,
                       ke_task_id_t const dest_id,
                       ke_task_id_t const src_id)
{
	struct sm_get_bss_info_cfm *cfm;
    struct sm_get_bss_info_req *req = (struct sm_get_bss_info_req*)param;
    struct vif_info_tag *vif = &vif_info_tab[req->vif_idx];
    struct mac_bss_info *bss = &vif->bss_info;    
    
	cfm = KE_MSG_ALLOC(SM_GET_BSS_INFO_CFM, src_id, dest_id, sm_get_bss_info_cfm);
	cfm->rssi = vif->u.sta.rssi;
	cfm->freq = bss->chan->freq;
	os_memcpy(cfm->bssid, (char *)&bss->bssid, 6);
	os_memcpy(cfm->ssid, bss->ssid.array, bss->ssid.length);
                               ...
                               ke_task_id_t const src_id)
{
	...
	ke_msg_send_basic(SM_BEACON_LOSE_IND, TASK_API, TASK_SM);
                    ...
                    ke_task_id_t const src_id)
{
    ...
    {
        os_printf("rxu_mgt_ind:%x %x\r\n", fctl,ke_state_get(TASK_SM));
    }
...
const struct ke_msg_handler sm_default_state[] =
{
    ...
    {SM_GET_BSS_INFO_REQ, (ke_msg_func_t)sm_get_bss_info_handler},
};
