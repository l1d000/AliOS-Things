...
void sm_connect_ind(uint16_t status)
{
    ...
    if ((NULL == con_par) || (NULL == ind))
    {
        os_printf("%s connect_param=0x%x ind=0x%x, ignore\n", __FUNCTION__, con_par, ind);
        return;
    }
    ...
    else
    {
        ...
        os_printf("SM_DISCONNECTING status=%d\r\n", status);
        // Delete the resources that were allocated for this connection
        sm_delete_resources(vif);
    }
    ...
    // Reset connect_ind to avoid duplicate free
...
void sm_auth_handler(struct rxu_mgt_ind const *param)
{
    ...
    struct sm_connect_req *con_par = sm_env.connect_param;
    ...
    switch (status)
    {
		...
		case MAC_ST_AUTH_ALGO_NOT_SUPPORTED:
            auth_type = co_read16p(payload + MAC_AUTH_ALGONBR_OFT);

			if((auth_type == MAC_AUTH_ALGO_OPEN) || (auth_type == MAC_AUTH_ALGO_SHARED))
			{
				con_par->auth_type = MAC_AUTH_ALGO_SHARED;
	...
	if(status != MAC_ST_SUCCESSFUL)
	{
    	status_ind = KE_MSG_ALLOC(SM_AUTHEN_FAIL_IND, TASK_API,TASK_SM, sm_fail_stat);
		status_ind->status = status;
    	ke_msg_send(status_ind);
	}
}
...
void sm_assoc_rsp_handler(struct rxu_mgt_ind const *param)
{
    ...
    switch (status)
    {
        ...
        default:
			assoc_state = KE_MSG_ALLOC(SM_ASSOC_FAIL_INID, TASK_API, TASK_SM, sm_fail_stat);
			assoc_state->status = status;
			os_printf("%s status=%d\n", __FUNCTION__, status);
			// Send the message to the task
    		ke_msg_send(assoc_state);
			// status is not successful
            #if NX_ROAMING
            if (sm_env.hand_off_ongoing)
            {
...
int sm_deauth_handler(struct rxu_mgt_ind const *param)
{
    ...
    os_printf("%s reason=%d,vif=%d\n", __FUNCTION__, co_read16p(payload + MAC_DEAUTH_REASON_OFT), vif->index);
    do
    {
