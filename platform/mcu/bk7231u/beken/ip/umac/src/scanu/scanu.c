...
void scanu_init(void)
{
    ...
    if (scanu_inited && (NULL != scanu_env.param))
    {
        ke_msg_free(ke_param2msg(scanu_env.param));
    }
    if (0 == scanu_inited)
...
{    
    ...
    do
    {
		...
		// Check if from real AP
		if (MAC_ADDR_CMP(&frm->h.addr2, &frm->h.addr3) == 0)
		{
			break;
		}
        ...
        if (elmt_addr != 0)
        {
            ...
            memset(&new_ssid, 0x00, sizeof(new_ssid));
            
            // first, get the SSID length
            new_ssid.length = co_read8p(elmt_addr + MAC_SSID_LEN_OFT);

            if(new_ssid.length > 0) 
            { 
                /* For hidden ssid's AP, beacon's ssid len is 0, but probe response has ssid */
	            if (new_ssid.length > MAC_SSID_LEN)
	            	new_ssid.length = MAC_SSID_LEN;
                
                scan->ssid.length = new_ssid.length;
                co_unpack8p(new_ssid.array, elmt_addr + MAC_SSID_SSID_OFT, new_ssid.length);
                if((scan->ssid.length)&&(new_ssid.array[0] != '\0'))
                {
                    memcpy(scan->ssid.array,new_ssid.array,scan->ssid.length);
                }
			}
        }
        ...
        #endif //(NX_P2P)
        {
            ...
            if (param->ssid_cnt)
            {
                for (i = 0; i < param->ssid_cnt; i++)
                {
                    ...
                    if ((ssid->length == 0) || (MAC_SSID_CMP(ssid, &new_ssid)))
                    {
        ...
        if (scanu_env.joining)
        {
            ...
            //cal bcn listen interval
            #if CFG_USE_STA_PS
            power_save_cal_bcn_liston_int(bss->beacon_period);
            #endif
                                          ...
                                          bool allocate)
{
    ...
    for (i = 0; i < MAX_BSS_LIST; i++)
    {
        ...
        else if (allocate)
        {
            ...
            scan_rslt = scan;
            memset(scan->ssid.array,0,MAC_SSID_LEN);
            scan->ssid.length = 0;
            break;
        }
    }
