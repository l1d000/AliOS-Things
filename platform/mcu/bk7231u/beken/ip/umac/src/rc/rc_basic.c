...
void rc_basic_check(uint8_t sta_idx)
{
    ...
    if (rc->tx_cnt > ME_RC_PERIOD)
    {
        ...
        #define RATE_DOWN_DIST          1   //(ME_RC_PERIOD/3)
        // Check if we need to do something
        if((tx_cnt - tx_fail < RATE_DOWN_DIST) && (rc->r_idx > rc->r_idx_min))
        {
            if(rc->r_idx > HW_RATE_6MBPS)
                rc->r_idx--;
            else {
                rc->r_idx = rc->r_idx_min;
            }
                
            // Keep in mind we have to update the rate
            rc->upd_field |= CO_BIT(STA_MGMT_POL_UPD_RATE);
        }
        else if((tx_cnt - tx_fail >= RATE_DOWN_DIST) && (rc->r_idx < rc->r_idx_max))
        {
        	/* recovery to high phyrate faster*/
            if((rc->r_idx + 4) < rc->r_idx_max)
            {
            	rc->r_idx = rc->r_idx + 4;
            }
			else
