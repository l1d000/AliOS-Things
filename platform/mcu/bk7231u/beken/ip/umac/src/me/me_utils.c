...
uint16_t me_build_capability(uint8_t vif_idx)
{
	...
	if(bk_sta_cipher_is_open() == 0)
	{
    	capa_info |= MAC_CAPA_PRIVA;// add privacy
	}
    ...
    if (pol_tbl->upd_field)
    {
        ...
        if (pol_tbl->upd_field & CO_BIT(STA_MGMT_POL_UPD_RATE))
        {
            ...
            for (i = 0; i < RATE_CONTROL_STEPS; i++)
            {
                ...
                if(i==0) {
                    //os_printf("i:%d, %d, %d, %08x, %d\r\n", i, mcs_idx, format, new_rate_info,
                    //    mcs_idx & HT_MCS_MASK );
#if CFG_SUPPORT_CALIBRATION
                    CHECK_OPERATE_RF_REG_IF_IN_SLEEP();
                    rwnx_cal_set_txpwr_by_rate((mcs_idx & HT_MCS_MASK) + 128, 0);// set tx output power for change rate
        ...
        if (pol_tbl->upd_field & CO_BIT(STA_MGMT_POL_UPD_RATE))
        {
            ...
            CHECK_OPERATE_RF_REG_IF_IN_SLEEP();
            #endif
            rwnx_cal_set_txpwr_by_rate(r_idx, 0);// set tx output power for change rate
            #if CFG_USE_STA_PS
            CHECK_OPERATE_RF_REG_IF_IN_SLEEP_END();
            #endif
#endif
            ...
            for (i = 0; i < RATE_CONTROL_STEPS; i++)
            {
				...
				if(((RATE_CONTROL_STEPS - 1) == i)
					&& ((HW_RATE_6MBPS == r_idx)
						|| (HW_RATE_9MBPS == r_idx)))
				{
					r_idx = HW_RATE_11MBPS;
				}

                #if CFG_SUPPORT_TPC_PA_MAP
                rate_info[i] &= ~ PROT_FRM_EX_RCX_MASK;
                #endif
            }
        }
