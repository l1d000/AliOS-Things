...
int me_send_ps_req(ke_msg_id_t const msgid,
                       ...
                       ke_task_id_t const src_id)
{
    ...
    {
        me_env.ps_on = param->ps_disable;
    
        if(param->ps_disable == PS_MODE_ON_DYN)
        {
        struct mm_set_ps_mode_req *ps;
        //os_printf("--s:%d set:%d\r\n",ke_state_get(TASK_ME),param->ps_disable);
        ...
        me_env.requester_id = TASK_NONE;
        // Send the message to the LMAC
        ps->new_state = param->ps_disable;
        // Move to BUSY state
        //os_printf("%s:%d me_set:%d\r\n",__FUNCTION__,__LINE__,ME_BUSY);
        ke_state_set(TASK_ME, ME_BUSY);
        ...
        if(param->ps_disable == PS_MODE_ON_DYN)
        {
            power_save_ps_mode_set(PS_DTIM_PS_OPENING);
        }
        GLOBAL_INT_RESTORE();
#endif
                                ...
                                ke_task_id_t const src_id)
{
...
#endif
    {
        ...
        struct sm_assoc_ind *sta_assoc =KE_MSG_ALLOC(SM_ASSOC_IND, TASK_API,
                                                                  TASK_ME, sm_assoc_ind);
        memcpy(sta_assoc->mac, &sta->mac_addr, sizeof(sta->mac_addr));
        ke_msg_send(sta_assoc);
    }
    else if ((VIF_AP == p_vif->type) && (PORT_OPEN == sta->ctrl_port_state))
    {
        struct apm_assoc_ind *sta_assoc =KE_MSG_ALLOC(APM_ASSOC_IND, TASK_API,
                                                                  TASK_ME, apm_assoc_ind);
        memcpy(sta_assoc->mac, &sta->mac_addr, sizeof(sta->mac_addr));
        ke_msg_send(sta_assoc);
    }
                       ...
                       ke_task_id_t const src_id)
{
    ...
    else
    {
        ...
        struct me_mgmt_tx_cfm *cfm;

        cfm = KE_MSG_ALLOC(ME_MGMT_TX_CFM,
                             src_id,
                             dest_id,
                             me_mgmt_tx_cfm);

		if(vif_mgmt_is_ap_inface(param->vif_idx))
        {
            uint16_t *framectrl;
        ...
        } else if(vif_mgmt_is_sta_inface(param->vif_idx))
		{
        	cfm->hostid = param->hostid;
		}

        cfm->status = me_mgmt_tx((struct me_mgmt_tx_req *)param);
		...
		if(vif_mgmt_is_ap_inface(param->vif_idx))
		{
	        if(CO_OK == cfm->status)
	        {
	            ...
	            ke_mgmt_packet_tx((unsigned char *)cfm->hostid, cfm->length, param->vif_idx);
                          ...
                          ke_task_id_t const src_id)
{
            ...
            (!me_env.active_vifs && !param->active))
    {
        ...
        if (src_id != TASK_NONE)
        {
			if((src_id == TASK_APM) && (param->active == false))
	    	{
	    		char *cfm_param = ke_msg_alloc(ME_SET_ACTIVE_CFM, src_id, dest_id, sizeof(char));
				*cfm_param = 0;
				bk_printf("[msg]ME_SET_ACTIVE_CFM param 0\n");
				ke_msg_send(cfm_param);
	    	}
			else
			{
                        ...
                        ke_task_id_t const src_id)
{
    ...
    {
		if(me_env.requester_id == TASK_APM)
    	{
    		char *cfm_param = ke_msg_alloc(ME_SET_ACTIVE_CFM, me_env.requester_id, dest_id, sizeof(char));
			*cfm_param = 0;
			bk_printf("set_active param 0\n");
			ke_msg_send(cfm_param);
    	}
		else
		{
...
#if NX_POWERSAVE
    //os_printf("%s:%d %d %d %d\r\n",__FUNCTION__,__LINE__,me_env.ps_on,
    //    me_env.ps_disable_vifs,param->ps_disable);
    if((mhdr_get_station_status() <  MSG_CONN_SUCCESS) && (param->ps_disable == 0))
        {
        PS_PRT("dis set ps!!\r\n");
        if (src_id != TASK_NONE)
...
#if CFG_USE_STA_PS
        GLOBAL_INT_DISABLE();
        if(ps->new_state == PS_MODE_OFF)
        {
            power_save_ps_mode_set(PS_DTIM_PS_CLOSING);
        }
        else if(ps->new_state == PS_MODE_ON_DYN)
        {
            power_save_ps_mode_set(PS_DTIM_PS_OPENING);
        }
        GLOBAL_INT_RESTORE();
...
#if CFG_USE_STA_PS
    if(power_save_ps_mode_get() == PS_DTIM_PS_CLOSING)
    {
        power_save_dtim_disable_handler();
        if(power_save_wkup_event_get() & NEED_ME_DISABLE_BIT)
        {
            me_env.ps_on = PS_MODE_OFF;
            power_save_wkup_event_clear(NEED_ME_DISABLE_BIT);
        }

    }else  if(power_save_ps_mode_get() == PS_DTIM_PS_OPENING)
    {
        //txl_cntrl_env.pck_cnt = 0;
        power_save_dtim_enable_handler();
    }
    else 
	{
    }
...
const struct ke_msg_handler me_default_state[] =
{
    ...
    {ME_PS_REQ, (ke_msg_func_t)me_send_ps_req},
