...
uint32_t me_add_ie_ext_supp_rates(uint32_t *frame_addr, struct mac_rateset *p_rateset)
{
	...
	if(MAC_SUPPORTED_RATES_LEN > p_rateset->length)
	{
		return 0;
	}
...
uint32_t me_add_ie_ht_capa(uint32_t *frame_addr)
{
    ...
    ht_capa_info &= ~(1<<1 | 1<< 6 | 1<< 10 | 1<< 12);
                                ...
                                struct sm_connect_req const *con_par)
{
    ...
    #if CFG_USE_STA_PS
    listen = power_save_get_liston_int();
    #else
...

{
    ...
    do
    {
        ...
        {
            elmt_length = MAC_RATESET_LEN - mac_rate_set_ptr->length;
        }
