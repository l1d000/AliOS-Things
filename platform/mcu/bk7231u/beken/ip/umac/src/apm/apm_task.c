                          ...
                          ke_task_id_t const src_id)
{
    ...
    if (ke_state_get(TASK_APM) == APM_BSS_PARAM_SETTING)
    {
        ...
        if(!co_list_is_empty(&apm_env.bss_config))
        {
        	bk_printf("[err]&apm_env.bss_config cnt:%d\n",co_list_cnt(&apm_env.bss_config));
	...
	if((param)&&((*((char*)param)) == 0))
	{
		bk_printf("[msg]APM_STOP_CFM\n");
		// Send the confirmation
		#if 1
			struct apm_stop_cfm *cfm = KE_MSG_ALLOC(APM_STOP_CFM, TASK_API, TASK_APM, apm_stop_cfm);
			cfm->status = 0;
			ke_msg_send(cfm);
    	#else
                     ...
                     ke_task_id_t const src_id)
{
    ...
    do
    {
       ...
       /// ke_msg_send_basic(APM_STOP_CFM, src_id, dest_id);
		bk_printf("[msg]wait APM_STOP_CFM\n");

        // We will now proceed to the AP starting
        return (KE_MSG_CONSUMED);
    } while(0);
	
	bk_printf("[msg]apm_stop_req_handler\n");
	#if 1
		struct apm_stop_cfm *cfm = KE_MSG_ALLOC(APM_STOP_CFM, src_id, dest_id, apm_stop_cfm);
		cfm->status = 1;
		ke_msg_send(cfm);
    #else
    // Send the confirmation
