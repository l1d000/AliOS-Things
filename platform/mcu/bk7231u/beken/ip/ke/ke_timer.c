...
void ke_timer_save_before_soft_reset(void)
{
	...
	for(;;)
	{
		...
		if(timepoint > ke_time())
		{
			timer->duration = timepoint - ke_time();
		}
		else if((ke_time() > KE_TIMER_COUNT_HALF_MAX) 
					&& (timepoint < KE_TIMER_DELAY_MAX))
		{
			timer->duration = timepoint + (KE_TIMER_COUNT_MAX - ke_time());
		}
		else
		{
			timer->duration = 100;
		...
		//os_printf("ket1:%x:%d:%d", timer, timer->duration, timer->time);
		timer = timer->next;
	}
}
...
void ke_timer_restore_after_soft_reset(void)
{
	...
	for(;;)
	{
		...
		timer->time = timer->duration + ke_time();
		
		//os_printf("ket2:%x:%d:%d", timer, timer->duration, timer->time);
		timer = timer->next;
	}
